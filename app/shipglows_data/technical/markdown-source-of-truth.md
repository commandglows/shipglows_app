...
This document records the data contract for ShipGlows while database and multi-user choices remain open.
...
- Future sync/database shipglows_data/workflow/specs that project Markdown data
...
- Repository Markdown files are canonical for ShipGlows operational state.
- `shipglows_data/` is the preferred project-local governance corpus for ShipGlows artifacts. Older root-level files are migration fallback when the corpus is missing.
- A ShipGlows project is canonically identified by a GitHub repository (`owner/repo`).
...
Task, audit, and spec summary/index records use the shared ShipGlows traffic-first line format defined in `/home/claude/shipglowz/skills/references/operational-record-format.md`.
...
```text
🔴 [shipglows_app] task: Run /sf-verify | status: todo | area: github-clone-indexer
🟠 [shipglows_app] audit: dependencies | date: 2026-04-27 | overall: C | issues: 0/1/2
🟢 [ShipGlows] spec: ShipGlows Terminal TUI V1 | status: ready | path: shipglows_data/workflow/specs/shipglows-terminal-tui-v1.md | next: /sf-start ShipGlows Terminal TUI V1
```
...
These documents define source metadata, read-model fields, duplicate policy, diagnostics redaction, and deterministic write gates for the future ShipGlows web projection surface.
...
rg -n "service-role|SUPABASE|FIREBASE|Firestore|FastAPI|canonical|source of truth" lib shipglows_data/technical shipglows_data/workflow/specs README.md
```
