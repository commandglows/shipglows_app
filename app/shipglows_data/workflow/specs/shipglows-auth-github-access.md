...
owner: "Diane"
confidence: medium
user_story: "En tant que fondatrice de ShipGlows, je veux separer clairement l'identite Firebase Auth de l'acces GitHub App, afin que ShipGlows puisse lire les repositories autorises sans exposer de secrets ni inventer un systeme de permissions concurrent a GitHub."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
...
evidence:
  ...
  - "User decision 2026-05-10: GitHub App installation can cover all repos or a selected subset; ShipGlows project creation still requires explicit repo selection in ShipGlows."
  - "User decision 2026-05-10: GitHub wins for repository access and can create/update ShipGlows membership automatically."
  - "User decision 2026-05-10: if GitHub access is lost, keep ShipGlows membership but block refresh/indexing and show access-lost status."
  ...
  - "User decision 2026-05-10: users can hide/remove projects from their personal ShipGlows view without deleting shared project data."
  ...
  - "User decision 2026-05-10: existing organization installations require explicit user action before repos are shown in ShipGlows."
...
next_step: "/sf-ready ShipGlows Project Onboarding Flow"
---
# Spec: ShipGlows Auth And GitHub Access
🟢 [shipglows_app] spec: ShipGlows Auth And GitHub Access | status: ready | path: shipglows_data/workflow/specs/shipglows-auth-github-access.md | next: /sf-ready ShipGlows Project Onboarding Flow

# Title

ShipGlows Auth And GitHub Access
...
En tant que fondatrice de ShipGlows, je veux separer clairement l'identite Firebase Auth de l'acces GitHub App, afin que ShipGlows puisse lire les repositories autorises sans exposer de secrets ni inventer un systeme de permissions concurrent a GitHub.

# Minimal Behavior Contract

ShipGlows authenticates people with Firebase Auth and authorizes repository access through a GitHub App checked by a trusted backend. The client can request GitHub connection, repo discovery, project access refresh, and indexing, but the backend verifies Firebase Auth, checks GitHub App installation access, generates short-lived installation tokens only server-side, updates Firestore membership/status summaries, and never exposes tokens or installation internals to Flutter. If GitHub access is lost, the existing Firestore projection remains visible/searchable with a warning, but clone/index/refresh actions are blocked until GitHub access is restored. The easy edge case to miss is treating a ShipGlows membership as stronger than GitHub: for repo-sensitive actions, GitHub remains the final authority.

# Success Behavior

- Given a user signs in with any supported Firebase Auth provider, ShipGlows creates or updates `users/{uid}` without requiring GitHub as login provider.
- Given the user connects GitHub, ShipGlows uses GitHub App installation flow as the primary access model.
- Given the GitHub App is installed on all repos or selected repos, ShipGlows supports both and only creates projects after explicit repo selection inside ShipGlows.
- Given GitHub confirms the user can access a repo with at least read permission, ShipGlows can create/update project membership automatically.
...
- If a user has ShipGlows membership but GitHub no longer confirms repo access, the user can view/search the last projection with warning but cannot refresh, clone, index, or trigger repo-sensitive actions.
...
- Firebase Auth is the ShipGlows identity model and does not have to use GitHub login.
- GitHub wins for repository access.
- ShipGlows membership can cache and summarize access, but cannot override GitHub for server-side repo actions.
...
- Client-visible project/user shipglows_data can expose non-sensitive summaries only.
...
- Official GitHub Apps shipglows_data checked 2026-05-10: GitHub Apps support fine-grained repository permissions and installation-based access.
- Official GitHub installation authentication shipglows_data checked 2026-05-10: installation access tokens are short-lived and should be generated server-side.
- Official GitHub Apps REST shipglows_data checked 2026-05-10: installation tokens can be scoped to repositories and cannot exceed app-granted permissions.
- Official GitHub webhook shipglows_data checked 2026-05-10: installation and repository-selection events can inform backend metadata.
- Official Firebase Auth shipglows_data checked 2026-05-10: Firebase Auth provides app identity.
- Official Cloud Functions callable shipglows_data checked 2026-05-10: callable functions can receive authenticated app requests and backend context.
- Official Firestore Security Rules condition shipglows_data checked 2026-05-10: rules use `request.auth` and must be shaped around allowed paths/queries.

# Invariants

- A Firebase `uid` is not a GitHub identity.
- A GitHub App installation is not a ShipGlows user.
- A ShipGlows project is still exactly one GitHub repository, represented by an opaque `projectId` plus GitHub repository metadata.
...
- Firestore: `shipglows-firestore-data-model.md` must include backend-only installation metadata and client-visible summaries.
- Runner/indexer: `shipglows-github-managed-clone-indexer.md` must call the trusted backend access check before clone/index.
...
- Add this spec to `shipglows_data/technical/shipglows-foundational-architecture.md` owned files.
- Future onboarding spec must cite this spec for GitHub App flow and access-loss UX.
- Future implementation shipglows_data must state that GitHub OAuth traces in legacy ContentFlow are not the target ShipGlows auth model.

# Edge Cases

- GitHub App installed on an organization before a user connects ShipGlows.
...
- GitHub webhook arrives before user explicitly connects GitHub in ShipGlows.
...
- [ ] Task 1: Update the Firestore data model contract for auth/access fields.
  - File: `shipglows_data/technical/firestore-data-model.md`
  ...
  - Validate with: `rg -n "githubInstallations|accessStatus|archived_orphaned|backend-only|membership" shipglows_data/technical/firestore-data-model.md`.
...
- [ ] Task 3: Define trusted backend function contracts.
  - File: `shipglows_data/technical/auth-github-access-contract.md`
  ...
  - Validate with: `rg -n "startGitHubAppInstall|verifyRepositoryAccess|refreshProjectAccess|installation token|redacted" shipglows_data/technical/auth-github-access-contract.md`.
- [ ] Task 4: Add local contract tests and fixtures.
  ...
  - File: `test/shipglows/data/` or nearest existing ShipGlows app-layer test directory.
  ...
  - Validate with: `flutter test test/data/firestore_projection test/shipglows/data`.
- [ ] Task 5: Add docs map and changelog alignment.
  ...
  - Validate with: `rg -n "auth-github-access|GitHub App|Firebase Auth|accessStatus" shipglows_data/technical shipglows_data/editorial CHANGELOG.md`.

# Acceptance Criteria

- Firebase Auth and GitHub App authorization are clearly separate.
- GitHub App is the primary target access model.
- GitHub login is not mandatory for ShipGlows login.
- GitHub wins for repository access.
...
- `checklist_path`: `shipglows_data/workflow/verification/shipglows-auth-github-access.md`.
...
- Technical decision: expose installation account/org label to the UI only as a non-sensitive summary on project/user shipglows_data, not by reading `githubInstallations` directly.
...
- Technical decision: GitHub webhooks update backend metadata but do not by themselves expose repos to users. User action is still required before repo discovery/project creation in ShipGlows.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-05-10 07:52:54 UTC | sf-spec | GPT-5 Codex | Created foundational auth/GitHub access spec from user decisions and security-first defaults. | Draft spec created. | /sf-ready ShipGlows Auth GitHub Access after foundational coherence pass |
| 2026-05-30 16:52:09 UTC | sf-spec | GPT-5 Codex | Repaired readiness gaps: fresh official docs, dependency versions, structured implementation tasks, Test Contract, proof order, scenarios, and exception policy. | reviewed | /sf-ready ShipGlows Auth GitHub Access |
| 2026-05-30 16:53:20 UTC | sf-ready | GPT-5 Codex | Readiness review passed: behavior contract, auth/GitHub separation, security constraints, fresh-docs gate, tasks, Test Contract, and proof path are actionable. | ready | /sf-ready ShipGlows Project Onboarding Flow |
