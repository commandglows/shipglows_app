...
owner: "Diane"
confidence: medium
user_story: "En tant que fondatrice de ShipGlows, je veux un modele Firestore clair pour les utilisateurs, repositories GitHub, projections Markdown, indexations et diagnostics, afin que les shipglows_data/workflow/specs fondamentales restent coherentes avant toute implementation."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
...
depends_on:
  ...
  - artifact: "https://firebase.google.com/shipglows_data/firestore/data-model"
    artifact_version: "checked-2026-05-09"
    required_status: "active"
  - artifact: "https://firebase.google.com/shipglows_data/firestore/security/get-started"
    artifact_version: "checked-2026-05-09"
    required_status: "active"
  - artifact: "https://firebase.google.com/shipglows_data/firestore/security/rules-structure"
    artifact_version: "checked-2026-05-09"
    required_status: "active"
  - artifact: "https://firebase.google.com/shipglows_data/firestore/query-data/get-data"
    artifact_version: "checked-2026-05-09"
    required_status: "active"
supersedes: []
evidence:
  - "User decision 2026-05-09: if two users connect the same GitHub repo, ShipGlows uses one shared project with membership, not duplicate projects."
  ...
  - "User decision 2026-05-09: model minimal ShipGlows roles now: owner and viewer."
...
next_step: "/sf-start ShipGlows Firestore Data Model"
---
# Spec: ShipGlows Firestore Data Model
🟢 [shipglows_app] spec: ShipGlows Firestore Data Model | status: ready | path: shipglows_data/workflow/specs/shipglows-firestore-data-model.md | next: /sf-start ShipGlows Firestore Data Model

# Title

ShipGlows Firestore Data Model
...
- `projects/{projectId}/members/{uid}` stores minimal ShipGlows membership.
...
- Define how one GitHub repo maps to one shared ShipGlows project.
...
- Each ShipGlows project maps to exactly one GitHub repo.
...
- Official Cloud Firestore data model shipglows_data checked 2026-05-09: documents, collections, and subcollections are the base model.
- Official Firestore Security Rules shipglows_data checked 2026-05-09: rules must explicitly authorize paths and must be designed around request/auth/resource data.
- Official Firestore query shipglows_data checked 2026-05-09: client reads should be shaped by document paths and query constraints; rules are not a substitute for product query design.
- `shipglows_data/workflow/specs/shipglows-github-managed-clone-indexer.md` defines runner/indexer producer behavior.
- `shipglows_data/technical/shipglows-foundational-architecture.md` defines GitHub/Markdown as canonical and Firestore as projection.
...
- Data: all future UI, indexing, onboarding, auth, and dashboard shipglows_data/workflow/specs must use this schema vocabulary.
...
- Update `shipglows_data/technical/shipglows-foundational-architecture.md` to list this as the Firestore schema owner.
...
- User loses GitHub access but still has a ShipGlows membership document.
...
1. Create `shipglows_data/technical/firestore-data-model.md`.
   ...
   - Validate with: `rg -n "users/\\{uid\\}|projects/\\{projectId\\}|indexedFiles|indexRuns|diagnostics|projectRefs|feedItems" shipglows_data/technical/firestore-data-model.md`.
2. Add schema/model contract types under `lib/data/firestore_projection/`.
   - Action: define pure Dart contracts for `ShipGlowsUserProfile`, `ShipGlowsProjectRecord`, `ProjectMemberRecord`, `IndexedFileRecord`, `IndexRunRecord`, `DiagnosticRecord`, `UserProjectRef`, and `UserFeedItem`.
...
5. Define unique GitHub repo resolution behavior in `shipglows_data/technical/firestore-data-model.md`.
   ...
   - Depends on: Task 1.
   - Validate with: `rg -n "opaque projectId|duplicate|rename|transfer|server-side lookup" shipglows_data/technical/firestore-data-model.md`.
6. Define projection freshness and deletion behavior in `shipglows_data/technical/firestore-data-model.md`.
   ...
   - Depends on: Task 1.
   - Validate with: `rg -n "sourceCommit|github\\.headCommit|projectionStatus|deleted|stale|fresh" shipglows_data/technical/firestore-data-model.md`.
7. Define index-run retention behavior in shipglows_data and validation helpers.
...
8. Add schema examples in `shipglows_data/technical/firestore-data-model.md`.
...
9. Update documentation indexes.
   - Action: align `shipglows_data/editorial/content-map.md`, `shipglows_data/technical/code-docs-map.md`, and `shipglows_data/technical/shipglows-foundational-architecture.md` with the new Firestore schema document and validation command.
   - Depends on: Tasks 1-8.
   - Validate with: `rg -n "Firestore data model|firestore-data-model|firestore_projection" shipglows_data/editorial/content-map.md shipglows_data/technical/code-docs-map.md shipglows_data/technical/shipglows-foundational-architecture.md`.
...
- Files to read first: `shipglows_data/technical/shipglows-foundational-architecture.md`, `shipglows_data/workflow/specs/shipglows-github-managed-clone-indexer.md`, `shipglows_data/workflow/specs/shipglows-auth-github-access.md`, `shipglows_data/workflow/specs/shipglows-markdown-artifact-governance.md`, `shipglows_data/workflow/specs/shipglows-dashboard-readonly-projection.md`, `shipglows_data/technical/code-docs-map.md`, `shipglows_data/editorial/content-map.md`, and current source-reader models under `lib/data/shipglows_sources/`.
...
- Patterns to reuse: keep pure parsing/model code close to existing `lib/data/shipglows_sources/` conventions and keep tests under a mirrored `test/data/firestore_projection/` directory.
...
- Validation commands: `flutter test test/data/firestore_projection`, `rg -n "users/\\{uid\\}|projects/\\{projectId\\}|sourceCommit|indexRuns|projectRefs|feedItems" shipglows_data/technical/firestore-data-model.md shipglows_data/workflow/specs/shipglows-firestore-data-model.md`, and `rg -n "token|clone path|service credential|installation token" shipglows_data/technical/firestore-data-model.md lib/data/firestore_projection test/data/firestore_projection` with findings manually reviewed for allowed redaction examples only.
...
- Fresh-shipglows_data verdict: checked. The model uses Firestore collections/documents/subcollections and path-based rule design consistent with official Firebase documentation.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-05-09 18:19:39 UTC | sf-spec | GPT-5 Codex | Created foundational Firestore data model spec from user decisions. | Draft spec created. | /sf-ready ShipGlows Firestore Data Model |
| 2026-05-10 10:14:49 UTC | sf-ready | GPT-5 Codex | Readiness gate after foundational coherence corrections. | ready | /sf-start ShipGlows Firestore Data Model |
| 2026-05-10 15:42:22 UTC | sf-build | GPT-5 Codex | Implemented Firestore data model shipglows_data, Dart contracts, path builders, validators, and tests. | implemented | Decide closure/ship scope for this chantier. |
...
| sf-start | done | Implemented shipglows_data/technical/firestore-data-model.md and `lib/data/firestore_projection/*` with pure Dart contracts, path builders, and validators. |
