...
scope: "Legacy ContentFlow inventory and ShipGlows reuse decisions"
owner: "Diane"
confidence: medium
risk_level: high
security_impact: yes
...
linked_systems:
  ...
  - "AGENT.md"
evidence:
  - "README.md presents ShipGlows Operations Dashboard as local-first, read-only, no auth, no backend."
  - "pubspec.yaml package name is shipglows_app."
  - "lib/main.dart defaults APP_TARGET to shipglows but still exposes legacy/contentflow runtime target."
  - "lib/shipglows/ contains the current ShipGlows dashboard runtime."
  ...
  - "CLAUDE.md, AGENT.md, shipglows_data/workflow/TASKS.md and most shipglows_data/workflow/specs still describe ContentFlow, Clerk, FastAPI, feedback, OpenRouter and content pipeline work."
depends_on: []
supersedes: []
next_step: "/sf-spec Purge and adapt legacy ContentFlow scope for ShipGlows"
---

# Exploration Report: Legacy ContentFlow -> ShipGlows Inventory

## Starting Question

ShipGlows was split or migrated from a ContentFlow Flutter codebase. The current
main branch launches ShipGlows by default, but a large ContentFlow runtime remains.
...
- `README.md` - ShipGlows is documented as local-first, read-only, no auth, no cloud sync, no backend.
- `pubspec.yaml` - package is `shipglows_app`.
- `lib/main.dart` - default target is ShipGlows, but `APP_TARGET=legacy|contentflow` still exists.
- `lib/shipglows/` - current dashboard app: overview, project detail, diagnostics, settings.
- `lib/data/shipglows_sources/` - current source reader/parser boundary for Markdown and ledgers.
...
- `shipglows_data/workflow/TASKS.md`, `CLAUDE.md`, `AGENT.md`, `shipglows_data/workflow/specs/` - mostly legacy ContentFlow tracking and specs.
...
for ShipGlows V2: auth, BYOK/OpenRouter, feedback, project models, settings,
...
| Brick | Current origin | Possible ShipGlows value | Initial bucket |
|---|---|---|---|
| `lib/shipglows/` dashboard | ShipGlows | Core runtime | Keep |
| `lib/data/shipglows_sources/` | ShipGlows | Markdown/ledger database reader | Keep |
| `lib/domain/project_health/` | ShipGlows | Project posture and next command logic | Keep |
...
| `lib/data/models/auth_session.dart` | ContentFlow | ShipGlows will need auth eventually | Keep as concept, redesign |
| `lib/data/services/offline_storage_service.dart` | ContentFlow | Useful queue/cache primitives | Keep as concept, adapt later |
| `lib/data/services/feedback_service.dart` | ContentFlow | User feedback is desired for ShipGlows too | Keep as concept, adapt later |
| `lib/data/services/notification_service.dart` | ContentFlow | FCM scaffold may be useful later | Archive/adapt |
| `lib/core/openrouter_guard.dart` | ContentFlow | BYOK/OpenRouter desired for ShipGlows too | Keep concept, decouple from ApiService |
...
| `lib/router.dart` | ContentFlow | Route structure not ShipGlows-native | Archive |
...
| `lib/presentation/screens/onboarding/` | ContentFlow | ShipGlows may need project/source onboarding | Adapt carefully |
| `feed/editor/history/calendar` | ContentFlow | Maybe useful if ShipGlows manages indexed content | Park |
| `angles/personas/ritual/seo/newsletter/research/reels` | ContentFlow | Mostly ContentFlow product pipeline | Park / transfer |
| `drip` | ContentFlow | Could become ShipGlows content publishing later | Park |
| `activity/runs/templates/analytics/performance/uptime/work_domains` | ContentFlow/System | Some operational dashboard ideas may fit ShipGlows | Review one by one |
| `shipglows_data/workflow/specs/*contentflow*` and FastAPI shipglows_data/workflow/specs | ContentFlow | Not ShipGlows source of truth | Transfer/archive |
| `shipglows_data/workflow/TASKS.md`, `CLAUDE.md`, `AGENT.md` | ContentFlow | Actively misleading for ShipGlows | Rewrite for ShipGlows |

## Option Space

### Option A: Hard purge now

- Summary: Delete all legacy ContentFlow runtime and shipglows_data now.
- Pros: Small, clean ShipGlows repo quickly.
...
- Cons: Keeps architecture confusion, stale tests, stale shipglows_data and misleading decisions.
- Verdict: Not enough.
...
1. Freeze the current ShipGlows runtime boundary:
   - `lib/shipglows/`
   - `lib/data/shipglows_sources/`
   - `lib/domain/project_health/`
...
1. Is ShipGlows a local operator cockpit only, or will it become a hosted user product?
2. Is the target user only you, or other users too?
3. Should ShipGlows remain desktop-first, or is web/mobile important soon?
...
9. Which entities are real ShipGlows entities: projects, tasks, shipglows_data/workflow/specs, audit events, operations, dependencies, content items, users, workspaces?
...
13. Is FastAPI still acceptable for local/dev tooling, or should it be excluded from ShipGlows?
...
17. Does ShipGlows auth need Google login only, email/password, GitHub, or all three?
...
21. Is OpenRouter BYOK needed for ShipGlows V1 or later?
...
26. Is feedback for users of ShipGlows, or operator notes for yourself?
...
30. Is feedback a core ShipGlows feature or just support tooling?
...
33. Could drip/publishing become relevant to ShipGlows, or should it be transferred out?
34. Do activity/runs/templates/performance/uptime have ShipGlows equivalents?
...
40. Should README document only ShipGlows, or also mention archived legacy mode?
...
- Reusing the archived Supabase WIP as ShipGlows direction - it mostly reflects a ContentFlow migration path.
...
- User story seed: As the ShipGlows operator, I want the repo separated into current ShipGlows runtime, reusable future ShipGlows bricks, archived ContentFlow code, and transferred/deleted legacy pieces, so future work can proceed without losing useful code or following stale ContentFlow assumptions.
...
- Invariants/constraints seed:
  ...
  - ShipGlows default runtime must remain working.
...
- Validation seed:
  ...
  - targeted ShipGlows source/parser tests
  - `rg` checks for active ContentFlow references after each staged cleanup
  - manual launch of ShipGlows dashboard

## Handoff

- Recommended next command: `/sf-spec Purge and adapt legacy ContentFlow scope for ShipGlows`
- Why this next step: The work crosses shipglows_data, runtime, tests, scripts and product decisions; it should not be executed as a casual cleanup.

## Exploration Run History

| Date UTC | Prompt/Focus | Action | Result | Next step |
|----------|--------------|--------|--------|-----------|
| 2026-05-08 | Inventory legacy ContentFlow before deleting anything | Read runtime, shipglows_data, routes, services, models and shipglows_data/workflow/specs; classified reusable bricks and decision questions | Staged extraction recommended | Answer direction questions, then create spec |
