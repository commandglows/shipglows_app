...
import 'package:shipglowz_app/data/shipglowz_sources/local_fake_clone_indexer.dart';
...
void main() {
  ...
  group('LocalFakeCloneIndexer', () {
    test('indexes allowed Markdown into Firestore-shaped records', () async {
      ...
      File(
        '${repo.path}/shipglowz_data/technical/firestore-data-model.md',
      ).writeAsStringSync('''
---
...
''');
    ...
    test(
      ...
      () async {
        ...
        File('${repo.path}/shipglowz_data/business/business.md')
          ..createSync(recursive: true)
          ..writeAsStringSync('# ok');
        File('${repo.path}/shipglowz_data/editorial/content-map.md')
          ..createSync(recursive: true)
          ..writeAsStringSync('---\nartifact: [bad\n---\n# bad');
        File('${repo.path}/shipglowz_data/technical/large.md')
          ..createSync(recursive: true)
          ..writeAsStringSync('# ${List.filled(80, 'x').join()}');
        File('${repo.path}/shipglowz_data/workflow/TASKS.md')
          ..createSync(recursive: true)
          ..writeAsStringSync('# ${List.filled(40, 'y').join()}');

        final previous = IndexedFileRecord(
          fileId: 'old_file',
          path: 'shipglowz_data/workflow/specs/deleted.md',
          artifactType: 'spec',
...
Future<Directory> _repoFixture() async {
  final repo = await Directory.systemTemp.createTemp('shipglowz_fake_repo_');
  Directory('${repo.path}/shipglowz_data/business').createSync(recursive: true);
  Directory('${repo.path}/shipglowz_data/editorial').createSync(recursive: true);
  Directory('${repo.path}/shipglowz_data/technical').createSync(recursive: true);
  Directory(
    '${repo.path}/shipglowz_data/workflow/specs',
  ).createSync(recursive: true);
