...
import 'package:shipglowz_app/data/shipglowz_sources/parsers/audit_log_parser.dart';
import 'package:shipglowz_app/data/shipglowz_sources/parsers/tasks_parser.dart';

void main() {
  group('TasksParser', () {
    test('prefers canonical task records and suppresses duplicates', () {
      const markdown = '''
🟠 [shipglowz_app] task: Harden parser | status: in_progress | area: parser
🟢 [shipglowz_app] task: Harden parser | status: todo | area: parser
''';
      final output = TasksParser().parse(markdown: markdown, source: 'TASKS.md');
      expect(output.records.length, 1);
      expect(output.records.single.project, 'shipglowz_app');
...
''';
  ...
  group('AuditLogParser', () {
    test('prefers canonical audit records over duplicate legacy table rows', () {
      const markdown = '''
🟡 [shipglowz_app] audit: dependencies | date: 2026-05-22 | overall: B | issues: 0/0/1

| Date | Scope | Deps | Overall | Issues | Project |
| --- | --- | --- | --- | --- | --- |
| 2026-05-22 | dependencies | — | B | 0/0/1 | shipglowz_app |
''';
