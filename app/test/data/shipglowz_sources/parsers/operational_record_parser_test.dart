...
import 'package:shipglowz_app/data/shipglowz_sources/parsers/operational_record_parser.dart';

void main() {
  group('OperationalRecordParser', () {
    test('parses valid records and normalizes legacy checkmark traffic', () {
      const markdown = '''
✅ [shipglowz_app] task: Fix parser \\| escaping | status: in_progress | area: parser\\[core\\]
🟠 [ShipGlowz] audit: dependencies | date: 2026-05-22 | overall: C | issues: 0/1/2
''';
    ...
    test('parses web-reader fixture records with source metadata', () {
      final markdown = File(
        'test/data/shipglowz_sources/fixtures/operational_records_web_reader.md',
      ).readAsStringSync();
      final output = OperationalRecordParser().parse(
        markdown: markdown,
      ...
      expect(escaped.dedupeKey, contains('task|shipglowz_app|title|escaped |title|area|infra'));

      final projected = task.firstWhere(
        (record) => record.title == 'Web reader task with filter and priority',
      );
      expect(projected.project, 'shipglowz_app');
      ...
      expect(projected.dedupeKey, contains('task|shipglowz_app|title|web reader task with filter and priority|area|reader'));
    ...
    test('reports malformed record and keeps parsing neighbors', () {
      const markdown = '''
🔴 [shipglowz_app] task Broken line without kind separator
🟢 [shipglowz_app] task: valid | status: todo
''';
      final output = OperationalRecordParser().parse(
        markdown: markdown,
        source: 'TASKS.md',
