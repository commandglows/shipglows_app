...
import 'package:shipglows_app/data/shipglows_sources/parsers/audit_log_parser.dart';
import 'package:shipglows_app/data/shipglows_sources/parsers/tasks_parser.dart';

void main() {
  group('TasksParser', () {
    test('prefers canonical task records and suppresses duplicates', () {
      const markdown = '''
🟠 [shipglows_app] task: Harden parser | status: in_progress | area: parser
🟢 [shipglows_app] task: Harden parser | status: todo | area: parser
''';
      final output = TasksParser().parse(markdown: markdown, source: 'TASKS.md');
      expect(output.records.length, 1);
      expect(output.records.single.project, 'shipglows_app');
...
''';
  ...
  group('AuditLogParser', () {
    test('prefers canonical audit records over duplicate legacy table rows', () {
      const markdown = '''
🟡 [shipglows_app] audit: dependencies | date: 2026-05-22 | overall: B | issues: 0/0/1

| Date | Scope | Deps | Overall | Issues | Project |
| --- | --- | --- | --- | --- | --- |
| 2026-05-22 | dependencies | — | B | 0/0/1 | shipglows_app |
''';
