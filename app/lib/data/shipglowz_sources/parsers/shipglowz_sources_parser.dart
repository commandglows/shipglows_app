...
class ParsedShipGlowzData {
  const ParsedShipGlowzData({
...
class ShipGlowzSourcesParser {
  ShipGlowzSourcesParser({
  ...
  ParsedShipGlowzData parse(SourceSnapshot snapshot) {
    ...
    final auditDoc = documents.firstWhere(
      (doc) =>
          ...
          doc.path.contains('/shipglowz_data/'),
      orElse: () =>
          const SourceDocument(path: '', redactedPath: '', content: ''),
    );
    ...
    for (final doc in documents.where(
      (doc) => doc.path.contains('/shipglowz_data/workflow/specs/'),
    )) {
