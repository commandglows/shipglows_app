...
class LedgerEventParser {
  ...
  ParserOutput<LedgerEvent> parse({
    ...
    final starts = RegExp(
      r'<!--\s*shipglowz:event start\s*-->',
    ).allMatches(markdown).length;
    final ends = RegExp(
      r'<!--\s*shipglowz:event end\s*-->',
    ).allMatches(markdown).length;
    if (starts != ends) {
      diagnostics.add(
        SourceDiagnostic(
          ...
          details: diagnosticDetails({
            ...
            'endSentinels': ends,
            'expectedBlockFormat':
                '<!-- shipglowz:event start --> + fenced yaml + <!-- shipglowz:event end -->',
          }),
          suggestedCommand: '/sf-verify ShipGlowz event log',
        ),
      );
    }

    final blocks = RegExp(
      r'<!--\s*shipglowz:event start\s*-->\s*```yaml\s*([\s\S]*?)\s*```\s*<!--\s*shipglowz:event end\s*-->',
      multiLine: true,
    ).allMatches(markdown);
