...
🟢 [contentflow_app] spec: Migrate Flutter Core Majors: Riverpod, GoRouter, Google Fonts, Riverpod Codegen | status: ready | path: shipglowz_data/workflow/specs/SPEC-migrate-flutter-core-majors.md | next: /sf-start Migrate Flutter Core Majors: Riverpod, GoRouter, Google Fonts, Riverpod Codegen
...
Ready. This spec resolves the readiness gaps found on 2026-04-27: open hypotheses are closed, metadata is coherent with local shipglowz_data, current official dependency shipglowz_data are named, codegen major policy is explicit, and security impact is treated as low but real because the migration touches auth-adjacent route/state behavior and hosted packages.
...
- Action: le mainteneur exécute la migration par lots ordonnés: baseline, Riverpod runtime, Riverpod API fixes, annotation/generator, GoRouter, Google Fonts, full validation, shipglowz_data.
...
- Mettre à jour `CHANGELOG.md` et `shipglowz_data/technical/guidelines.md` seulement si la migration change commandes, contraintes, imports legacy ou politique codegen.
...
- Respecter `shipglowz_data/technical/guidelines.md`: ne jamais bypasser `AppAccessState` pour le route gating.
...
- Les versions exactes doivent être confirmées par `flutter pub outdated`, `flutter pub get` et shipglowz_data officielles actuelles pendant l'implémentation.
...
Fresh external shipglowz_data verdict: fresh-shipglowz_data checked on 2026-04-27.

- `flutter_riverpod`: pub.dev versions page shows latest stable `3.3.1`, minimum Dart SDK `3.7`; official changelog and Riverpod migration shipglowz_data identify relevant breaks: legacy provider imports, `AsyncValue.valueOrNull` removed, `AsyncValue.value` null-on-error, `ProviderObserverContext`, automatic retry, paused listeners, provider update filtering by `==`, ref lifecycle changes, and `ProviderException`.
...
- `google_fonts`: pub.dev page shows latest stable `8.0.2`; shipglowz_data state support for HTTP fetching, caching and asset bundling; changelog notes WOFF2/WOFF bundled web font changes and font catalog additions/removals.

Official sources consulted:

- https://pub.dev/packages/flutter_riverpod/versions
- https://pub.dev/packages/flutter_riverpod/changelog
- https://riverpod.dev/shipglowz_data/whats_new
- https://riverpod.dev/shipglowz_data/3.0_migration
...
- `shipglowz_data/technical/guidelines.md`: update only if the implementation establishes a new convention for `package:flutter_riverpod/legacy.dart`, Riverpod retry policy, GoRouter matching, or Riverpod codegen policy.
...
- Business, pricing, onboarding copy, screenshots and support shipglowz_data: no change expected because the user-facing feature contract is intentionally unchanged.
- Existing behavior shipglowz_data/workflow/specs remain contracts to revalidate, not shipglowz_data to rewrite: `SPEC-offline-sync-v2.md`, `spec-no-ui-jump-on-resume.md`, `SPEC-project-flows-selection-onboarding-archive.md`.
...
- [x] Tâche 2 : Establish rollback checkpoints by lot
  ...
  - Action : Record previous package versions and planned file groups for Riverpod runtime, Riverpod API, codegen, GoRouter, Google Fonts and shipglowz_data.
  - User story link : Lets maintainers reduce risk without global resets.
  - Depends on : Tâche 1
  - Validate with : `git diff --stat` before first edit and `shipglowz_data/workflow/specs/migrate-flutter-core-majors-baseline.md`
...
- [x] Tâche 25 : Update developer shipglowz_data/changelog
  - Fichier : `CHANGELOG.md`
  - Action : Add migration note with final package majors, validation commands and no-Turso conclusion; update `shipglowz_data/technical/guidelines.md` only for new Riverpod/GoRouter conventions.
  - User story link : Helps future maintainers understand migration consequences.
  - Depends on : Tâche 24
  - Validate with : manual shipglowz_data diff review
  - Notes : Completed on 2026-04-27 with migration entry in `CHANGELOG.md` and concise Riverpod 3 legacy/codegen policy note in `shipglowz_data/technical/guidelines.md`. No business/branding shipglowz_data changed.

- [x] Tâche 26 : Final rollback and security review
  - Fichier : `pubspec.yaml`, `pubspec.lock`, touched Dart/test/shipglowz_data files
  ...
  - Depends on : Tâche 25
  - Validate with : `git diff --stat`, `git diff -- pubspec.lock`, route test results and shipglowz_data review
...
- [x] CA 17 : Given implementation changes developer workflow or conventions, when shipglowz_data are reviewed, then `CHANGELOG.md` and any relevant `shipglowz_data/technical/guidelines.md` updates are present.
...
7. Run full gate, smoke, shipglowz_data, rollback/security review.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-04-27 | sf-verify | GPT-5 | Verified Flutter core major migration against spec, diff, official dependency shipglowz_data, package resolution, analyze/test/codegen checks, and Clerk runtime smoke availability. | partial | Run Clerk/web runtime smoke, add migration changelog/shipglowz_data note, then rerun sf-verify. |
| 2026-04-27 | sf-verify | GPT-5 | Re-verified after Clerk smoke success, shipglowz_data/changelog update, build artifact cleanup, bug gate, dependency/source scan, secret scan, analyze, tests, and codegen gate. | verified | Run /sf-end to close the chantier. |
| 2026-04-27 | sf-end | GPT-5 | Closed the migration chantier after verified checks, shipglowz_data, task tracker updates, and final security/rollback review. | closed | Run /sf-ship to commit and push the scoped migration changes. |
| 2026-04-27 | sf-ship | GPT-5 | Shipped scoped Flutter core major migration changes after green analyze, tests, codegen, Clerk smoke, shipglowz_data, and tracker updates. | shipped | None. |
