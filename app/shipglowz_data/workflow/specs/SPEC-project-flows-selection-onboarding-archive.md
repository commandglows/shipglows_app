...
🟢 [contentflow_app] spec: Project Flows: Optional Source URL, Explicit No Selection, Active Project Editing, and Archive-First Deletion | status: ready | path: shipglowz_data/workflow/specs/SPEC-project-flows-selection-onboarding-archive.md | next: /sf-start Project flows selection onboarding archive
## Title
...
- Review `contentflow_app/shipglowz_data/technical/guidelines.md`, `contentflow_app/README.md`, and settings/onboarding support copy for GitHub-only project language. Update any text that says projects must be GitHub repositories.
- Review backend shipglowz_data in `contentflow_lab/README.md` or project API shipglowz_data if they document `/api/projects` as GitHub-only or hard-delete-only.
...
- Stop conditions:
  ...
  - If existing project source/crawl shipglowz_data/workflow/specs already define a conflicting canonical source field, stop and update this spec before implementation rather than implementing both.
