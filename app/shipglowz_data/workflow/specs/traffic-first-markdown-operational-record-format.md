...
owner: "Diane"
confidence: medium
user_story: "En tant qu'opératrice ShipGlowz, je veux que les tâches, audits et specs utilisent des lignes Markdown traffic-first avec le projet visible au début, afin que les fichiers bruts, le TUI et la future application web lisent les mêmes signaux opérationnels sans normalisation divergente."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
...
depends_on:
  ...
  - artifact: "shipglowz_data/technical/code-docs-map.md"
    artifact_version: "0.1.0"
    required_status: "draft"
  - artifact: "/home/claude/shipglowz/shipglowz_data/workflow/specs/shipglowz-terminal-tui-v1.md"
    artifact_version: "0.1.0"
...
evidence:
  ...
  - "lib/data/shipglowz_sources/parsers/tasks_parser.dart currently derives task state from project sections and table/status text."
  - "lib/data/shipglowz_sources/parsers/audit_log_parser.dart currently expects Markdown audit tables with Date and Project columns."
  - "lib/data/shipglowz_sources/parsers/specs_parser.dart currently parses spec frontmatter and flow sections."
...
- System effect: Flutter readers, TUI readers, tests, docs, and ShipGlowz writer protocols agree on one canonical operational record grammar while preserving legacy table compatibility during migration.
...
- User/operator response: Readers emit source diagnostics with file, line, excerpt, and suggested ShipGlowz command instead of silently dropping the entire file.
...
ShipGlowz Markdown files are the canonical operational source, but current task and audit trackers rely heavily on wide tables and project sections. The TUI has started converting those tables into readable traffic-first display lines, yet the raw Markdown still varies by file and workflow. This makes the source harder to scan, forces each app surface to repeat normalization logic, and allows writer skills to keep creating divergent formats.
...
Define a canonical traffic-first operational record line format for tasks, audits, and spec summary/index records. Update project docs, shared ShipGlowz writer-skill references, Flutter parsers, TUI readers, tests, and migration tooling so new records are created correctly at the source while old tables remain readable during a controlled compatibility period.
...
- Update `shipglowz_data/technical/markdown-source-of-truth.md` with grammar, field rules, examples, diagnostics, and compatibility policy.
- Update `shipglowz_data/technical/code-docs-map.md` with parser and writer update triggers if ownership changes.
- Add a shared ShipGlowz-owned reference for writer skills, for example `/home/claude/shipglowz/skills/references/operational-record-format.md`.
...
- Update Flutter source parsers under `lib/data/shipglowz_sources/parsers/` to parse traffic-first records before falling back to legacy tables.
...
- Runtime:
  - Flutter/Dart parsers in `lib/data/shipglowz_sources/parsers/`.
  - TUI TypeScript readers in `tui/src/sources/`.
  - ShipGlowz-owned skills and references under `/home/claude/shipglowz/skills/`.
  - Global trackers under `/home/claude/shipglowz_data/`.
- Document contracts:
  - `explorations/2026-05-22-markdown-operational-record-format.md@1.0.0`.
  - `shipglowz_data/technical/markdown-source-of-truth.md@0.1.0`.
  - `shipglowz_data/technical/code-docs-map.md@0.1.0`.
  - `/home/claude/shipglowz/shipglowz_data/workflow/specs/shipglowz-terminal-tui-v1.md@0.1.0`.
- Metadata gaps:
  ...
  - Some existing ShipGlowz writer skills may encode tracker write rules only in prose; readiness must identify the exact skills to patch.
- Fresh external docs: not needed. This spec changes local Markdown conventions, local parsers, local tests, and local ShipGlowz skill instructions; it does not depend on external SDK, framework, API, auth, build, routing, or service behavior.
...
- Update `shipglowz_data/technical/markdown-source-of-truth.md` with the operational record grammar, examples, required fields, legacy compatibility, diagnostics, and migration policy.
- Update `shipglowz_data/technical/code-docs-map.md` if parser ownership, validation commands, or update triggers change.
- Update `shipglowz_data/technical/terminal-tui.md` so the TUI docs describe canonical record parsing instead of display-only normalization.
...
- [ ] Task 1: Create the shared operational record contract.
  ...
  - Validate with: `python3 /home/claude/shipglowz/tools/shipglowz_metadata_lint.py /home/claude/shipglowz/skills/references/operational-record-format.md`
...
- [ ] Task 2: Update project source-of-truth docs.
  ...
  - Depends on: Task 1.
  - Validate with: `python3 /home/claude/shipglowz/tools/shipglowz_metadata_lint.py shipglowz_data/technical/markdown-source-of-truth.md`
...
- [ ] Task 3: Update documentation ownership map.
  ...
  - Depends on: Task 2.
  - Validate with: `python3 /home/claude/shipglowz/tools/shipglowz_metadata_lint.py shipglowz_data/technical/code-docs-map.md`
  - Notes: Mention the shared ShipGlowz-owned reference by absolute path because it lives outside the project repo.

- [ ] Task 4: Add Flutter operational record parser primitives.
  - File: `lib/data/shipglowz_sources/parsers/operational_record_parser.dart`
...
- [ ] Task 5: Extend parsed models for canonical records.
  - File: `lib/data/shipglowz_sources/parsers/parsed_models.dart`
...
- [ ] Task 6: Update task parsing with canonical-first fallback.
  - File: `lib/data/shipglowz_sources/parsers/tasks_parser.dart`
...
- [ ] Task 7: Update audit parsing with canonical-first fallback.
  - File: `lib/data/shipglowz_sources/parsers/audit_log_parser.dart`
...
- [ ] Task 8: Update spec parsing for operational summaries.
  - File: `lib/data/shipglowz_sources/parsers/specs_parser.dart`
...
- [ ] Task 9: Add Flutter parser tests and fixtures.
  - File: `test/data/shipglowz_sources/parsers/operational_record_parser_test.dart`
...
- [ ] Task 16: Migrate active Markdown sources after compatibility is proven.
  ...
  - File: `shipglowz_data/workflow/AUDIT_LOG.md`
  - File: `shipglowz_data/workflow/specs/*.md`
  - File: `/home/claude/shipglowz_data/TASKS.md`
  - File: `/home/claude/shipglowz_data/AUDIT_LOG.md`
  ...
  - Depends on: Task 15.
  - Validate with: `flutter test test/data/shipglowz_sources && cd tui && bun test && bun run typecheck`
...
- [ ] Task 17: Run final coherence validation.
  - File: `shipglowz_data/workflow/specs/traffic-first-markdown-operational-record-format.md`
  ...
  - Validate with: `python3 /home/claude/shipglowz/tools/shipglowz_metadata_lint.py shipglowz_data/technical/markdown-source-of-truth.md shipglowz_data/technical/code-docs-map.md shipglowz_data/workflow/specs/traffic-first-markdown-operational-record-format.md && flutter test test/data/shipglowz_sources && cd tui && bun test && bun run typecheck`
  - Notes: This spec should then move to `/sf-verify` before closure.

## Acceptance Criteria

- [ ] AC 1: Given a task record such as `🔴 [shipglowz_app] task: Run /sf-verify | status: todo`, when Flutter and TUI readers parse it, then both expose traffic `🔴`, project `shipglowz_app`, kind `task`, title `Run /sf-verify`, and status `todo`.
...
- Unit:
  - `flutter test test/data/shipglowz_sources` for Dart parser primitives, task parser, audit parser, spec parser, diagnostics, and legacy fallback.
...
- Manual:
  ...
  - Use the TUI project filter to inspect tasks, audits, and specs for at least `shipglowz_app` and one global project.
...
- Workflow risk: high, because several skills may continue writing old tables if only the app parsers are updated. Mitigate by creating a shared ShipGlowz-owned reference and updating writer skills before migrating live files.
...
- Validate with:
  - `python3 /home/claude/shipglowz/tools/shipglowz_metadata_lint.py shipglowz_data/workflow/specs/traffic-first-markdown-operational-record-format.md`
  - `python3 /home/claude/shipglowz/tools/shipglowz_metadata_lint.py shipglowz_data/technical/markdown-source-of-truth.md shipglowz_data/technical/code-docs-map.md`
