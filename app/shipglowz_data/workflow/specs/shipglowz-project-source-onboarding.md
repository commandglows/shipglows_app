...
owner: "Diane"
confidence: medium
user_story: "En tant que fondatrice de ShipGlowz, je veux onboarder des projets ShipGlowz comme repositories GitHub canoniques, afin que le dashboard sache quel repo lire, indexer et synchroniser sans separer artificiellement projet et repo GitHub."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
...
depends_on:
  ...
  - artifact: "shipglowz_data/technical/shipglowz-legacy-reuse-roadmap.md"
    artifact_version: "0.1.0"
    required_status: "draft"
  - artifact: "shipglowz_data/technical/legacy-contentflow-inventory.md"
    artifact_version: "0.1.0"
    required_status: "draft"
  - artifact: "shipglowz_data/technical/recovered-branch-reality.md"
    artifact_version: "0.1.0"
    required_status: "draft"
...
evidence:
  ...
  - "User direction on 2026-05-09: a ShipGlowz project is necessarily a GitHub repository."
  ...
  - "Local branch backup/local-supabase-wip-2026-05-08 contains unpushed Supabase migration WIP, including shipglowz_data/workflow/specs/full-supabase-migration.md and Supabase service/migration files."
  - "Current active ShipGlowz runtime reads Markdown and project registries but has no first-class onboarding flow."
  - "Legacy ContentFlow project/onboarding code exists but is backend/workspace/content-pipeline oriented."
next_step: "/sf-ready ShipGlowz Project Source Onboarding"
---
# Spec: ShipGlowz Project Source Onboarding
🟢 [shipglowz_app] spec: ShipGlowz Project Source Onboarding | status: ready | path: shipglowz_data/workflow/specs/shipglowz-project-source-onboarding.md | next: /sf-ready ShipGlowz Project Source Onboarding

# Title

ShipGlowz Project Source Onboarding

# Status

Ready spec. User decisions captured on 2026-05-09: in ShipGlowz, a project is necessarily a GitHub repository; each onboarded project has a mandatory infrastructure-managed clone for repository/Markdown indexing; a remote database exists as projection/sync/index layer. V1 is read-only.

# User Story

En tant que fondatrice de ShipGlowz, je veux onboarder des projets ShipGlowz comme repositories GitHub canoniques, afin que le dashboard sache quel repo lire, indexer et synchroniser sans separer artificiellement projet et repo GitHub.

# Minimal Behavior Contract

Le chantier definit un modele `ShipGlowzProject` dont l'identite canonique est un repository GitHub (`owner/repo`, URL GitHub, provider metadata). L'interface permet de connecter ou declarer un repo GitHub, puis l'infrastructure ShipGlowz verifie ou cree un clone gere et indexe les fichiers Markdown attendus. La base distante est une projection/sync/index du repo et du clone gere; elle ne remplace pas le repository GitHub ni les Markdown comme source de verite. L'edge case facile a rater est d'exposer le chemin du clone comme preference utilisateur alors qu'il doit rester un detail d'infrastructure.

# Success Behavior

- Etant donne une URL GitHub valide, l'utilisateur peut la declarer comme projet ShipGlowz.
...
ShipGlowz sait lire des sources Markdown deja referencees, mais il n'a pas encore un modele produit clair pour ajouter ou gerer les repositories GitHub depuis l'interface. Le legacy ContentFlow a des ecrans projets/onboarding, mais ils melangent backend, workspace, contenu, pipeline et auth. Il faut garder l'idee GitHub/project sans importer cette architecture.

# Solution

Definir une couche ShipGlowz dediee `project = GitHub repository`: nom, `owner/repo`, URL GitHub, statut d'acces, clone gere, source de lecture Markdown, diagnostics, et statut de projection distante. Le modele laisse de la place pour GitHub OAuth, workspace multi-user et choix final du provider de base distante, sans confondre ces couches.

# Scope In

- Definir le vocabulaire ShipGlowz: project = GitHub repository, managed clone = copie de travail interne/source de lecture-indexation, remote database = projection/sync/index, workspace = futur regroupement multi-user.
- Creer un modele ShipGlowz project/repository distinct du `Project` legacy ou l'adapter sans dependances backend.
...
- `lib/data/shipglowz_sources/parsers/projects_parser.dart`
- `lib/shipglowz/providers/dashboard_provider.dart`
- `lib/shipglowz/presentation/widgets/settings_panel.dart`
- Fresh external shipglowz_data: not needed because this spec does not implement external providers.

# Invariants

- Active implementation targets `lib/shipglowz/**`, `lib/data/shipglowz_sources/**`, and reusable validation helpers.
...
- Docs: update `shipglowz_data/technical/markdown-source-of-truth.md`, `shipglowz_data/technical/code-docs-map.md`, and `README.md`.

# Documentation Coherence

- `shipglowz_data/technical/shipglowz-legacy-reuse-roadmap.md` should mark project/source onboarding as first active reuse chantier.
- `shipglowz_data/technical/markdown-source-of-truth.md` should describe source registration if implementation proceeds.
- `shipglowz_data/editorial/content-map.md` should add any new source registry or UI surface.
...
1. Store V1 project registrations in a ShipGlowz Markdown/registry file, with GitHub URL, `owner/repo`, clone path, and projection status.
2. Create or adapt a ShipGlowz-specific project/repository model under `lib/shipglowz/` or `lib/data/shipglowz_sources/`.
...
4. Add a GitHub project + local clone onboarding UI in `lib/shipglowz/presentation/`, reachable from settings or an empty dashboard state.
...
- The active ShipGlowz UI has a clear route or panel for adding a GitHub repository as a project.
...
- Existing `test/data/shipglowz_sources` tests remain passing.
...
- Recommended V1 model: `ShipGlowzProject = GitHub owner/repo + URL + display name + local clone path + access/diagnostic status + projection status`.
- Recommended V1 storage: ShipGlowz Markdown/registry file, not `shared_preferences`, because project identity should be portable and inspectable.
...
None for project identity. User decision captured: a ShipGlowz project is necessarily a GitHub repository, with a local clone and remote database projection. V1 storage should be a ShipGlowz Markdown/registry file unless implementation discovers a hard blocker.
...
| Step | Status | Notes |
| --- | --- | --- |
| sf-spec | done | Draft created in `shipglowz_data/workflow/specs/shipglowz-project-source-onboarding.md`. |
| sf-ready | ready | Project identity and V1 storage direction are decided. |
| governance corpus gate | already existed | `shipglowz_data/technical/` and `shipglowz_data/editorial/content-map.md` exist. |
