...
owner: "Diane"
confidence: medium
user_story: "En tant que fondatrice de ShipGlowz, je veux un runner/indexer qui lit un repo GitHub via un clone géré et projette les fichiers Markdown ShipGlowz dans Firestore, afin que l'app puisse afficher les projets sans exposer de secrets ni rendre Firestore canonique."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
...
depends_on:
  ...
  - artifact: "shipglowz_data/workflow/specs/firebase-firestore-projection-migration.md"
    artifact_version: "0.4.0"
    required_status: "ready"
  - artifact: "shipglowz_data/workflow/specs/shipglowz-project-source-onboarding.md"
    artifact_version: "0.4.0"
    required_status: "ready"
  ...
  - artifact: "shipglowz_data/workflow/specs/shipglowz-firestore-data-model.md"
    artifact_version: "1.0.0"
    required_status: "ready"
  - artifact: "shipglowz_data/technical/firestore-data-model.md"
    artifact_version: "0.1.0"
    required_status: "draft"
...
supersedes: []
evidence:
  - "User decision 2026-05-09: one user can have multiple projects and each ShipGlowz project is exactly one GitHub repository."
...
---
# Spec: ShipGlowz GitHub Managed Clone Indexer
🟢 [shipglowz_app] spec: ShipGlowz GitHub Managed Clone Indexer | status: ready | path: shipglowz_data/workflow/specs/shipglowz-github-managed-clone-indexer.md | next: /sf-ready ShipGlowz Dashboard Read-only Projection

# Title

ShipGlowz GitHub Managed Clone Indexer
...
- Update shipglowz_data maps so future implementation agents know this spec owns the runner/indexer slice.
...
- `shipglowz_data/workflow/specs/shipglowz-auth-github-access.md` defines access statuses, server-side revalidation, backend-only installation metadata, and GitHub-wins behavior.
- `shipglowz_data/workflow/specs/shipglowz-firestore-data-model.md` and `shipglowz_data/technical/firestore-data-model.md` define existing projection records and path builders under `lib/data/firestore_projection/`.
- Existing source reader/parser patterns live under `lib/data/shipglowz_sources/`.
- Foundational architecture contract is `shipglowz_data/technical/shipglowz-foundational-architecture.md`.
- Firebase translation contract is `shipglowz_data/workflow/specs/firebase-firestore-projection-migration.md`.
...
- Keep `shipglowz_data/technical/shipglowz-foundational-architecture.md` as the canonical decision record.
- Keep `shipglowz_data/technical/firestore-data-model.md` and `lib/data/firestore_projection/*` aligned when new index-run fields or status enums are added.
- Update `shipglowz_data/workflow/TASKS.md` and `CHANGELOG.md` after implementation changes behavior or creates new code surfaces.
- Future implementation must update README only when Firebase/GitHub runner code exists.
- Future `sf-docs` pass should refine the exact ShipGlowz Markdown artifact allowlist.
...
- Repo is empty or has no allowed ShipGlowz Markdown files.
...
- [x] Task 1: Extend the Firestore projection model for runner-owned index request metadata.
  ...
  - Action: Add or extend pure Dart records/enums for index request status, runner diagnostic codes, access status mirroring `shipglowz-auth-github-access.md`, idempotency `requestId`, and active-run metadata without importing Firebase SDKs.
  ...
  - Depends on: Existing `lib/data/firestore_projection/*` contracts from `shipglowz-firestore-data-model.md`.
...
- [x] Task 3: Create the runner contract document.
  - File: `shipglowz_data/technical/github-managed-clone-indexer.md`
  ...
  - Validate with: `rg -n "requestId|already_running|source_too_large|index_timeout|verifyGitHubAccess|indexRepository|2 MB|20 MB|token" shipglowz_data/technical/github-managed-clone-indexer.md`.
...
- [x] Task 4: Define allowed Markdown artifact indexing policy.
  ...
  - Action: Add a pure allowlist/classifier for `shipglowz_data/business/*`, `shipglowz_data/editorial/*`, `shipglowz_data/technical/*`, `shipglowz_data/workflow/TASKS.md`, `shipglowz_data/workflow/AUDIT_LOG.md`, `shipglowz_data/workflow/specs/*.md`, and root compatibility docs (`AGENT.md`, `AGENTS.md`, `CLAUDE.md`, `README.md`, `CHANGELOG.md`).
  - User story link: Ensures the indexer reads ShipGlowz governance artifacts, not arbitrary repository Markdown.
  - Depends on: Task 3.
  - Validate with: `flutter test test/data/shipglowz_sources/shipglowz_artifact_index_policy_test.dart`.
  - Notes: `shipglowz_data/` wins over root fallback docs; do not index `.git`, build output, secrets, or arbitrary docs by default.
- [x] Task 5: Define the local fake runner adapter for implementation tests.
  - File: `lib/data/shipglowz_sources/local_fake_clone_indexer.dart`
  - File: `test/data/shipglowz_sources/local_fake_clone_indexer_test.dart`
  ...
  - Validate with: `flutter test test/data/shipglowz_sources/local_fake_clone_indexer_test.dart`.
  - Notes: This is not the production runner; it is the test adapter for server-shaped payloads.
- [x] Task 6: Define runner repository interfaces for the active ShipGlowz app layer.
  - File: `lib/shipglowz/data/github_managed_clone_indexer_repository.dart`
  - File: `test/shipglowz/data/github_managed_clone_indexer_repository_test.dart`
  ...
  - Validate with: `flutter test test/shipglowz/data/github_managed_clone_indexer_repository_test.dart`.
...
- [x] Task 7: Add security and failure-mode tests.
  ...
  - Validate with: `flutter test test/data/firestore_projection test/data/shipglowz_sources test/shipglowz/data`.
...
- [x] Task 8: Draft Firestore and backend security-rule requirements before production writes.
  ...
  - Validate with: `rg -n "App Check|rate limit|server-owned|membership|redacted|Security Rules|backend-only" shipglowz_data/technical/github-managed-clone-indexer.md`.
...
- [x] Task 9: Update documentation maps and public docs only where behavior exists.
  ...
  - File: `shipglowz_data/technical/shipglowz-foundational-architecture.md`
  - File: `README.md`
  - File: `CHANGELOG.md`
  ...
  - Validate with: `rg -n "github-managed-clone-indexer|local_fake_clone_indexer|GitHub managed clone" shipglowz_data/editorial/content-map.md shipglowz_data/technical README.md CHANGELOG.md`.
  - Notes: Do not claim Firebase/GitHub production indexing exists until it does.
- [x] Task 10: Run the bounded verification gate.
  - File: `shipglowz_data/workflow/specs/shipglowz-github-managed-clone-indexer.md`
  ...
  - Validate with: `flutter analyze`, `flutter test`, `rg -n "token|installationToken|clonePath|cloneUrl|serviceCredential|x-access-token" lib test shipglowz_data/technical`.
...
- Parser tests using existing `test/data/shipglowz_sources` patterns.
...
- Files to read first: `shipglowz_data/technical/firestore-data-model.md`, `lib/data/firestore_projection/*`, `lib/data/shipglowz_sources/source_file_reader.dart`, `lib/data/shipglowz_sources/source_path_policy.dart`, `shipglowz_data/workflow/specs/shipglowz-auth-github-access.md`, and `shipglowz_data/technical/shipglowz-foundational-architecture.md`.
...
- Validation commands: `flutter test test/data/firestore_projection`, `flutter test test/data/shipglowz_sources`, `flutter test test/shipglowz/data`, `flutter analyze`, `flutter test`, and `rg -n "token|installationToken|clonePath|cloneUrl|serviceCredential|x-access-token" lib test shipglowz_data/technical`.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-05-09 17:22:02 UTC | sf-spec | GPT-5 Codex | Created runner/indexer spec from foundational architecture decisions. | Draft spec created. | /sf-ready ShipGlowz GitHub Managed Clone Indexer |
| 2026-05-14 16:15:30 UTC | sf-ready | GPT-5 Codex | Readiness gate for managed clone/indexer spec, including adversarial/security/fresh-docs review. | not ready | /sf-spec ShipGlowz GitHub Managed Clone Indexer readiness fixes |
| 2026-05-14 16:23:00 UTC | sf-spec | GPT-5 Codex | Repaired readiness gaps: dependencies, concrete implementation tasks, auth model, idempotency, concurrency, size limits, token retry, docs freshness, and validation commands. | draft updated | /sf-ready ShipGlowz GitHub Managed Clone Indexer |
| 2026-05-14 17:32:39 UTC | sf-ready | GPT-5 Codex | Re-ran readiness gate after repairs: structure, metadata, user-story traceability, task ordering, adversarial review, security review, language doctrine, and fresh Firebase/GitHub docs. | ready | /sf-start ShipGlowz GitHub Managed Clone Indexer |
| 2026-05-14 17:49:11 UTC | sf-start | GPT-5 Codex | Implemented bounded pure-Dart contracts, validators, local fake runner, repository interface, technical doc, docs maps, changelog, and tests without production Firebase/GitHub wiring. | implemented | /sf-verify ShipGlowz GitHub Managed Clone Indexer |
| 2026-05-14 18:06:57 UTC | sf-verify | GPT-5.5 xhigh | Verified implementation against ready spec; corrected wire enum serialization and managed-clone path redaction; ran targeted/full Flutter checks, secret scan, and documentation scans. | verified | /sf-end ShipGlowz GitHub Managed Clone Indexer |
| 2026-05-14 20:31:08 UTC | sf-ship | GPT-5 Codex | Quick ship after bug gate, secret scan, `flutter analyze`, full `flutter test`, and `git diff --check`; no full closeout. | shipped | /sf-prod shipglowz_app |
| 2026-05-30 16:49:28 UTC | sf-end | GPT-5 Codex | Closed the managed clone/indexer slice after confirming implementation, verification, and quick ship were already complete; moved product focus to dashboard read-only projection. | closed | /sf-ready ShipGlowz Dashboard Read-only Projection |

# Current Chantier Flow

| Step | Status | Notes |
|------|--------|-------|
| sf-spec | done | Spec created from user architecture decisions and official Firebase/GitHub shipglowz_data. |
