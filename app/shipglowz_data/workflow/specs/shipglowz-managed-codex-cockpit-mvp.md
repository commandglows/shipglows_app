...
linked_systems:
  ...
  - "ShipGlowz AgentRuntime contract"
...
depends_on:
  ...
  - artifact: "shipglowz_data/workflow/specs/shipglowz-foundational-coherence-review.md"
  ...
  - artifact: "https://docs.github.com/en/enterprise-cloud@latest/apps/creating-github-apps/authenticating-with-a-github-app/authenticating-as-a-github-app-installation"
    artifact_version: "checked-2026-07-18"
    required_status: "active"
  - artifact: "shipglowz_data/workflow/research/flutter-codex-cockpit-open-source-architecture.md"
...
evidence:
  - "Operator validation 2026-07-18: ShipGlowz provides and manages the server; the end user should not administer SSH, tmux, PTY, terminal, or server operations."
  ...
  - "Repository inspection 2026-07-18: the active ShipGlowz runtime already has Riverpod, go_router, project health parsing, local source redaction, projection contracts, and responsive Flutter widgets."
  - "Repository inspection 2026-07-18: legacy auth and GitHub service code currently belong to the legacy runtime and are not yet wired into the active ShipGlowz runtime."
  ...
  - "Repository inspection 2026-08-01: the Flutter prototype already exists under `app/lib/shipglowz/`, including overview, project detail, settings, health models, repositories, providers, and tests; this spec extends that prototype rather than creating a new app."
  ...
  - "CTO architecture reframe 2026-08-01: ShipGlowz owns the multi-agent control plane; Codex app-server is the first runtime adapter, while OpenCode, Kilo and ACP must remain possible behind the same normalized contract."
next_step: "/102-sg-start ShipGlowz Managed Agent Cockpit MVP"
---

# Spec: ShipGlowz Managed Agent Cockpit MVP

🟢 [shipglowz_app] spec: ShipGlowz Managed Agent Cockpit MVP | status: ready | path: shipglowz_data/workflow/specs/shipglowz-managed-codex-cockpit-mvp.md | next: /102-sg-start ShipGlowz Managed Agent Cockpit MVP

# Title

ShipGlowz Managed Agent Cockpit MVP

# Status

Ready after the multi-agent readiness review on 2026-08-01. The existing Flutter prototype is the implementation base. The product has three deliberately separated surfaces: the health Cockpit, semantic agent work for normal use, and a separately authorized operator Workspace for a real PTY/tmux/Neovim session. ShipGlowz owns a runtime-neutral control plane: Codex app-server is the first complete adapter, while OpenCode, Kilo and ACP remain possible through the same `AgentRuntime` contract. `just-bash` remains only an optional sandbox for bounded ShipGlowz skill checks; it is not the real terminal. Supabase Auth is the recommended cross-platform identity baseline behind a portable provider boundary.

# User Story

En tant qu'utilisatrice de ShipGlowz, je veux voir la santé de tous mes projets, piloter des conversations avec l'agent de code adapté à chaque tâche et, lorsque je l'autorise explicitement, ouvrir un espace opérateur terminal dans la même application, afin de travailler depuis mon navigateur sans administrer l'infrastructure.

# Minimal Behavior Contract

After signing in, the user sees one Cockpit summarizing every authorized GitHub repository across tech, content, SEO, performance, and security. Opening a project reveals agent conversations as child tabs; semantic messages, tools, plans, approvals and results remain the default interaction. A separately protected Workspace tab may open a real server-side PTY attached only to an allowlisted tmux session, allowing terminal use and Neovim without exposing credentials, clone paths or raw control protocols. Audit and fix actions use the ShipGlowz-managed control plane, which verifies access, selects an authorized runtime and execution environment, prepares isolated repository workspaces, runs ShipGlowz skills and records durable evidence. A failed, interrupted, stale, unauthorized or disconnected action remains visible with a recoverable state. The primary edge case is a selected runtime that cannot satisfy the requested capabilities: ShipGlowz must reject it before work begins, explain the missing capability, and never silently broaden permissions or fall back to another runtime.
...
3. `AgentRuntime` is a ShipGlowz-owned port for runtime capability discovery, sessions/threads, turns, interruption, approvals, normalized events and redacted diagnostics. `codex app-server` over local stdio is the first complete adapter; OpenCode, Kilo and ACP are staged adapters, never direct public endpoints.
...
- `AgentRuntime`, `ExecutionProvider`, `CapabilityBroker` and `ModelGateway` are ShipGlowz-owned ports. They preserve the choice of Codex, OpenCode, Kilo, ACP, self-hosted sandbox, Vercel sandbox, direct model provider or future runtime without changing Flutter, health or authorization contracts.
...
- ShipGlowz `AgentRuntime` capability contract, Codex app-server first adapter and normalized event mapping.
...
- Read-only ingestion of existing ShipGlowz Markdown and GitHub evidence into Cockpit health.
...
- Bounded `just-bash` integration proof for at least one read-only ShipGlowz skill against a controlled repository snapshot.
...
- Asking end users to install, configure, update, or monitor any agent runtime, tmux, Git, GitHub credentials, or the ShipGlowz server.
...
- Every request is authorized server-side from the selected AuthProvider identity, ShipGlowz project membership, and fresh GitHub App access when repository-sensitive.
...
- UI work must use the declared Flutter design-system authority. The active ShipGlowz private theme must be reconciled with `app/lib/presentation/theme/app_theme.dart` before new visual tokens are introduced.
...
- Fresh docs checked 2026-08-01: Vercel AI SDK's HarnessAgent can normalize several agent harnesses but is experimental; OpenCode ACP is editor-to-subprocess JSON-RPC over stdio, while Kilo documents local HTTP/SSE runtime surfaces. ShipGlowz therefore keeps its own remote contract and capability matrix.
...
- Existing code foundations include `lib/shipglowz/**`, `lib/data/shipglowz_sources/**`, `lib/domain/project_health/**`, Firestore-shaped projection DTOs/validators, and legacy Clerk/GitHub service concepts.
- Canonical design-system authority is `shipglowz_data/technical/design-system-authority.md` with Flutter carrier `lib/presentation/theme/app_theme.dart`.

# Invariants

- One ShipGlowz project equals exactly one GitHub repository.
- One project may have many conversations; one conversation belongs to exactly one project and one tenant.
- One ShipGlowz conversation maps to one selected-runtime thread/session identifier stored server-side; raw runtime identifiers are never authorization proof.
...
- `POST /v1/projects/:projectId/conversations`: create a ShipGlowz conversation and selected-runtime thread/session after capability validation.
...
- Active runtime: `lib/shipglowz/**` becomes server-backed and no longer depends on browser-inaccessible local filesystem reads for the primary Cockpit.
...
- Operations: ShipGlowz owns runtime installation/authentication/upgrade policy, process health, workspace cleanup, quotas, backups of operational projection, capability reporting and incident diagnostics. Codex is the first operated runtime, not the only supported one.
...
- Update `shipglowz_data/technical/runtime-boundary.md` to name the managed runner and remove claims that the active Web runtime is only a local-reader dashboard.
- Update `shipglowz_data/technical/shipglowz-foundational-architecture.md` with a dated supersession note for the old read-only/agent-out decision while retaining canonical-data and GitHub invariants.
- Update `shipglowz_data/technical/code-docs-map.md` with runner, API/event, Cockpit, conversation, and health-projection ownership.
- Add `shipglowz_data/technical/operator-guides/managed-agent-workspace.md` covering runtime capability selection, PTY/tmux capability, Neovim session policy, reconnect/cleanup, and incident recovery.
- Add runner-owned `AgentRuntime`, `ExecutionProvider`, `CapabilityBroker` and health-evaluator contracts. Document that portable ShipGlowz skills produce versioned evidence and dimension outcomes; do not delegate this authority to a runtime, just-bash or a generic dashboard.
- Update `shipglowz_data/technical/design-system-authority.md` if the active ShipGlowz theme carrier is consolidated.
...
- Update `shipglowz_data/workflow/TASKS.md`, verification artifact, and `CHANGELOG.md` during implementation closure without rewriting unrelated operator edits.
...
- [ ] Task 1: Establish the ShipGlowz control-plane contracts, runner package, and secure configuration.
...
- [~] Task 2: Implement portable Supabase Auth authentication and tenant/project authorization middleware.
  - Files: `runner/src/auth/**`, `runner/src/projects/projectAccess.ts`, `runner/test/auth/**`, `app/lib/shipglowz/auth/**`, `app/test/shipglowz/auth/**`.
...
- [~] Task 3: Implement GitHub App access and managed workspace lifecycle.
  ...
  - User story link: Lets ShipGlowz operate repositories while hiding server and Git mechanics.
...
- [ ] Task 7: Establish the proprietary ShipGlowz health evaluator and five-dimensional Cockpit projection.
  - Files: `runner/src/skills/**`, `runner/src/health/**`, `runner/test/skills/**`, `runner/test/health/**`, `app/lib/domain/project_health/**`, `app/lib/shipglowz/data/cockpit/**`, `app/test/domain/project_health/**`, `app/test/shipglowz/data/cockpit/**`.
...
- [ ] Task 8: Integrate the active-runtime identity and runner API adapters.
  - Files: `app/lib/shipglowz/auth/**`, `app/lib/shipglowz/data/api/**`, `app/lib/shipglowz/providers/**`, `app/web_auth/**`, `app/android/**`, `app/windows/**`, `app/test/shipglowz/auth/**`, `app/test/shipglowz/data/api/**`.
...
- [ ] Task 9: Reconcile the Flutter design authority and implement the Cockpit shell.
  - Files: `app/lib/presentation/theme/app_theme.dart`, `app/lib/shipglowz/app.dart`, `app/lib/shipglowz/router.dart`, `app/lib/shipglowz/presentation/screens/cockpit_screen.dart`, `app/lib/shipglowz/presentation/widgets/cockpit/**`, corresponding widget/golden tests.
  - Action: Make the active ShipGlowz theme consume canonical semantic tokens; implement responsive Cockpit navigation, health matrix, project parent tabs, loading/empty/stale/access-lost/error states, keyboard/focus behavior, and accessible labels.
...
- [ ] Task 10: Implement agent conversation tabs and action controls.
  - Files: `app/lib/shipglowz/presentation/screens/project_workspace_screen.dart`, `app/lib/shipglowz/presentation/widgets/conversations/**`, `app/lib/shipglowz/providers/conversations/**`, corresponding tests.
...
- [ ] Task 11: Implement the separately authorized operator Workspace.
  - Files: `runner/src/operator-workspace/**`, `runner/src/pty/**`, `runner/test/operator-workspace/**`, `runner/test/pty/**`, `app/lib/shipglowz/presentation/screens/operator_workspace_screen.dart`, `app/lib/shipglowz/presentation/widgets/operator_workspace/**`, `app/lib/shipglowz/providers/operator_workspace/**`, `app/test/shipglowz/operator_workspace/**`, platform runner files for Web/Android/Windows.
...
- [ ] Task 12: Add observability, operational controls, and secure runbook.
  - Files: `runner/src/observability/**`, `app/lib/shipglowz/observability/**`, `app/shipglowz_data/technical/operator-guides/managed-agent-runner.md`, related tests.
  ...
  - User story link: Enables ShipGlowz to own the server so the user does not have to.
...
- [ ] Task 13: Complete end-to-end proof and documentation coherence.
  - Files: `app/shipglowz_data/workflow/verification/shipglowz-managed-agent-cockpit-mvp.md`, impacted technical maps/docs, `app/README.md`, `app/CHANGELOG.md`, and `app/.github/workflows/**`.
...
- `AC-004b`: A request whose selected runtime lacks a required capability is rejected before execution with a stable, user-visible capability error; ShipGlowz never silently changes runtime or expands permissions.
...
- `AC-017`: At least one proprietary ShipGlowz skill can run through bounded `just-bash` against a controlled snapshot, produce versioned evidence, and update health without accessing secrets or unrestricted network/filesystem state.
...
- `checklist_path`: `shipglowz_data/workflow/verification/shipglowz-managed-agent-cockpit-mvp.md`.
...
- Keep every runtime transport behind `AgentRuntime` and keep Flutter behind the normalized ShipGlowz event contract. No Flutter model may import a runtime wire type.
...
| --- | --- | --- | --- | --- | --- |
| 2026-07-18 08:20:45 UTC | 000-shipglowz | GPT-5 Codex | Routed the validated non-trivial product vision to the build lifecycle. | routed | 001-sg-build |
...
| 2026-07-18 08:20:45 UTC | 100-sg-spec | GPT-5 Codex | Converted operator decisions, current code evidence, preferred stacks, and fresh official integration docs into an executable high-risk MVP contract. | drafted | /101-sg-ready ShipGlowz Managed Codex Cockpit MVP |
| 2026-07-18 08:30:36 UTC | 101-sg-ready | GPT-5 Codex | Ran structure, user-story, stack, freshness, adversarial, security, linked-system, task-order, and proof-contract gates; fixed Android auth, Codex transport, mutation concurrency, and foundational supersession ambiguity. | ready | /102-sg-start ShipGlowz Managed Codex Cockpit MVP |
...
| 2026-08-01 22:36:56 UTC | 100-sg-spec | GPT-5 Codex | Reframed the MVP as a ShipGlowz multi-agent control plane: introduced `AgentRuntime`, `ExecutionProvider`, `CapabilityBroker`, runtime capability checks and durable run semantics; retained Codex app-server as the first full adapter and required a second-adapter conformance fixture. | amended; readiness review required | Rerun /101-sg-ready ShipGlowz Managed Agent Cockpit MVP |
