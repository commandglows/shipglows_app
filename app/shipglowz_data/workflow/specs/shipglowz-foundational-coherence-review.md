...
owner: "Diane"
confidence: medium
user_story: "En tant que fondatrice de ShipGlowz, je veux une spec unique de coherence qui verrouille les contrats transverses entre auth, onboarding, indexer, gouvernance Markdown, projection Firestore et dashboard, afin d'eviter des implementations divergentes ou insecurisees avant /sf-start."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
...
depends_on:
  ...
  - artifact: "shipglowz_data/technical/foundational-specs-handoff.md"
    artifact_version: "0.1.0"
    required_status: "draft"
  ...
  - artifact: "shipglowz_data/workflow/specs/shipglowz-github-managed-clone-indexer.md"
    artifact_version: "0.1.0"
    required_status: "draft"
  - artifact: "shipglowz_data/workflow/specs/shipglowz-project-onboarding-flow.md"
    artifact_version: "0.1.0"
    required_status: "draft"
  - artifact: "shipglowz_data/workflow/specs/shipglowz-markdown-artifact-governance.md"
    artifact_version: "0.1.0"
    required_status: "draft"
  - artifact: "shipglowz_data/workflow/specs/shipglowz-dashboard-readonly-projection.md"
    artifact_version: "0.1.0"
    required_status: "draft"
  - artifact: "https://firebase.google.com/shipglowz_data/auth"
    artifact_version: "checked-2026-05-10"
    required_status: "active"
  - artifact: "https://firebase.google.com/shipglowz_data/firestore/security/rules-conditions"
    artifact_version: "checked-2026-05-10"
    required_status: "active"
  - artifact: "https://firebase.google.com/shipglowz_data/functions"
    artifact_version: "checked-2026-05-10"
    required_status: "active"
  - artifact: "https://shipglowz_data.github.com/en/apps/overview"
    artifact_version: "checked-2026-05-10"
    required_status: "active"
  - artifact: "https://shipglowz_data.github.com/en/apps/creating-github-apps/authenticating-with-a-github-app/authenticating-as-a-github-app-installation"
    artifact_version: "checked-2026-05-10"
    required_status: "active"
supersedes: []
evidence:
  - "shipglowz_data/technical/foundational-specs-handoff.md references /sf-ready ShipGlowz Foundational Coherence Review as required gate before implementation."
...
next_step: "/sf-verify ShipGlowz Foundational Coherence Review"
---
# Spec: ShipGlowz Foundational Coherence Review
🟢 [shipglowz_app] spec: ShipGlowz Foundational Coherence Review | status: ready | path: shipglowz_data/workflow/specs/shipglowz-foundational-coherence-review.md | next: /sf-verify ShipGlowz Foundational Coherence Review

# Title

ShipGlowz Foundational Coherence Review
...
En tant que fondatrice de ShipGlowz, je veux une spec unique de coherence qui verrouille les contrats transverses entre auth, onboarding, indexer, gouvernance Markdown, projection Firestore et dashboard, afin d'eviter des implementations divergentes ou insecurisees avant /sf-start.
...
- Trigger: operator runs `/sf-ready ShipGlowz Foundational Coherence Review` against this canonical spec.
...
- Proof of success: `sf-ready` checklist can be answered without ambiguity for all required gates (behavior, ordering, security, shipglowz_data coherence, language doctrine, freshness gate) and outputs a single next command.
...
The repository has a complete foundational corpus but the contracts are spread across multiple draft shipglowz_data/workflow/specs and a handoff doc. A fresh implementation context can still choose inconsistent status names, permission checks, query boundaries, and error semantics. Without a single coherence spec, `/sf-ready` can pass individual shipglowz_data while cross-spec contradictions remain.
...
- Define cross-spec contract matrix for:
  ...
  - `shipglowz_data/` governance precedence
  - dashboard query/read boundaries
- Define required state vocabulary and mapping rules between shipglowz_data/workflow/specs.
...
- Must remain compatible with `shipglowz_data/technical/shipglowz-foundational-architecture.md` decisions.
...
- Must keep spec internal-contract anchors in English section headings for ShipGlowz tooling compatibility.
...
- Fresh external shipglowz_data verdict: `fresh-shipglowz_data checked` via official Firebase and GitHub shipglowz_data already referenced by foundational specs; coherence spec must fail readiness if those dependencies become stale or contradictory.

# Invariants

- One ShipGlowz project maps to exactly one GitHub repository.
...
- Upstream dependencies:
  - Auth/session semantics from Firebase Auth and GitHub App shipglowz_data.
...
- `shipglowz_data/` appears after an initial fallback-root indexing cycle.
...
| --- | --- | --- | --- | --- |
| Project identity | One ShipGlowz project equals exactly one GitHub repository, keyed by opaque `projectId`; GitHub owner/name/repository ID are metadata for deduplication and access checks. | `shipglowz_data/workflow/specs/shipglowz-firestore-data-model.md` | Auth, onboarding, indexer, dashboard | Any spec uses `owner/repo` as durable Firestore document ID or creates duplicate projects for the same repo. |
| Identity versus repository access | Firebase Auth identifies the ShipGlowz user; GitHub App installation/access proves repository readability. These are separate and both are required for sensitive project actions. | `shipglowz_data/workflow/specs/shipglowz-auth-github-access.md` | Onboarding, Firestore rules, dashboard | Any spec treats Firebase login alone as proof of repo access or makes GitHub login mandatory for Firebase identity. |
| Server-side authorization | Repo listing, create-or-join, refresh, clone, indexing, projection writes, and access resync are initiated through trusted backend checks. Client UI can request, not authorize. | `shipglowz_data/workflow/specs/shipglowz-auth-github-access.md` | Indexer, onboarding, Firestore model | Any sensitive operation can be completed with UI-only checks or client-supplied repo metadata. |
| Read-only V1 boundary | V1 may read repository content and write app/projection state; it must not write Markdown, commit, push, open PRs, run terminal sessions, or execute agents. | `shipglowz_data/technical/shipglowz-foundational-architecture.md` | Indexer, dashboard, onboarding | Any foundational spec adds repo write-back, terminal, or agent execution behavior. |
| Projection authority | Firestore stores app state and projection, never canonical repository content. GitHub/clone content wins conflicts. | `shipglowz_data/workflow/specs/shipglowz-firestore-data-model.md` | Indexer, dashboard, Markdown governance | Any spec lets Firestore override repo Markdown without a future write-back spec. |
| Managed clone | Clone materialization is mandatory for reliable indexing, infrastructure-owned, and hidden from user configuration. | `shipglowz_data/workflow/specs/shipglowz-github-managed-clone-indexer.md` | Onboarding, dashboard diagnostics | Any spec asks the user to choose clone paths or exposes clone paths as user-managed settings. |
| Governance corpus | `shipglowz_data/` is the preferred project-local governance corpus; root-level docs are fallback or legacy evidence during migration and cannot override `shipglowz_data/`. | `shipglowz_data/workflow/specs/shipglowz-markdown-artifact-governance.md` | Indexer, dashboard projection | Any spec indexes arbitrary repository Markdown or lets stale root docs override `shipglowz_data/`. |
| Dashboard read model | Dashboard reads user-scoped Firestore projection, project refs, diagnostics, and stale/access warnings without triggering repo writes. | `shipglowz_data/workflow/specs/shipglowz-dashboard-readonly-projection.md` | Firestore model, auth/access, indexer | Any dashboard query can scan globally, cross tenants, mutate canonical content, or hide access-loss warnings. |
| Access loss | Existing projection can remain visible/searchable with clear warning; refresh, clone, indexing, and access-sensitive writes are blocked until GitHub access is restored. | `shipglowz_data/workflow/specs/shipglowz-auth-github-access.md` | Dashboard, indexer, Firestore model | Any spec deletes shared project data immediately or keeps refreshing after access is lost. |
| Idempotent create-or-join | Concurrent or repeated onboarding for the same GitHub repo resolves to one shared project with safe membership updates. | `shipglowz_data/workflow/specs/shipglowz-project-onboarding-flow.md` | Firestore model, auth/access | Duplicate requests can create duplicate projects, widen permissions, or overwrite membership incorrectly. |

# Canonical State Vocabulary

| State | Meaning | Allowed producer | Allowed consumer | Forbidden interpretation |
| --- | --- | --- | --- | --- |
| `needs_github_connection` | User is authenticated in ShipGlowz but has no usable GitHub App installation/access path for project onboarding. | Auth/access backend or onboarding flow | Onboarding UI | A project exists or indexing can start. |
| `select_repository` | User can choose from repos the GitHub App installation allows and ShipGlowz has server-validated. | Trusted backend repo-list endpoint | Onboarding UI | Client-provided repo list is authoritative. |
...
| `archived` | Shared project is no longer active for normal indexing/visibility, usually after orphaning or explicit later cleanup policy. | Trusted backend/admin policy | Dashboard filters, cleanup shipglowz_data | Immediate destructive repository or projection deletion. |
...
| `hidden_for_user` | User removed the project from personal ShipGlowz view without deleting shared project data. | User-scoped preference action | Dashboard filters | Project deletion or GitHub access revocation. |

# Mandatory Security Controls

| Area | Required control | Owner spec | Validation expectation |
| --- | --- | --- | --- |
| Authentication | Every client request that touches user/project state must bind to Firebase Auth identity. | `shipglowz_data/workflow/specs/shipglowz-auth-github-access.md` | Rules/functions examples reject unauthenticated requests. |
| Authorization | Repo-sensitive actions revalidate GitHub App installation access server-side at action time. | `shipglowz_data/workflow/specs/shipglowz-auth-github-access.md` | No create, refresh, clone, or index action relies only on cached client claims. |
| Tenant boundary | Client reads are scoped through user membership/project refs; queries must be compatible with Firestore rules because rules are not filters. | `shipglowz_data/workflow/specs/shipglowz-firestore-data-model.md` | No dashboard/global collection query can depend on rules to trim inaccessible rows. |
| Backend trust | Cloud Functions or equivalent trusted runner owns token exchange, repo listing, clone/index, and projection writes. | `shipglowz_data/workflow/specs/shipglowz-github-managed-clone-indexer.md` | Flutter/client never receives GitHub App installation tokens or clone credentials. |
| Token handling | GitHub installation tokens are short-lived, generated server-side, never stored in client-readable Firestore, and never logged. | `shipglowz_data/workflow/specs/shipglowz-auth-github-access.md` | Token storage/logging is explicitly prohibited. |
...
- [x] Task 1: Create coherence contract matrix section in this spec.
  - Fichier : `shipglowz_data/workflow/specs/shipglowz-foundational-coherence-review.md`
  ...
  - Depends on : None.
  - Validate with : `rg -n "contract matrix|state mapping|authority" shipglowz_data/workflow/specs/shipglowz-foundational-coherence-review.md`.
...
- [x] Task 2: Normalize cross-spec state vocabulary.
  ...
  - Depends on : Task 1.
  - Validate with : `rg -n "access_lost|stale|indexing|archived|state vocabulary" shipglowz_data/workflow/specs/shipglowz-foundational-coherence-review.md`.
...
- [x] Task 3: Define mandatory security control checklist.
  ...
  - Depends on : Task 1.
  - Validate with : `rg -n "Authentication|Authorization|Multi-tenant|rate|replay|idempot" shipglowz_data/workflow/specs/shipglowz-foundational-coherence-review.md`.
...
- [x] Task 4: Define coherence acceptance criteria and failure gates.
  ...
  - Validate with : `rg -n "CA [0-9]+" shipglowz_data/workflow/specs/shipglowz-foundational-coherence-review.md`.
  - Notes : Include at least one misuse/bypass criterion.

- [x] Task 5: Update foundational reference shipglowz_data to point to this canonical gate.
  - Fichier : `shipglowz_data/technical/foundational-specs-handoff.md`, `shipglowz_data/technical/shipglowz-foundational-architecture.md`, `shipglowz_data/technical/code-docs-map.md`
  ...
  - Depends on : Task 4.
  - Validate with : `rg -n "shipglowz-foundational-coherence-review.md|Foundational Coherence" shipglowz_data/technical/foundational-specs-handoff.md shipglowz_data/technical/shipglowz-foundational-architecture.md shipglowz_data/technical/code-docs-map.md`.
  - Notes : Documentation-only, no runtime changes.

# Acceptance Criteria

- [ ] CA 1 : Given all six foundational specs exist, when `/sf-ready ShipGlowz Foundational Coherence Review` runs on this spec, then it can evaluate each required gate using explicit criteria from this document without inferencing from chat history.
...
- [ ] CA 7 : Given documentation pointers are reviewed, when starting from architecture or handoff shipglowz_data, then this canonical coherence spec is discoverable as the mandatory readiness gate.

# Test Strategy

- Spec consistency checks:
  - `rg -n "# (Title|Status|User Story|Minimal Behavior Contract|Success Behavior|Error Behavior|Problem|Solution|Scope In|Scope Out|Constraints|Dependencies|Invariants|Links & Consequences|Documentation Coherence|Edge Cases|Implementation Tasks|Acceptance Criteria|Test Strategy|Risks|Execution Notes|Open Questions|Skill Run History|Current Chantier Flow)" shipglowz_data/workflow/specs/shipglowz-foundational-coherence-review.md`
- Cross-reference checks:
  - `rg -n "ShipGlowz Foundational Coherence Review|shipglowz-foundational-coherence-review.md" shipglowz_data/technical shipglowz_data/workflow/specs`
- Invariant drift checks across foundational specs:
  - `rg -n "projectId|GitHub wins|read-only|access-lost|stale|shipglowz_data" shipglowz_data/workflow/specs/shipglowz-*.md shipglowz_data/technical/shipglowz-foundational-architecture.md`
...
- High: tenant boundary leak if project membership and query filters diverge across shipglowz_data/workflow/specs.
- Medium: operational confusion if state vocabulary stays inconsistent.
- Medium: shipglowz_data drift if architecture/handoff/map references are not updated with this canonical gate.
...
- Explicit constraints:
  ...
  - Do not mark foundational implementation shipglowz_data/workflow/specs `ready` by convenience; enforce evidence.
  - Keep machine-stable headings/anchors in English.
- Validation commands:
  - `rg -n "coherence|state|invariant|access_lost|stale" shipglowz_data/workflow/specs/shipglowz-foundational-coherence-review.md shipglowz_data/workflow/specs/shipglowz-*.md shipglowz_data/technical/*.md`
...
|----------|-------|-------|--------|--------|-----------|
| 2026-05-10 16:03:25 UTC | sf-spec | GPT-5 Codex | Created canonical foundational coherence spec from existing foundational architecture and handoff intent. | draft saved | /sf-ready ShipGlowz Foundational Coherence Review |
| 2026-05-10 16:17:31 UTC | sf-build | GPT-5 Codex | Added coherence matrix, state vocabulary, security controls, failure gates, shipglowz_data links, and validation checks. | implemented | /sf-end ShipGlowz Foundational Coherence Review |
| 2026-05-10 16:31:18 UTC | sf-ship | GPT-5 Codex | Quick ship for foundational coherence documentation scope. | shipped | /sf-end ShipGlowz Foundational Coherence Review |
...
| Step | Status | Notes |
|------|--------|-------|
| sf-spec | done | Canonical coherence spec created in `shipglowz_data/workflow/specs/shipglowz-foundational-coherence-review.md`. |
| sf-ready | satisfied in sf-build | Structure, metadata, security, shipglowz_data coherence, and freshness evidence checked after corrections. |
| sf-start | implemented | Coherence contract and shipglowz_data navigation updates applied; no runtime code changed. |
| sf-verify | passed | Required sections, coherence anchors, shipglowz_data cross-links, and implementation task completion validated with scoped checks. |
