...
owner: "Diane"
confidence: medium
user_story: "En tant qu'utilisatrice ShipGlowz, je veux connecter GitHub, choisir un repository et voir son statut d'indexation, afin de transformer un repo GitHub en projet ShipGlowz lisible sans comprendre l'infrastructure interne."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
...
depends_on:
  ...
  - artifact: "https://firebase.google.com/docs/functions/callable"
    ...
    required_status: "active"
supersedes:
  - "shipglowz_data/workflow/specs/shipglowz-project-source-onboarding.md as cloud/Firebase onboarding flow target"
evidence:
  - "User decision 2026-05-09: a ShipGlowz project is exactly one GitHub repository."
  ...
  - "User decision 2026-05-10: GitHub App installation can cover all repos or selected repos, but ShipGlowz project creation requires explicit repo selection in the app."
...
next_step: "/sf-ready ShipGlowz Markdown Artifact Governance"
---
# Spec: ShipGlowz Project Onboarding Flow
🟢 [shipglowz_app] spec: ShipGlowz Project Onboarding Flow | status: ready | path: shipglowz_data/workflow/specs/shipglowz-project-onboarding-flow.md | next: /sf-ready ShipGlowz Markdown Artifact Governance

# Title

ShipGlowz Project Onboarding Flow
...
En tant qu'utilisatrice ShipGlowz, je veux connecter GitHub, choisir un repository et voir son statut d'indexation, afin de transformer un repo GitHub en projet ShipGlowz lisible sans comprendre l'infrastructure interne.

# Minimal Behavior Contract

The onboarding flow accepts a signed-in ShipGlowz user, guides them to connect or verify GitHub App access, lists only repositories GitHub confirms they can read, lets them explicitly choose one repository, creates or joins the shared ShipGlowz project for that repo, starts a server-side read-only clone/index request, and shows indexing status until the project is ready or blocked. If GitHub App access, repo permission, clone, or indexing fails, the UI shows a clear recoverable state without exposing tokens, clone paths, or backend implementation details. The easy edge case to miss is auto-creating projects just because an organization installed the GitHub App; ShipGlowz still waits for explicit user repo selection.

# Success Behavior

- Given the user is not signed in, onboarding starts with ShipGlowz sign-in through Firebase Auth.
- Given the user is signed in but GitHub is not connected, onboarding shows a GitHub connection step.
- Given GitHub App is not installed, onboarding sends the user to the GitHub App installation flow and returns to ShipGlowz with a pending verification state.
- Given GitHub App is installed on all repos or selected repos, ShipGlowz lists only repos the user can read.
- Given the user selects a repo, ShipGlowz creates or joins the shared `projects/{projectId}` for that GitHub repo.
- Given project creation succeeds, ShipGlowz starts index/clone through the trusted backend and shows `queued`, `indexing`, then `ready`.
- Given indexing finds ShipGlowz Markdown artifacts, the dashboard opens the project from Firestore projection.
...
The foundational backend and data decisions are mostly clear, but the first-run user path is not. Without a flow spec, implementation could expose technical infrastructure, create projects too early, confuse GitHub installation with ShipGlowz project selection, or fail to resume long-running indexing work.
...
- Define links to Firestore, auth/access, and runner/indexer shipglowz_data/workflow/specs.
...
- User must explicitly select a repository before ShipGlowz creates/joins a project.
...
- `shipglowz_data/workflow/specs/shipglowz-auth-github-access.md` defines Firebase Auth/GitHub App separation and access-loss behavior.
- `shipglowz_data/workflow/specs/shipglowz-firestore-data-model.md` defines project, membership, projectRefs, feedItems, indexRuns, diagnostics, and projection records.
- `shipglowz_data/workflow/specs/shipglowz-github-managed-clone-indexer.md` defines backend clone/index behavior.
- `shipglowz_data/technical/shipglowz-foundational-architecture.md` defines source-of-truth and V1 read-only invariants.
- Official GitHub App install shipglowz_data checked 2026-05-10 for third-party app installation flow.
- Official Firebase Auth and callable functions shipglowz_data checked 2026-05-10 for signed-in user and backend request assumptions.
...
- User preferences after onboarding live in user-scoped shipglowz_data.
...
- Docs: old local-clone onboarding assumptions in `shipglowz-project-source-onboarding.md` are superseded for cloud/Firebase target flow.
...
- GitHub App installed on an org before the user connects ShipGlowz.
...
- Indexing succeeds but no ShipGlowz Markdown artifacts are found.
...
- [ ] Task 1: Create the onboarding technical flow document.
  - File: `shipglowz_data/technical/project-onboarding-flow.md`
  ...
  - Validate with: `rg -n "signed_out|needs_github_connection|select_repository|indexing_project|ready|blocked" shipglowz_data/technical/project-onboarding-flow.md`.
- [ ] Task 2: Define onboarding state DTOs and pure contracts.
  - File: `lib/shipglowz/` or nearest existing app-layer ShipGlowz data directory chosen during implementation.
...
- [ ] Task 3: Define fake backend/repository contracts consumed by onboarding.
  - File: `test/shipglowz/` or nearest existing ShipGlowz test directory.
  ...
  - Validate with: `flutter test test/shipglowz`.
- [ ] Task 4: Implement user-visible onboarding screens or panels.
  - File: `lib/shipglowz/presentation/**` or the existing ShipGlowz route/presentation layer.
...
- [ ] Task 5: Update docs maps and changelog after implementation.
  ...
  - Validate with: `rg -n "project-onboarding-flow|GitHub App|select_repository|indexing_project" shipglowz_data/technical shipglowz_data/editorial CHANGELOG.md`.
...
- A repo maps to one shared ShipGlowz project.
...
- Empty repo/no known ShipGlowz artifacts is a valid state.
- Access-loss behavior matches `shipglowz-auth-github-access.md`.
...
- `checklist_path`: `shipglowz_data/workflow/verification/shipglowz-project-onboarding-flow.md`.
...
- Product decision: onboarding should feel like "Connect GitHub -> Choose repository -> ShipGlowz prepares it", not "configure backend/clone/database".
...
|----------|-------|-------|--------|--------|-----------|
| 2026-05-10 09:13:19 UTC | sf-spec | GPT-5 Codex | Created foundational project onboarding flow spec. | Draft spec created. | /sf-ready ShipGlowz Project Onboarding Flow after foundational coherence pass |
| 2026-05-30 16:54:01 UTC | sf-spec | GPT-5 Codex | Repaired readiness gaps: dependency versions, fresh official docs, structured tasks, Test Contract, proof order, scenarios, and exception policy. | reviewed | /sf-ready ShipGlowz Project Onboarding Flow |
| 2026-05-30 16:55:06 UTC | sf-ready | GPT-5 Codex | Readiness review passed: explicit repo selection, create-or-join, resume behavior, failure states, security constraints, fresh-docs gate, and Test Contract are actionable. | ready | /sf-ready ShipGlowz Markdown Artifact Governance |

# Current Chantier Flow

| Step | Status | Notes |
|------|--------|-------|
| sf-spec | done | Onboarding flow spec created from foundational auth, Firestore, and runner shipglowz_data/workflow/specs. |
