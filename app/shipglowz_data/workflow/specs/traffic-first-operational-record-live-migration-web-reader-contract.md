...
owner: "Diane"
confidence: medium
user_story: "En tant que fondatrice de ShipGlowz, je veux que la migration live des tâches, audits et specs vers le format traffic-first soit validée par le contrat de lecture de la future app web, afin que les fichiers Markdown restent la source lisible et exploitable sans normalisation cachée."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
  ...
  - "test/data/shipglowz_sources/parsers/"
  - "/home/claude/shipglowz/tui/src/sources/"
  - "future ShipGlowz web reader"
...
depends_on:
  ...
  - artifact: "/home/claude/shipglowz/skills/references/operational-record-format.md"
    artifact_version: "1.0.0"
...
evidence:
  ...
  - "/home/claude/shipglowz/tui/src/sources/readers.ts already parses traffic-first records for the terminal dashboard."
...
next_step: "/sf-prod shipglowz_app"
---

# Spec: Traffic-First Operational Record Live Migration And Web Reader Contract


🟢 [shipglowz_app] spec: Traffic-First Operational Record Live Migration And Web Reader Contract | status: ready | path: shipglowz_data/workflow/specs/traffic-first-operational-record-live-migration-web-reader-contract.md | next: /sf-prod shipglowz_app
## Title
...
- canonical source format: existing dry-run counts remain clean and generated records follow `/home/claude/shipglowz/skills/references/operational-record-format.md`;
...
- `/home/claude/shipglowz/skills/references/operational-record-format.md` is the shared writer-skill contract.
...
- `shipglowz-dashboard-readonly-projection.md` defines the dashboard as read-only projection consumer.
- `shipglowz-firestore-data-model.md` defines projection vocabulary for source commit, indexed files, diagnostics, and user-scoped dashboard feeds.
...
- TUI readers: `/home/claude/shipglowz/tui/src/sources/readers.ts` must remain canonical-first for the terminal dashboard.
...
- Create `shipglowz_data/technical/operational-record-web-reader-contract.md`.
- Update `shipglowz_data/technical/markdown-source-of-truth.md` to point to the live migration and web-reader contract once ready.
- Update `shipglowz_data/technical/code-docs-map.md` with the web-reader contract and write-mode validation surface.
- Update the traffic-first format spec after live migration with the final counts and next lifecycle step.
- Keep `/home/claude/shipglowz/skills/references/operational-record-format.md` as the grammar reference, not the migration execution plan.
...
- [x] Task 1: Create the web-reader contract doc.
  - File: `shipglowz_data/technical/operational-record-web-reader-contract.md`
  ...
  - Validate with: `rg -n "sourcePath|dedupeKey|traffic|project|diagnostics|projection|web reader" shipglowz_data/technical/operational-record-web-reader-contract.md`
...
- [x] Task 2: Add shared fixture examples for web-reader behavior.
  - File: `test/data/shipglowz_sources/fixtures/operational_records_web_reader.md`
  ...
  - Depends on: Task 1.
  - Validate with: `flutter test test/data/shipglowz_sources/parsers`
...
- [x] Task 3: Extend parser tests for web-facing read-model fields.
  ...
  - Validate with: `flutter test test/data/shipglowz_sources/parsers/operational_record_parser_test.dart`
...
- [x] Task 6: Migrate project-local active trackers.
  - File: `shipglowz_data/workflow/TASKS.md` and `shipglowz_data/workflow/AUDIT_LOG.md`
  ...
  - Validate with: `python3 scripts/migrate_operational_records.py --dry-run` and `flutter test test/data/shipglowz_sources`
  - Notes: Preserve non-operational prose only when it remains useful context.

- [x] Task 7: Migrate master trackers.
  - File: `/home/claude/shipglowz_data/TASKS.md` and `/home/claude/shipglowz_data/AUDIT_LOG.md`
...
- [x] Task 9: Update docs and ownership maps after migration.
  - File: `shipglowz_data/technical/markdown-source-of-truth.md`, `shipglowz_data/technical/code-docs-map.md`, and `/home/claude/shipglowz/skills/references/operational-record-format.md`
  ...
  - Validate with: `rg -n "operational-record-web-reader-contract|live migration|legacy fallback|traffic-first" shipglowz_data/technical /home/claude/shipglowz/skills/references/operational-record-format.md`
...
- [x] Task 10: Run final migration validation and record counts.
  - File: `shipglowz_data/workflow/specs/traffic-first-operational-record-live-migration-web-reader-contract.md` and `shipglowz_data/workflow/verification/traffic-first-operational-record-live-migration-web-reader-contract.md`
  ...
  - Validate with: `flutter test test/data/shipglowz_sources`, `cd /home/claude/shipglowz/tui && bun test && bun run typecheck`, metadata lint, and `git diff --check`
...
- `surface`: local Markdown operational sources, migration tooling, Flutter parsers, TUI readers, web-reader contract docs, and rebuildable projection fields for `task`, `audit`, and `spec` records. Covered source files include `shipglowz_data/workflow/TASKS.md`, `shipglowz_data/workflow/AUDIT_LOG.md`, `shipglowz_data/workflow/specs/*.md`, `/home/claude/shipglowz_data/TASKS.md`, `/home/claude/shipglowz_data/AUDIT_LOG.md`, `scripts/migrate_operational_records.py`, `lib/data/shipglowz_sources/parsers/`, `test/data/shipglowz_sources/`, and `/home/claude/shipglowz/tui/src/sources/`.
...
- `checklist_path`: `shipglowz_data/workflow/verification/traffic-first-operational-record-live-migration-web-reader-contract.md`.
- `required_scenario_ids`:
  ...
  - `OR-DOC-001`: technical docs and shared ShipGlowz grammar reference mention the web-reader contract, live migration state, and zero-legacy post-migration policy.
...
- Run parser tests with `flutter test test/data/shipglowz_sources`.
- Run the targeted operational record parser test after adding web-reader fixtures.
- Run TUI checks with `cd /home/claude/shipglowz/tui && bun test && bun run typecheck`.
- Run ShipGlowz metadata lint on touched specs and docs.
- Run `git diff --check` in the project repo and targeted diff checks for `/home/claude/shipglowz_data/TASKS.md`.
...
- Do not stage, commit, or ship automatically unless the invoked ShipGlowz lifecycle step explicitly requires it.
...
- Treat `/home/claude/shipglowz_data/` as a separate high-impact target; re-read files immediately before writing.
...
| 2026-05-30 16:47:00 UTC | sf-ship | GPT-5 Codex | Shipped the verified traffic-first operational record migration and web-reader contract changes. | shipped | /sf-prod shipglowz_app |
...
Next step: `/sf-prod shipglowz_app`
