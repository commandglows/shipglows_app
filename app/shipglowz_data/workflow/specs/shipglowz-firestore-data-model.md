...
owner: "Diane"
confidence: medium
user_story: "En tant que fondatrice de ShipGlowz, je veux un modele Firestore clair pour les utilisateurs, repositories GitHub, projections Markdown, indexations et diagnostics, afin que les shipglowz_data/workflow/specs fondamentales restent coherentes avant toute implementation."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
...
depends_on:
  ...
  - artifact: "https://firebase.google.com/shipglowz_data/firestore/data-model"
    artifact_version: "checked-2026-05-09"
    required_status: "active"
  - artifact: "https://firebase.google.com/shipglowz_data/firestore/security/get-started"
    artifact_version: "checked-2026-05-09"
    required_status: "active"
  - artifact: "https://firebase.google.com/shipglowz_data/firestore/security/rules-structure"
    artifact_version: "checked-2026-05-09"
    required_status: "active"
  - artifact: "https://firebase.google.com/shipglowz_data/firestore/query-data/get-data"
    artifact_version: "checked-2026-05-09"
    required_status: "active"
supersedes: []
evidence:
  - "User decision 2026-05-09: if two users connect the same GitHub repo, ShipGlowz uses one shared project with membership, not duplicate projects."
  ...
  - "User decision 2026-05-09: model minimal ShipGlowz roles now: owner and viewer."
...
next_step: "/sf-start ShipGlowz Firestore Data Model"
---
# Spec: ShipGlowz Firestore Data Model
🟢 [shipglowz_app] spec: ShipGlowz Firestore Data Model | status: ready | path: shipglowz_data/workflow/specs/shipglowz-firestore-data-model.md | next: /sf-start ShipGlowz Firestore Data Model

# Title

ShipGlowz Firestore Data Model
...
- `projects/{projectId}/members/{uid}` stores minimal ShipGlowz membership.
...
- Define how one GitHub repo maps to one shared ShipGlowz project.
...
- Each ShipGlowz project maps to exactly one GitHub repo.
...
- Official Cloud Firestore data model shipglowz_data checked 2026-05-09: documents, collections, and subcollections are the base model.
- Official Firestore Security Rules shipglowz_data checked 2026-05-09: rules must explicitly authorize paths and must be designed around request/auth/resource data.
- Official Firestore query shipglowz_data checked 2026-05-09: client reads should be shaped by document paths and query constraints; rules are not a substitute for product query design.
- `shipglowz_data/workflow/specs/shipglowz-github-managed-clone-indexer.md` defines runner/indexer producer behavior.
- `shipglowz_data/technical/shipglowz-foundational-architecture.md` defines GitHub/Markdown as canonical and Firestore as projection.
...
- Data: all future UI, indexing, onboarding, auth, and dashboard shipglowz_data/workflow/specs must use this schema vocabulary.
...
- Update `shipglowz_data/technical/shipglowz-foundational-architecture.md` to list this as the Firestore schema owner.
...
- User loses GitHub access but still has a ShipGlowz membership document.
...
1. Create `shipglowz_data/technical/firestore-data-model.md`.
   ...
   - Validate with: `rg -n "users/\\{uid\\}|projects/\\{projectId\\}|indexedFiles|indexRuns|diagnostics|projectRefs|feedItems" shipglowz_data/technical/firestore-data-model.md`.
2. Add schema/model contract types under `lib/data/firestore_projection/`.
   - Action: define pure Dart contracts for `ShipGlowzUserProfile`, `ShipGlowzProjectRecord`, `ProjectMemberRecord`, `IndexedFileRecord`, `IndexRunRecord`, `DiagnosticRecord`, `UserProjectRef`, and `UserFeedItem`.
...
5. Define unique GitHub repo resolution behavior in `shipglowz_data/technical/firestore-data-model.md`.
   ...
   - Depends on: Task 1.
   - Validate with: `rg -n "opaque projectId|duplicate|rename|transfer|server-side lookup" shipglowz_data/technical/firestore-data-model.md`.
6. Define projection freshness and deletion behavior in `shipglowz_data/technical/firestore-data-model.md`.
   ...
   - Depends on: Task 1.
   - Validate with: `rg -n "sourceCommit|github\\.headCommit|projectionStatus|deleted|stale|fresh" shipglowz_data/technical/firestore-data-model.md`.
7. Define index-run retention behavior in shipglowz_data and validation helpers.
...
8. Add schema examples in `shipglowz_data/technical/firestore-data-model.md`.
...
9. Update documentation indexes.
   - Action: align `shipglowz_data/editorial/content-map.md`, `shipglowz_data/technical/code-docs-map.md`, and `shipglowz_data/technical/shipglowz-foundational-architecture.md` with the new Firestore schema document and validation command.
   - Depends on: Tasks 1-8.
   - Validate with: `rg -n "Firestore data model|firestore-data-model|firestore_projection" shipglowz_data/editorial/content-map.md shipglowz_data/technical/code-docs-map.md shipglowz_data/technical/shipglowz-foundational-architecture.md`.
...
- Files to read first: `shipglowz_data/technical/shipglowz-foundational-architecture.md`, `shipglowz_data/workflow/specs/shipglowz-github-managed-clone-indexer.md`, `shipglowz_data/workflow/specs/shipglowz-auth-github-access.md`, `shipglowz_data/workflow/specs/shipglowz-markdown-artifact-governance.md`, `shipglowz_data/workflow/specs/shipglowz-dashboard-readonly-projection.md`, `shipglowz_data/technical/code-docs-map.md`, `shipglowz_data/editorial/content-map.md`, and current source-reader models under `lib/data/shipglowz_sources/`.
...
- Patterns to reuse: keep pure parsing/model code close to existing `lib/data/shipglowz_sources/` conventions and keep tests under a mirrored `test/data/firestore_projection/` directory.
...
- Validation commands: `flutter test test/data/firestore_projection`, `rg -n "users/\\{uid\\}|projects/\\{projectId\\}|sourceCommit|indexRuns|projectRefs|feedItems" shipglowz_data/technical/firestore-data-model.md shipglowz_data/workflow/specs/shipglowz-firestore-data-model.md`, and `rg -n "token|clone path|service credential|installation token" shipglowz_data/technical/firestore-data-model.md lib/data/firestore_projection test/data/firestore_projection` with findings manually reviewed for allowed redaction examples only.
...
- Fresh-shipglowz_data verdict: checked. The model uses Firestore collections/documents/subcollections and path-based rule design consistent with official Firebase documentation.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-05-09 18:19:39 UTC | sf-spec | GPT-5 Codex | Created foundational Firestore data model spec from user decisions. | Draft spec created. | /sf-ready ShipGlowz Firestore Data Model |
| 2026-05-10 10:14:49 UTC | sf-ready | GPT-5 Codex | Readiness gate after foundational coherence corrections. | ready | /sf-start ShipGlowz Firestore Data Model |
| 2026-05-10 15:42:22 UTC | sf-build | GPT-5 Codex | Implemented Firestore data model shipglowz_data, Dart contracts, path builders, validators, and tests. | implemented | Decide closure/ship scope for this chantier. |
...
| sf-start | done | Implemented shipglowz_data/technical/firestore-data-model.md and `lib/data/firestore_projection/*` with pure Dart contracts, path builders, and validators. |
