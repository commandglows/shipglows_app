...
owner: "Diane"
confidence: medium
user_story: "En tant qu'utilisatrice ShipGlowz, je veux ouvrir un dashboard qui lit les projets, artefacts, statuts et diagnostics depuis la projection Firestore, afin de comprendre l'etat de mes repositories sans declencher d'ecriture ni exposer l'infrastructure GitHub/clone."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
...
evidence:
  ...
  - "shipglowz-firestore-data-model.md defines user-scoped projectRefs/feedItems, shared projects, indexedFiles, indexRuns, diagnostics, stale projection, and access-lost states."
  - "shipglowz-auth-github-access.md defines old projection visibility/search after GitHub access loss while clone/index/refresh actions are blocked."
  - "shipglowz-project-onboarding-flow.md routes ready projects to the dashboard and requires indexing state to resume from Firestore."
  - "shipglowz-markdown-artifact-governance.md requires dashboard grouping by governance family/type and missing corpus as a setup warning."
  - "markdown-source-of-truth.md states repository Markdown is canonical and remote storage is projection/index/sync."
next_step: "/sf-ship ShipGlowz Dashboard Read-only Projection"
---
# Spec: ShipGlowz Dashboard Read-only Projection
🟢 [shipglowz_app] spec: ShipGlowz Dashboard Read-only Projection | status: ready | path: shipglowz_data/workflow/specs/shipglowz-dashboard-readonly-projection.md | next: /sf-ship ShipGlowz Dashboard Read-only Projection

# Title

ShipGlowz Dashboard Read-only Projection
...
En tant qu'utilisatrice ShipGlowz, je veux ouvrir un dashboard qui lit les projets, artefacts, statuts et diagnostics depuis la projection Firestore, afin de comprendre l'etat de mes repositories sans declencher d'ecriture ni exposer l'infrastructure GitHub/clone.

# Minimal Behavior Contract

The dashboard accepts a Firebase-authenticated user, reads only membership-verifiable or user-scoped Firestore projection documents, and renders project status, indexed ShipGlowz artifacts, freshness, access warnings, diagnostics, and setup gaps without writing repository content or exposing GitHub tokens, installation internals, clone paths, or server filesystem details. If projection data is missing, stale, access-lost, partially indexed, or unauthorized, the dashboard shows a clear read-only state and only offers backend-request actions that are allowed by the auth/access shipglowz_data/workflow/specs. The easy edge case to miss is treating dashboard state as canonical or letting a broad Firestore query reveal projects or artifacts outside the signed-in user's membership.
...
- Given a project has no recognized ShipGlowz artifacts, the dashboard shows an empty governance/setup state instead of treating indexing as failed.
- Given `shipglowz_data/` is missing, duplicated, or legacy-cased, the dashboard shows the governance corpus warning emitted by the indexer.
...
- GitHub repository and ShipGlowz Markdown files remain canonical.
...
- `shipglowz_data/workflow/specs/shipglowz-firestore-data-model.md` defines the Firestore documents consumed by the dashboard.
- `shipglowz_data/workflow/specs/shipglowz-auth-github-access.md` defines Firebase Auth, GitHub access loss, UI cache TTL, and server revalidation rules.
- `shipglowz_data/workflow/specs/shipglowz-github-managed-clone-indexer.md` defines the projection producer and index/run statuses.
- `shipglowz_data/workflow/specs/shipglowz-project-onboarding-flow.md` defines dashboard entry from `ready`, `indexing_project`, blocked, and empty setup states.
- `shipglowz_data/workflow/specs/shipglowz-markdown-artifact-governance.md` defines artifact families, `shipglowz_data/` corpus policy, missing corpus warnings, and safe content projection.
- `shipglowz_data/technical/shipglowz-foundational-architecture.md` defines GitHub/Markdown as canonical, mandatory hidden clone, and read-only V1.
- `shipglowz_data/technical/markdown-source-of-truth.md` defines repository Markdown as source of truth and database projection as rebuildable.
- Fresh external shipglowz_data: not needed for this spec because it is a local product/read-model contract. Implementation shipglowz_data/workflow/specs that add Firebase SDK queries or Firestore rules must re-check official Firebase shipglowz_data.
...
- No `shipglowz_data/` corpus exists.
- Both `shipglowz_data/` and `ShipGlowz_Data/` exist.
...
- [x] Task 1: Create dashboard read-model documentation.
  - File: `shipglowz_data/technical/dashboard-readonly-projection.md`
  ...
  - Validate with: `rg -n "projectRefs|indexedFiles|indexRuns|diagnostics|read-only|access-lost" shipglowz_data/technical/dashboard-readonly-projection.md`
...
- [x] Task 2: Define dashboard state taxonomy.
  ...
  - Depends on: Task 1.
  - Validate with: State table covers every terminal/recoverable state from onboarding, auth/access, and indexer shipglowz_data/workflow/specs.
...
- [x] Task 4: Define dashboard DTO/domain contracts.
  - File: `lib/shipglowz/` and/or future data layer files chosen during implementation.
...
- [x] Task 6: Define dashboard UI behavior.
  - File: `lib/shipglowz/presentation/**`
...
- [x] Task 7: Add security-focused tests.
  - File: `test/shipglowz/` or existing test directories chosen during implementation.
...
- [x] Task 8: Update documentation maps after implementation.
  - File: `shipglowz_data/editorial/content-map.md`, `shipglowz_data/technical/code-docs-map.md`, `shipglowz_data/technical/shipglowz-foundational-architecture.md`, and future README only if behavior exists.
  - Action: Link concrete dashboard shipglowz_data and code ownership.
  - User story link: Keeps future agents on the projection-only contract.
  - Depends on: Tasks 1-7.
  - Validate with: `rg -n "Dashboard read-only|dashboard-readonly|ShipGlowz dashboard" shipglowz_data/editorial/content-map.md shipglowz_data/technical`
...
- [x] AC 6: Given no known ShipGlowz artifacts were indexed, when the dashboard opens the project, then the project shows an empty/setup state, not an unexplained failure.
- [x] AC 7: Given `shipglowz_data/` is missing or duplicated, when dashboard reads diagnostics, then it shows the governance corpus warning without blocking all project visibility.
...
- `checklist_path`: `shipglowz_data/workflow/verification/shipglowz-dashboard-readonly-projection.md`.
...
- Medium maintainability risk if dashboard status names drift from onboarding, auth/access, indexer, and Firestore shipglowz_data/workflow/specs.
...
- Files to read first before implementation: `shipglowz_data/workflow/specs/shipglowz-firestore-data-model.md`, `shipglowz_data/workflow/specs/shipglowz-auth-github-access.md`, `shipglowz_data/workflow/specs/shipglowz-project-onboarding-flow.md`, `shipglowz_data/workflow/specs/shipglowz-markdown-artifact-governance.md`, and `shipglowz_data/technical/markdown-source-of-truth.md`.
...
- Fresh-shipglowz_data verdict: not needed for this spec. Re-check official Firebase/Firestore shipglowz_data before writing SDK queries, Cloud Functions, or Firestore rules.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-05-10 09:26:21 UTC | sf-spec | GPT-5 Codex | Created foundational dashboard read-only projection spec from existing foundational specs and shipglowz_data. | Draft spec created. | /sf-ready ShipGlowz Dashboard Read-only Projection after foundational coherence pass |
| 2026-05-30 16:50:20 UTC | sf-spec | GPT-5 Codex | Repaired readiness gaps after managed clone/indexer closeout: dependency versions, status, Test Contract, proof order, scenarios, and exceptions. | reviewed | /sf-ready ShipGlowz Dashboard Read-only Projection |
| 2026-05-30 16:51:17 UTC | sf-ready | GPT-5 Codex | Reviewed readiness after Test Contract repair. | not ready: blocking dependencies still draft (`shipglowz-auth-github-access.md`, `shipglowz-project-onboarding-flow.md`, `shipglowz-markdown-artifact-governance.md`). | /sf-ready ShipGlowz Auth GitHub Access |
| 2026-05-30 16:56:47 UTC | sf-ready | GPT-5 Codex | Re-ran readiness after dependencies were repaired and marked ready; dashboard contract, proof path, state taxonomy, security boundaries, and docs coherence are actionable. | ready | /sf-start ShipGlowz Dashboard Read-only Projection |
| 2026-05-30 17:04:36 UTC | sf-start | GPT-5 Codex | Implemented read-only projection contract docs, in-memory repository/read models, UI projection panel, user-scope/access-lost/filter/sort/account-switch tests, and docs map alignment. | implemented | /sf-verify ShipGlowz Dashboard Read-only Projection |
| 2026-05-30 17:06:32 UTC | sf-verify | GPT-5 Codex | Verified local contract scope against scenario ids, checks, redaction scan, metadata lint, analyzer, and widget/repository tests. | verified | /sf-end ShipGlowz Dashboard Read-only Projection |
| 2026-05-30 17:07:27 UTC | sf-end | GPT-5 Codex | Closed the local dashboard read-only projection contract slice and prepared it for ship. | closed | /sf-ship ShipGlowz Dashboard Read-only Projection |
