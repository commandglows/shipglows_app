...
evidence:
  ...
  - "shipglowz_app already contains a ready Managed Codex Cockpit MVP specification plus partially started Flutter projection and runner foundation work."
...
depends_on:
  - artifact: "shipglowz_data/workflow/specs/shipglowz-managed-codex-cockpit-mvp.md"
    artifact_version: "1.0.0"
    required_status: ready
supersedes: []
superseded_by: "shipglowz_data/workflow/research/flutter-codex-cockpit-open-source-architecture.md"
...
> Superseded on 2026-08-01 after a broader open-source scan and operator correction. The VS Code Remote Tunnels recommendation and the rejection of a Flutter terminal are withdrawn. The current decision lives in `shipglowz_data/workflow/research/flutter-codex-cockpit-open-source-architecture.md`: one ShipGlowz Flutter product may safely combine a semantic Codex Cockpit with a separately authorized terminal/tmux/Neovim operator workspace.

## Starting Question

Which route should be built first to make current tmux panes containing Codex accessible from any modern browser without a client install, while preserving the longer-term ShipGlowz Flutter application that manages Codex conversations across projects? Which routes should be abandoned?

## Context Read

- `shipglowz/CLAUDE.md` - confirmed the existing server-first PM2 and Caddy operating model.
- `shipglowz_data/technical/local-tunnels-and-mcp-login.md` - confirmed that native Windows currently exposes only a selected SSH tunnel and that the Hetzner browser console is the immediate no-install fallback.
- `shipglowz_app/app/CLAUDE.md` and `AGENT.md` - confirmed the Flutter Web/Android product boundary and the high-risk terminal/agent security rules.
- `shipglowz-managed-codex-cockpit-mvp.md` - found an existing ready specification that already chooses a semantic Codex client over terminal emulation.
- `shipglowz_app/runner/**` and active Flutter Cockpit files - confirmed that runner scaffolding and five-dimensional Cockpit projection work have started but the Codex adapter and conversation interface are not yet implemented.
...
```text
Operator continuity                         ShipGlowz product
──────────────────                         ─────────────────
...
- Pros: self-hosted control plane, workspace lifecycle, identity integration, future multi-user/agent isolation, and strong fit if ShipGlowz later provisions separate environments per project or customer.
...
- Pros: excellent agent-first terminal UX, session concepts, mobile-aware web rendering, and a valuable design reference for ShipGlowz conversations.
...
- Decision: use as UX and architecture inspiration. Do not fork it or select it as the backend for the first ShipGlowz route.
...
Product build
  Flutter Web/Android -> authenticated API/SSE -> managed runner
                                              -> codex app-server
                                             semantic ShipGlowz Cockpit
```

The first move should be a VS Code Remote Tunnels pilot, not a custom terminal build. Hetzner remains the compute backend; VS Code Server is the remote workspace layer; `vscode.dev` is the zero-install browser client; tmux remains the session authority. This directly tests the real keyboard, zoom, font, paste, resize, reconnect, file navigation, and preview-port experience on the new Windows machine before ShipGlowz owns another web service.
...
- Do not treat Microsoft authentication as ShipGlowz product authorization or expose this route to future customers.
...
- Whether Coder becomes justified when ShipGlowz needs isolated workspaces for several users, projects, or concurrent agents.
...
- Self-hosted fallback lifecycle: if Remote Tunnels is rejected, ShipGlowz's current Caddy lifecycle may require an explicit always-on ownership rule for code-server/ttyd instead of accidental coupling to preview apps.
...
- User story seed: As the trusted ShipGlowz operator, I can open an authenticated browser workspace on any modern device and resume my existing tmux/Codex session without installing SSH, WSL, a terminal, or an editor on the client device.
