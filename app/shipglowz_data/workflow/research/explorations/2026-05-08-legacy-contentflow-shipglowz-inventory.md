...
scope: "Legacy ContentFlow inventory and ShipGlowz reuse decisions"
owner: "Diane"
confidence: medium
risk_level: high
security_impact: yes
...
linked_systems:
  ...
  - "AGENT.md"
evidence:
  - "README.md presents ShipGlowz Operations Dashboard as local-first, read-only, no auth, no backend."
  - "pubspec.yaml package name is shipglowz_app."
  - "lib/main.dart defaults APP_TARGET to shipglowz but still exposes legacy/contentflow runtime target."
  - "lib/shipglowz/ contains the current ShipGlowz dashboard runtime."
  ...
  - "CLAUDE.md, AGENT.md, shipglowz_data/workflow/TASKS.md and most shipglowz_data/workflow/specs still describe ContentFlow, Clerk, FastAPI, feedback, OpenRouter and content pipeline work."
depends_on: []
supersedes: []
next_step: "/sf-spec Purge and adapt legacy ContentFlow scope for ShipGlowz"
---

# Exploration Report: Legacy ContentFlow -> ShipGlowz Inventory

## Starting Question

ShipGlowz was split or migrated from a ContentFlow Flutter codebase. The current
main branch launches ShipGlowz by default, but a large ContentFlow runtime remains.
...
- `README.md` - ShipGlowz is documented as local-first, read-only, no auth, no cloud sync, no backend.
- `pubspec.yaml` - package is `shipglowz_app`.
- `lib/main.dart` - default target is ShipGlowz, but `APP_TARGET=legacy|contentflow` still exists.
- `lib/shipglowz/` - current dashboard app: overview, project detail, diagnostics, settings.
- `lib/data/shipglowz_sources/` - current source reader/parser boundary for Markdown and ledgers.
...
- `shipglowz_data/workflow/TASKS.md`, `CLAUDE.md`, `AGENT.md`, `shipglowz_data/workflow/specs/` - mostly legacy ContentFlow tracking and specs.
...
shipglowz_app
  ...
  +-- Legacy embedded runtime
        ...
        lib/data/services/api_service.dart
        web_auth/
        ContentFlow specs, docs, and tasks
```

Deleting the legacy runtime now is risky because some bricks are likely useful
for ShipGlowz V2: auth, BYOK/OpenRouter, feedback, project models, settings,
...
| Brick | Current origin | Possible ShipGlowz value | Initial bucket |
|---|---|---|---|
| `lib/shipglowz/` dashboard | ShipGlowz | Core runtime | Keep |
| `lib/data/shipglowz_sources/` | ShipGlowz | Markdown/ledger database reader | Keep |
| `lib/domain/project_health/` | ShipGlowz | Project posture and next command logic | Keep |
...
| `lib/data/models/auth_session.dart` | ContentFlow | ShipGlowz will need auth eventually | Keep as concept, redesign |
| `lib/data/services/offline_storage_service.dart` | ContentFlow | Useful queue/cache primitives | Keep as concept, adapt later |
| `lib/data/services/feedback_service.dart` | ContentFlow | User feedback is desired for ShipGlowz too | Keep as concept, adapt later |
| `lib/data/services/notification_service.dart` | ContentFlow | FCM scaffold may be useful later | Archive/adapt |
| `lib/core/openrouter_guard.dart` | ContentFlow | BYOK/OpenRouter desired for ShipGlowz too | Keep concept, decouple from ApiService |
...
| `lib/router.dart` | ContentFlow | Route structure not ShipGlowz-native | Archive |
...
| `lib/presentation/screens/onboarding/` | ContentFlow | ShipGlowz may need project/source onboarding | Adapt carefully |
| `feed/editor/history/calendar` | ContentFlow | Maybe useful if ShipGlowz manages indexed content | Park |
| `angles/personas/ritual/seo/newsletter/research/reels` | ContentFlow | Mostly ContentFlow product pipeline | Park / transfer |
| `drip` | ContentFlow | Could become ShipGlowz content publishing later | Park |
| `activity/runs/templates/analytics/performance/uptime/work_domains` | ContentFlow/System | Some operational dashboard ideas may fit ShipGlowz | Review one by one |
| `shipglowz_data/workflow/specs/*contentflow*` and FastAPI shipglowz_data/workflow/specs | ContentFlow | Not ShipGlowz source of truth | Transfer/archive |
| `shipglowz_data/workflow/TASKS.md`, `CLAUDE.md`, `AGENT.md` | ContentFlow | Actively misleading for ShipGlowz | Rewrite for ShipGlowz |

## Option Space

### Option A: Hard purge now

- Summary: Delete all legacy ContentFlow runtime and shipglowz_data now.
- Pros: Small, clean ShipGlowz repo quickly.
...
- Cons: Keeps architecture confusion, stale tests, stale shipglowz_data and misleading decisions.
- Verdict: Not enough.
...
1. Freeze the current ShipGlowz runtime boundary:
   - `lib/shipglowz/`
   - `lib/data/shipglowz_sources/`
   - `lib/domain/project_health/`
...
1. Is ShipGlowz a local operator cockpit only, or will it become a hosted user product?
2. Is the target user only you, or other users too?
3. Should ShipGlowz remain desktop-first, or is web/mobile important soon?
...
9. Which entities are real ShipGlowz entities: projects, tasks, shipglowz_data/workflow/specs, audit events, operations, dependencies, content items, users, workspaces?
...
13. Is FastAPI still acceptable for local/dev tooling, or should it be excluded from ShipGlowz?
...
17. Does ShipGlowz auth need Google login only, email/password, GitHub, or all three?
...
21. Is OpenRouter BYOK needed for ShipGlowz V1 or later?
...
26. Is feedback for users of ShipGlowz, or operator notes for yourself?
...
30. Is feedback a core ShipGlowz feature or just support tooling?
...
33. Could drip/publishing become relevant to ShipGlowz, or should it be transferred out?
34. Do activity/runs/templates/performance/uptime have ShipGlowz equivalents?
...
40. Should README document only ShipGlowz, or also mention archived legacy mode?
...
- Reusing the archived Supabase WIP as ShipGlowz direction - it mostly reflects a ContentFlow migration path.
...
- User story seed: As the ShipGlowz operator, I want the repo separated into current ShipGlowz runtime, reusable future ShipGlowz bricks, archived ContentFlow code, and transferred/deleted legacy pieces, so future work can proceed without losing useful code or following stale ContentFlow assumptions.
...
- Invariants/constraints seed:
  ...
  - ShipGlowz default runtime must remain working.
...
- Validation seed:
  ...
  - targeted ShipGlowz source/parser tests
  - `rg` checks for active ContentFlow references after each staged cleanup
  - manual launch of ShipGlowz dashboard

## Handoff

- Recommended next command: `/sf-spec Purge and adapt legacy ContentFlow scope for ShipGlowz`
- Why this next step: The work crosses shipglowz_data, runtime, tests, scripts and product decisions; it should not be executed as a casual cleanup.

## Exploration Run History

| Date UTC | Prompt/Focus | Action | Result | Next step |
|----------|--------------|--------|--------|-----------|
| 2026-05-08 | Inventory legacy ContentFlow before deleting anything | Read runtime, shipglowz_data, routes, services, models and shipglowz_data/workflow/specs; classified reusable bricks and decision questions | Staged extraction recommended | Answer direction questions, then create spec |
