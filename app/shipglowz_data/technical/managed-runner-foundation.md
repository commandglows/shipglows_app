...
linked_systems:
  ...
  - "app/lib/shipglowz/data/managed_runner_api.dart"
  - "app/lib/shipglowz/providers/managed_runner_provider.dart"
...
next_step: "Map dashboard project identities into the Flutter conversation panel and run provider-configured Codex/GitHub smoke proof."
---
...
The managed runner is ShipGlowz's private control-plane service. It gives the Flutter application one versioned API boundary while keeping agent processes, repository workspaces, credentials, filesystem locations, PTY/tmux control and provider transports on the server.
...
- Flutter exposes a provider-neutral `ShipGlowzAuthProvider`. The optional Supabase adapter maps only `userId`, access token and expiry into that contract; no Supabase wire type reaches feature code.
...
- The workspace manager owns opaque mirror, audit and fix paths. It rejects traversal and escaping symlinks, creates detached audit worktrees, creates one local `shipglowz/fix/...` branch per fix conversation, serializes mutation per project, and can remove stale worktrees.
...
- The runner, not the Flutter client, maps a Supabase subject to a ShipGlowz actor and tenant. A bearer token alone never selects a project or expands permissions.
...
cd ../app && flutter analyze && flutter test test/shipglowz/auth/auth_provider_test.dart
```
...
Update this document whenever the runner gains an adapter, route family, auth provider, persistence schema, execution provider, capability rule or public diagnostic surface. Do not describe a live Supabase, GitHub or Codex integration as proven until its provider-configured smoke test passes. The GitHub App and workspace classes are locally contract-tested, but no real App installation, hosted runner, clone, or provider-configured smoke has yet run.
