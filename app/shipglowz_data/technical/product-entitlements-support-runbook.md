...
Allowed: `product_id=shipglowz_app`, environment, redacted user id, visible email, status, reason code, request id, `checked_at`, `expires_at`, backend status.
...
| --- | --- | --- |
| recognized, no access | No active `shipglowz_app` entitlement. | Confirm suite ledger access path. |
...
```bash
rg -n "recognized|no access|revoked|refunded|pending_review|wrong environment|redacted|shipglowz_app" shipglowz_data/technical/product-entitlements-support-runbook.md
```
