...
ShipGlowz App separates identity, GitHub authorization, cache, and product access. The canonical product id is `shipglowz_app`.

WinFlowz suite ledger is the durable source of truth. ShipGlowz App may keep a bounded snapshot or server-owned mirror, but it must not create a second durable entitlement ledger.
...
```bash
rg -n "shipglowz_app|grantsAccess|suite ledger|fail closed|OPEN_ACCESS|cache" shipglowz_data/technical/product-entitlements.md
