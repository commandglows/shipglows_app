...
depends_on:
  ...
  - "shipglowz_data/workflow/specs/shipglowz-legacy-contentflow-fusion.md@0.3.0"
supersedes: []
evidence:
...
# ShipGlowz Legacy Reuse Roadmap

## Purpose

This roadmap turns the embedded ContentFlow code into ShipGlowz decisions. It is not a deletion plan. It is a decision aid for choosing what to reuse, rewrite, park, or archive.

## Owned Files

- `shipglowz_data/technical/shipglowz-legacy-reuse-roadmap.md`
...
| 2 | Settings/preferences | Adapt | ShipGlowz will need source paths, theme, language, BYOK and project settings | Split generic settings from ContentFlow content settings |
...
- Split settings into `ShipGlowzAppSettings` later.
- Remove content-specific assumptions only when the replacement model exists.

Decision question:

- Should ShipGlowz settings be per device first, per workspace first, or both with local defaults and cloud projection later?
...
- Adapt the project concept into a ShipGlowz GitHub repository model.
...
- Answered 2026-05-09: a ShipGlowz project is necessarily a GitHub repository. It has a local clone and a remote database projection. Future workspaces may group repositories, but project and GitHub repo are not separate concepts.
...
- Is the first hosted ShipGlowz account model "one user owns private repos" or "workspace/team with shared repos"?
...
- Delay write queue design until ShipGlowz write-back rules are explicit.

Decision question:

- Do we need ShipGlowz to edit Markdown from the UI soon, or is V1.5 still read/index/analyze only?
