...
- `shipflow_data/workflow/specs/shipflow-firestore-data-model.md` defines the Firestore documents consumed by the dashboard.
- `shipflow_data/workflow/specs/shipflow-auth-github-access.md` defines Firebase Auth, GitHub access loss, UI cache TTL, and server revalidation rules.
- `shipflow_data/workflow/specs/shipflow-github-managed-clone-indexer.md` defines the projection producer and index/run statuses.
- `shipflow_data/workflow/specs/shipflow-project-onboarding-flow.md` defines dashboard entry from `ready`, `indexing_project`, blocked, and empty setup states.
- `shipflow_data/workflow/specs/shipflow-markdown-artifact-governance.md` defines artifact families, `shipflow_data/` corpus policy, missing corpus warnings, and safe content projection.
- `shipflow_data/technical/shipflow-foundational-architecture.md` defines GitHub/Markdown as canonical, mandatory hidden clone, and read-only V1.
- `shipflow_data/technical/markdown-source-of-truth.md` defines repository Markdown as source of truth and database projection as rebuildable.
- Fresh external shipflow_data: not needed for this spec because it is a local product/read-model contract. Implementation shipflow_data/workflow/specs that add Firebase SDK queries or Firestore rules must re-check official Firebase shipflow_data.
...
- [ ] Task 8: Update documentation maps after implementation.
  ...
  - Validate with: `rg -n "Dashboard read-only|dashboard-readonly|ShipFlow dashboard" shipflow_data/editorial/content-map.md shipflow_data/technical`
...
- Medium maintainability risk if dashboard status names drift from onboarding, auth/access, indexer, and Firestore shipflow_data/workflow/specs.
...
- Files to read first before implementation: `shipflow_data/workflow/specs/shipflow-firestore-data-model.md`, `shipflow_data/workflow/specs/shipflow-auth-github-access.md`, `shipflow_data/workflow/specs/shipflow-project-onboarding-flow.md`, `shipflow_data/workflow/specs/shipflow-markdown-artifact-governance.md`, and `shipflow_data/technical/markdown-source-of-truth.md`.
...
- Fresh-shipflow_data verdict: not needed for this spec. Re-check official Firebase/Firestore shipflow_data before writing SDK queries, Cloud Functions, or Firestore rules.
