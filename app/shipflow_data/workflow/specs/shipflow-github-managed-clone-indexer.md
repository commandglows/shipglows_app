...
owner: "Diane"
confidence: medium
user_story: "En tant que fondatrice de ShipFlow, je veux un runner/indexer qui lit un repo GitHub via un clone géré et projette les fichiers Markdown ShipFlow dans Firestore, afin que l'app puisse afficher les projets sans exposer de secrets ni rendre Firestore canonique."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
...
depends_on:
  ...
  - artifact: "shipflow_data/workflow/specs/shipflow-firestore-data-model.md"
    ...
    required_status: "ready"
  - artifact: "shipflow_data/technical/firestore-data-model.md"
    artifact_version: "0.1.0"
    required_status: "draft"
...
- `shipflow_data/workflow/specs/shipflow-auth-github-access.md` defines access statuses, server-side revalidation, backend-only installation metadata, and GitHub-wins behavior.
- `shipflow_data/workflow/specs/shipflow-firestore-data-model.md` and `shipflow_data/technical/firestore-data-model.md` define existing projection records and path builders under `lib/data/firestore_projection/`.
...
- Keep `shipflow_data/technical/firestore-data-model.md` and `lib/data/firestore_projection/*` aligned when new index-run fields or status enums are added.
- Update `shipflow_data/workflow/TASKS.md` and `CHANGELOG.md` after implementation changes behavior or creates new code surfaces.
...
- [ ] Task 1: Extend the Firestore projection model for runner-owned index request metadata.
  - File: `lib/data/firestore_projection/firestore_projection_models.dart`
  - Action: Add or extend pure Dart records/enums for index request status, runner diagnostic codes, access status mirroring `shipflow-auth-github-access.md`, idempotency `requestId`, and active-run metadata without importing Firebase SDKs.
  ...
  - Depends on: Existing `lib/data/firestore_projection/*` contracts from `shipflow-firestore-data-model.md`.
...
- [ ] Task 2: Add path and validation support for runner-owned payloads.
...
- [ ] Task 3: Create the runner contract document.
  - File: `shipflow_data/technical/github-managed-clone-indexer.md`
  ...
  - Validate with: `rg -n "requestId|already_running|source_too_large|index_timeout|verifyGitHubAccess|indexRepository|2 MB|20 MB|token" shipflow_data/technical/github-managed-clone-indexer.md`.
  - Notes: Explicitly state that GitHub App installation access is the only V1 repo authorization model.
- [ ] Task 4: Define allowed Markdown artifact indexing policy.
  ...
  - Action: Add a pure allowlist/classifier for `shipflow_data/business/*`, `shipflow_data/editorial/*`, `shipflow_data/technical/*`, `shipflow_data/workflow/TASKS.md`, `shipflow_data/workflow/AUDIT_LOG.md`, `shipflow_data/workflow/specs/*.md`, and root compatibility docs (`AGENT.md`, `AGENTS.md`, `CLAUDE.md`, `README.md`, `CHANGELOG.md`).
  - User story link: Ensures the indexer reads ShipFlow governance artifacts, not arbitrary repository Markdown.
  - Depends on: Task 3.
  - Validate with: `flutter test test/data/shipflow_sources/shipflow_artifact_index_policy_test.dart`.
  - Notes: `shipflow_data/` wins over root fallback docs; do not index `.git`, build output, secrets, or arbitrary docs by default.
- [ ] Task 5: Define the local fake runner adapter for implementation tests.
  - File: `lib/data/shipflow_sources/local_fake_clone_indexer.dart`
  - File: `test/data/shipflow_sources/local_fake_clone_indexer_test.dart`
  ...
  - Validate with: `flutter test test/data/shipflow_sources/local_fake_clone_indexer_test.dart`.
  - Notes: This is not the production runner; it is the test adapter for server-shaped payloads.
- [ ] Task 6: Define runner repository interfaces for the active ShipFlow app layer.
  - File: `lib/shipflow/data/github_managed_clone_indexer_repository.dart`
  - File: `test/shipflow/data/github_managed_clone_indexer_repository_test.dart`
  ...
  - Validate with: `flutter test test/shipflow/data/github_managed_clone_indexer_repository_test.dart`.
...
- [ ] Task 7: Add security and failure-mode tests.
  - File: `test/data/shipflow_sources/local_fake_clone_indexer_test.dart`
  ...
  - Validate with: `flutter test test/data/firestore_projection test/data/shipflow_sources test/shipflow/data`.
...
- [ ] Task 8: Draft Firestore and backend security-rule requirements before production writes.
  - File: `shipflow_data/technical/github-managed-clone-indexer.md`
  ...
  - Validate with: `rg -n "App Check|rate limit|server-owned|membership|redacted|Security Rules|backend-only" shipflow_data/technical/github-managed-clone-indexer.md`.
  - Notes: Do not deploy real rules or Cloud Functions in this chantier.
- [ ] Task 9: Update documentation maps and public docs only where behavior exists.
  - File: `shipflow_data/editorial/content-map.md`
  - File: `shipflow_data/technical/code-docs-map.md`
  - File: `shipflow_data/technical/shipflow-foundational-architecture.md`
  - File: `README.md`
  - File: `CHANGELOG.md`
  ...
  - Validate with: `rg -n "github-managed-clone-indexer|local_fake_clone_indexer|GitHub managed clone" shipflow_data/editorial/content-map.md shipflow_data/technical README.md CHANGELOG.md`.
  - Notes: Do not claim Firebase/GitHub production indexing exists until it does.
- [ ] Task 10: Run the bounded verification gate.
  - File: `shipflow_data/workflow/specs/shipflow-github-managed-clone-indexer.md`
  ...
  - Validate with: `flutter analyze`, `flutter test`, `rg -n "token|installationToken|clonePath|cloneUrl|serviceCredential|x-access-token" lib test shipflow_data/technical`.
...
- Files to read first: `shipflow_data/technical/firestore-data-model.md`, `lib/data/firestore_projection/*`, `lib/data/shipflow_sources/source_file_reader.dart`, `lib/data/shipflow_sources/source_path_policy.dart`, `shipflow_data/workflow/specs/shipflow-auth-github-access.md`, and `shipflow_data/technical/shipflow-foundational-architecture.md`.
...
- Validation commands: `flutter test test/data/firestore_projection`, `flutter test test/data/shipflow_sources`, `flutter test test/shipflow/data`, `flutter analyze`, `flutter test`, and `rg -n "token|installationToken|clonePath|cloneUrl|serviceCredential|x-access-token" lib test shipflow_data/technical`.
...
| 2026-05-14 16:15:30 UTC | sf-ready | GPT-5 Codex | Readiness gate for managed clone/indexer spec, including adversarial/security/fresh-docs review. | not ready | /sf-spec ShipFlow GitHub Managed Clone Indexer readiness fixes |
| 2026-05-14 16:23:00 UTC | sf-spec | GPT-5 Codex | Repaired readiness gaps: dependencies, concrete implementation tasks, auth model, idempotency, concurrency, size limits, token retry, docs freshness, and validation commands. | draft updated | /sf-ready ShipFlow GitHub Managed Clone Indexer |
| 2026-05-14 17:32:39 UTC | sf-ready | GPT-5 Codex | Re-ran readiness gate after repairs: structure, metadata, user-story traceability, task ordering, adversarial review, security review, language doctrine, and fresh Firebase/GitHub docs. | ready | /sf-start ShipFlow GitHub Managed Clone Indexer |
