...
Ready. This spec resolves the readiness gaps found on 2026-04-27: open hypotheses are closed, metadata is coherent with local shipflow_data, current official dependency shipflow_data are named, codegen major policy is explicit, and security impact is treated as low but real because the migration touches auth-adjacent route/state behavior and hosted packages.
...
- Action: le mainteneur exécute la migration par lots ordonnés: baseline, Riverpod runtime, Riverpod API fixes, annotation/generator, GoRouter, Google Fonts, full validation, shipflow_data.
...
- Les versions exactes doivent être confirmées par `flutter pub outdated`, `flutter pub get` et shipflow_data officielles actuelles pendant l'implémentation.
...
Fresh external shipflow_data verdict: fresh-shipflow_data checked on 2026-04-27.

- `flutter_riverpod`: pub.dev versions page shows latest stable `3.3.1`, minimum Dart SDK `3.7`; official changelog and Riverpod migration shipflow_data identify relevant breaks: legacy provider imports, `AsyncValue.valueOrNull` removed, `AsyncValue.value` null-on-error, `ProviderObserverContext`, automatic retry, paused listeners, provider update filtering by `==`, ref lifecycle changes, and `ProviderException`.
...
- `google_fonts`: pub.dev page shows latest stable `8.0.2`; shipflow_data state support for HTTP fetching, caching and asset bundling; changelog notes WOFF2/WOFF bundled web font changes and font catalog additions/removals.

Official sources consulted:

- https://pub.dev/packages/flutter_riverpod/versions
- https://pub.dev/packages/flutter_riverpod/changelog
- https://riverpod.dev/shipflow_data/whats_new
- https://riverpod.dev/shipflow_data/3.0_migration
...
- Business, pricing, onboarding copy, screenshots and support shipflow_data: no change expected because the user-facing feature contract is intentionally unchanged.
- Existing behavior shipflow_data/workflow/specs remain contracts to revalidate, not shipflow_data to rewrite: `SPEC-offline-sync-v2.md`, `spec-no-ui-jump-on-resume.md`, `SPEC-project-flows-selection-onboarding-archive.md`.
...
- [x] Tâche 2 : Establish rollback checkpoints by lot
  ...
  - Action : Record previous package versions and planned file groups for Riverpod runtime, Riverpod API, codegen, GoRouter, Google Fonts and shipflow_data.
  - User story link : Lets maintainers reduce risk without global resets.
  - Depends on : Tâche 1
  - Validate with : `git diff --stat` before first edit and `shipflow_data/workflow/specs/migrate-flutter-core-majors-baseline.md`
...
- [x] Tâche 25 : Update developer shipflow_data/changelog
  ...
  - Depends on : Tâche 24
  - Validate with : manual shipflow_data diff review
  - Notes : Completed on 2026-04-27 with migration entry in `CHANGELOG.md` and concise Riverpod 3 legacy/codegen policy note in `shipflow_data/technical/guidelines.md`. No business/branding shipflow_data changed.

- [x] Tâche 26 : Final rollback and security review
  - Fichier : `pubspec.yaml`, `pubspec.lock`, touched Dart/test/shipflow_data files
  ...
  - Depends on : Tâche 25
  - Validate with : `git diff --stat`, `git diff -- pubspec.lock`, route test results and shipflow_data review
...
- [x] CA 17 : Given implementation changes developer workflow or conventions, when shipflow_data are reviewed, then `CHANGELOG.md` and any relevant `shipflow_data/technical/guidelines.md` updates are present.
...
7. Run full gate, smoke, shipflow_data, rollback/security review.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-04-27 | sf-verify | GPT-5 | Verified Flutter core major migration against spec, diff, official dependency shipflow_data, package resolution, analyze/test/codegen checks, and Clerk runtime smoke availability. | partial | Run Clerk/web runtime smoke, add migration changelog/shipflow_data note, then rerun sf-verify. |
| 2026-04-27 | sf-verify | GPT-5 | Re-verified after Clerk smoke success, shipflow_data/changelog update, build artifact cleanup, bug gate, dependency/source scan, secret scan, analyze, tests, and codegen gate. | verified | Run /sf-end to close the chantier. |
| 2026-04-27 | sf-end | GPT-5 | Closed the migration chantier after verified checks, shipflow_data, task tracker updates, and final security/rollback review. | closed | Run /sf-ship to commit and push the scoped migration changes. |
| 2026-04-27 | sf-ship | GPT-5 | Shipped scoped Flutter core major migration changes after green analyze, tests, codegen, Clerk smoke, shipflow_data, and tracker updates. | shipped | None. |
