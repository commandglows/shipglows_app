...
🟢 [shipflow_app] task: Garder `origin/main` comme base saine et conserver le WIP Supabase local en archive | status: done
🟢 [shipflow_app] task: Nettoyer les mentions ContentFlow résiduelles dans les surfaces actives (README, CLAUDE, AGENT, content-map) | status: done
🟢 [shipflow_app] task: Corriger les imports/tests obsoletes dans l'ordre des artefacts actifs après classification | status: done
🟢 [shipflow_app] task: Stabiliser la frontiere ShipFlow actif / ContentFlow legacy | status: done
🟢 [shipflow_app] task: Classer les briques ContentFlow avant suppression (lib/data/services + lib/presentation) | status: done
🟢 [shipflow_app] task: Centraliser les accès legacy dans un contrat de test (`test/legacy_contract.dart`) | status: done
🟢 [shipflow_app] task: Ajouter un garde `scripts/validate-legacy-test-boundary.sh` pour bloquer les imports legacy directs en tests | status: done
🟢 [shipflow_app] task: Ajouter un garde `scripts/validate-shipflow-runtime-boundary.sh` pour bloquer les imports legacy directs dans `lib/shipflow` | status: done
🟢 [shipflow_app] task: Ajouter `scripts/validate-boundary-suite.sh` (orchestration locale/CI des checks de frontière) | status: done
🟢 [shipflow_app] task: Intégrer `validate-boundary-suite.sh` dans le workflow CI | status: done
🟢 [shipflow_app] task: Documenter Markdown/repo comme source de verite et DB future comme projection | status: done
🟢 [shipflow_app] task: Creer le gate canonique de coherence fondation ShipFlow avant implementation Firebase/GitHub/Firestore | status: done
🟢 [shipflow_app] task: Implementer le modele Firestore documentaire et les contrats Dart purs (`lib/data/firestore_projection/*`) avec tests cibles | status: done
🟢 [shipflow_app] task: Deplacer le contenu actif vers `app/` et réarmer la façade `site/` avec le contenu ShipFlow App en gardant le design | status: done
🟠 [shipflow_app] task: Implementer Firebase Auth, GitHub App access, Firestore rules, and Cloud Functions only after the ready specs and fresh official-doc checks pass | status: deferred
🟠 [shipflow_app] task: Spec future write-back, agent runner, terminal, BYOK, and feedback separately; keep them out of the V1 read-only implementation | status: deferred
...
| ✅ | Feedback Admin v1 côté Flutter — soumission texte/audio, historique local léger, accès anonyme et écran admin in-app | High | Medium | ✅ done — client FastAPI prêt, build-time allowlist + shipflow_data ajoutés |
