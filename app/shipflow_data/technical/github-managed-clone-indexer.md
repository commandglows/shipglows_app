...
depends_on:
  ...
  - "shipflow_data/technical/firestore-data-model.md@0.1.0"
supersedes: []
evidence:
  - "Pure Dart contracts and local fake runner implementation."
next_review: "2026-06-14"
next_step: "/sf-verify ShipFlow GitHub Managed Clone Indexer"
---
...
- `projectId` as an opaque ShipFlow project id.
...
- Index only the allowlisted ShipFlow Markdown corpus.
...
The current implementation in `lib/data/shipflow_sources/local_fake_clone_indexer.dart` is not a production runner. It reads a local fixture/repository path through `SourcePathPolicy`, applies `ShipFlowArtifactIndexPolicy`, emits Firestore-shaped projection records, reconciles deleted files as tombstones, and validates that client-readable payloads contain no secret-like fields.
...
flutter test test/shipflow/data
rg -n "requestId|already_running|source_too_large|index_timeout|verifyGitHubAccess|indexRepository|2 MB|20 MB|token" shipflow_data/technical/github-managed-clone-indexer.md
rg -n "App Check|rate limit|server-owned|membership|redacted|Security Rules|backend-only" shipflow_data/technical/github-managed-clone-indexer.md
```
