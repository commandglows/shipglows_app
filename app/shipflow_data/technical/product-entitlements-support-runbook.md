...
docs_impact: yes
linked_systems:
  - "ShipFlow entry screen"
  - "AppAccessState diagnostics"
  - "WinFlowz suite ledger"
depends_on:
  - artifact: "shipflow_data/technical/product-entitlements.md"
    artifact_version: "0.1.0"
...
Allowed: `product_id=shipflow_app`, environment, redacted user id, visible email, status, reason code, request id, `checked_at`, `expires_at`, backend status.
...
| --- | --- | --- |
| recognized, no access | No active `shipflow_app` entitlement. | Confirm suite ledger access path. |
...
```bash
rg -n "recognized|no access|revoked|refunded|pending_review|wrong environment|redacted|shipflow_app" shipflow_data/technical/product-entitlements-support-runbook.md
```
