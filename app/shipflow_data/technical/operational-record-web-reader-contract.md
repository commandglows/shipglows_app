...
linked_systems:
  ...
  - "test/data/shipflow_sources/parsers/operational_record_parser_test.dart"
  - "test/data/shipflow_sources/fixtures/operational_records_web_reader.md"
...
Define the normalized read-model contract for rendering operational records in future ShipFlow web surfaces from Markdown sources, without introducing a second parser behavior. Migration output must be readable by this model directly.
...
Canonical line shape and required fields remain defined by `/home/claude/shipflow/skills/references/operational-record-format.md`.
...
1. Canonical record keys are derived per `/home/claude/shipflow/skills/references/operational-record-format.md` and are stable for the same project + semantic identity.
...
`OperationalRecordParser` in Flutter and terminal readers in `/home/claude/shipflow/tui/src/sources/` must expose the same required read-model fields and diagnostics semantics:
