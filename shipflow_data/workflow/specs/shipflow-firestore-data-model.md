...
owner: "Diane"
confidence: medium
user_story: "En tant que fondatrice de ShipFlow, je veux un modele Firestore clair pour les utilisateurs, repositories GitHub, projections Markdown, indexations et diagnostics, afin que les shipflow_data/workflow/specs fondamentales restent coherentes avant toute implementation."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
...
depends_on:
  ...
  - artifact: "https://firebase.google.com/shipflow_data/firestore/data-model"
    artifact_version: "checked-2026-05-09"
    required_status: "active"
  - artifact: "https://firebase.google.com/shipflow_data/firestore/security/get-started"
    artifact_version: "checked-2026-05-09"
    required_status: "active"
  - artifact: "https://firebase.google.com/shipflow_data/firestore/security/rules-structure"
    artifact_version: "checked-2026-05-09"
    required_status: "active"
  - artifact: "https://firebase.google.com/shipflow_data/firestore/query-data/get-data"
    artifact_version: "checked-2026-05-09"
    required_status: "active"
supersedes: []
evidence:
...
En tant que fondatrice de ShipFlow, je veux un modele Firestore clair pour les utilisateurs, repositories GitHub, projections Markdown, indexations et diagnostics, afin que les shipflow_data/workflow/specs fondamentales restent coherentes avant toute implementation.
...
- Official Cloud Firestore data model shipflow_data checked 2026-05-09: documents, collections, and subcollections are the base model.
- Official Firestore Security Rules shipflow_data checked 2026-05-09: rules must explicitly authorize paths and must be designed around request/auth/resource data.
- Official Firestore query shipflow_data checked 2026-05-09: client reads should be shaped by document paths and query constraints; rules are not a substitute for product query design.
- `shipflow_data/workflow/specs/shipflow-github-managed-clone-indexer.md` defines runner/indexer producer behavior.
- `shipflow_data/technical/shipflow-foundational-architecture.md` defines GitHub/Markdown as canonical and Firestore as projection.
...
- Data: all future UI, indexing, onboarding, auth, and dashboard shipflow_data/workflow/specs must use this schema vocabulary.
...
- Update `shipflow_data/technical/shipflow-foundational-architecture.md` to list this as the Firestore schema owner.
...
1. Create `shipflow_data/technical/firestore-data-model.md`.
   ...
   - Validate with: `rg -n "users/\\{uid\\}|projects/\\{projectId\\}|indexedFiles|indexRuns|diagnostics|projectRefs|feedItems" shipflow_data/technical/firestore-data-model.md`.
...
5. Define unique GitHub repo resolution behavior in `shipflow_data/technical/firestore-data-model.md`.
   ...
   - Depends on: Task 1.
   - Validate with: `rg -n "opaque projectId|duplicate|rename|transfer|server-side lookup" shipflow_data/technical/firestore-data-model.md`.
6. Define projection freshness and deletion behavior in `shipflow_data/technical/firestore-data-model.md`.
   ...
   - Depends on: Task 1.
   - Validate with: `rg -n "sourceCommit|github\\.headCommit|projectionStatus|deleted|stale|fresh" shipflow_data/technical/firestore-data-model.md`.
7. Define index-run retention behavior in shipflow_data and validation helpers.
...
8. Add schema examples in `shipflow_data/technical/firestore-data-model.md`.
...
9. Update documentation indexes.
   - Action: align `shipflow_data/editorial/content-map.md`, `shipflow_data/technical/code-docs-map.md`, and `shipflow_data/technical/shipflow-foundational-architecture.md` with the new Firestore schema document and validation command.
   - Depends on: Tasks 1-8.
   - Validate with: `rg -n "Firestore data model|firestore-data-model|firestore_projection" shipflow_data/editorial/content-map.md shipflow_data/technical/code-docs-map.md shipflow_data/technical/shipflow-foundational-architecture.md`.
...
- Files to read first: `shipflow_data/technical/shipflow-foundational-architecture.md`, `shipflow_data/workflow/specs/shipflow-github-managed-clone-indexer.md`, `shipflow_data/workflow/specs/shipflow-auth-github-access.md`, `shipflow_data/workflow/specs/shipflow-markdown-artifact-governance.md`, `shipflow_data/workflow/specs/shipflow-dashboard-readonly-projection.md`, `shipflow_data/technical/code-docs-map.md`, `shipflow_data/editorial/content-map.md`, and current source-reader models under `lib/data/shipflow_sources/`.
...
- Validation commands: `flutter test test/data/firestore_projection`, `rg -n "users/\\{uid\\}|projects/\\{projectId\\}|sourceCommit|indexRuns|projectRefs|feedItems" shipflow_data/technical/firestore-data-model.md shipflow_data/workflow/specs/shipflow-firestore-data-model.md`, and `rg -n "token|clone path|service credential|installation token" shipflow_data/technical/firestore-data-model.md lib/data/firestore_projection test/data/firestore_projection` with findings manually reviewed for allowed redaction examples only.
...
- Fresh-shipflow_data verdict: checked. The model uses Firestore collections/documents/subcollections and path-based rule design consistent with official Firebase documentation.
...
| 2026-05-10 15:42:22 UTC | sf-build | GPT-5 Codex | Implemented Firestore data model shipflow_data, Dart contracts, path builders, validators, and tests. | implemented | Decide closure/ship scope for this chantier. |
...
| sf-start | done | Implemented shipflow_data/technical/firestore-data-model.md and `lib/data/firestore_projection/*` with pure Dart contracts, path builders, and validators. |
