...
- Review backend shipflow_data in `contentflow_lab/README.md` or project API shipflow_data if they document `/api/projects` as GitHub-only or hard-delete-only.
...
- Stop conditions:
  ...
  - If existing project source/crawl shipflow_data/workflow/specs already define a conflicting canonical source field, stop and update this spec before implementation rather than implementing both.
