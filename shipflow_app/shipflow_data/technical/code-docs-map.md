...
| Product entitlement gate | Local product-access contract separating identity, GitHub access, cache, and entitlement truth | `shipflow_data/technical/product-entitlements.md`, `shipflow_data/technical/product-entitlement-bridge-contract.md`, and `shipflow_data/technical/product-entitlements-support-runbook.md` | `flutter test test/data/models/app_entitlement_test.dart test/core/app_access_resume_test.dart` | Any `shipflow_app` entitlement snapshot, `AppAccessState`, bootstrap parsing, open-access, cache authorization, support diagnostics, or suite bridge contract change |
| `lib/data/models/` | Mixed shared and legacy models | `shipflow_data/technical/legacy-contentflow-inventory.md`; entitlement-specific model owned by `shipflow_data/technical/product-entitlements.md` | `flutter test test/data test/core` | Any model reused by active ShipFlow or future features |
| `lib/data/services/` | Mostly legacy service layer | `shipflow_data/technical/legacy-contentflow-inventory.md`; bootstrap entitlement boundary owned by `shipflow_data/technical/product-entitlement-bridge-contract.md` | `flutter test test/data` | Any API, auth, feedback, storage, offline, notification, bootstrap, or entitlement snapshot parsing change |
...
| `site` + `app UI` | Shared visual governance for both website and Flutter UI | `shipflow_data/technical/design-system-authority.md` | `python3 /home/claude/shipflow/tools/design_system_drift_check.py --changed --warn-only --root /home/claude/shipflow_app` puis `rg -n "design_system_authority" shipflow_data/technical/design-system-authority.md` | Toute évolution visuelle production, tokens, ou migration vers token carriers |
...
| Product access mirror | Future server-owned `suiteAccess` or equivalent mirror for product entitlement checks before protected Firestore data | `shipflow_data/technical/product-entitlements.md` and `shipflow_data/technical/firestore-data-model.md` | `rg -n "suiteAccess|product entitlement|shipflow_app|server-owned mirror|grantsAccess" shipflow_data/technical/firestore-data-model.md shipflow_data/technical/product-entitlements.md` | Any Firestore rules, mirror schema, product access projection, or dashboard protected read/write implementation |
...
| Dashboard read-only projection | Dashboard read model for user-scoped Firestore projection, artifact groups, freshness, access warnings, diagnostics, and index status | `shipflow_data/technical/dashboard-readonly-projection.md` and `shipflow_data/workflow/specs/shipflow-dashboard-readonly-projection.md` | `flutter test test/shipflow/data/dashboard_readonly_projection_repository_test.dart` | Any ShipFlow dashboard route, provider, Firestore query, projection DTO, artifact detail, diagnostics panel, stale/access-lost state, or refresh affordance |
