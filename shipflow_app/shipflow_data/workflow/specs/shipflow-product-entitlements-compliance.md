...
owner: "Diane"
confidence: high
user_story: "En tant qu'operatrice de ShipFlow App, je veux que l'acces produit soit verifie par un entitlement serveur pour product_id=shipflow_app, afin qu'une session authentifiee, un acces GitHub ou un cache local ne puisse jamais accorder a lui seul l'acces aux donnees protegees."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
  - "ShipFlow App Flutter"
...
ShipFlow Product Entitlements Compliance
...
En tant qu'operatrice de ShipFlow App, je veux que l'acces produit soit verifie par un entitlement serveur pour `product_id=shipflow_app`, afin qu'une session authentifiee, un acces GitHub ou un cache local ne puisse jamais accorder a lui seul l'acces aux donnees protegees.

# Minimal Behavior Contract

ShipFlow App accepte une session d'identite valide, lit un snapshot d'entitlement serveur pour `product_id=shipflow_app` dans le bootstrap, et n'autorise les donnees protegees que si ce snapshot est actif ou trialing non expire, `grants_access=true`, et dans le bon environnement. Sans snapshot, avec snapshot malformed, inactive, expired, revoked, refunded, pending review, unavailable, ou avec cache seulement, l'app fail closed et affiche un etat recuperable.

# Success Behavior

- Given un bootstrap contient un snapshot `active` ou `trialing` non expire pour `shipflow_app`, with `grants_access=true` et environnement courant, when access is resolved, then `AppAccessState` devient `ready` ou `needsOnboarding`.
...
- canonical product id `shipflow_app`;
...
- Any second durable ShipFlow-local entitlement ledger.
...
- `checklist_path`: `shipflow_data/workflow/verification/shipflow-product-entitlements-compliance.md`.
...
- Product entitlement doctrine: `/home/claude/shipflow/skills/references/product-entitlements-playbook.md`.
- Suite architecture: `/home/claude/winflowz/shipflow_data/workflow/specs/unified-suite-authentication.md`.
...
- `shipflow_app` is the product namespace.
...
- `shipflow_data/technical/product-entitlement-bridge-contract.md`
- `shipflow_data/technical/product-entitlements-support-runbook.md`
- `shipflow_data/technical/firestore-data-model.md`
- `shipflow_data/technical/code-docs-map.md`
- `shipflow_data/editorial/content-map.md`
- `shipflow_data/workflow/verification/shipflow-product-entitlements-compliance.md`
...
- [x] Task 1: Create ShipFlow product entitlement contract doc.
...
- [x] AC 1: Signed-in user without `shipflow_app` entitlement is denied protected data.
...
- High product risk if ShipFlow creates a second durable ledger.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-05-30 18:53:35 UTC | sf-spec | GPT-5 Codex | Created product entitlements compliance spec from audit findings, product-entitlements playbook, local ShipFlow App scan, and suite ledger preflight. | draft saved | /sf-ready ShipFlow Product Entitlements Compliance |
| 2026-05-30 18:58:26 UTC | sf-ready | GPT-5 Codex | Reviewed structure, metadata, user-story alignment, product-entitlements doctrine, suite ledger preflight, adversarial/security readiness, language doctrine, fresh-doc gate, and proof contract. | ready | /sf-start ShipFlow Product Entitlements Compliance |
| 2026-05-30 20:44:06 UTC | sf-start | GPT-5.3 Codex Spark subagent plus GPT-5 Codex integration | Implemented local-contract-first entitlement model, access gates, route/UI states, docs, support runbook, verification checklist, and focused tests. | implemented | /sf-verify ShipFlow Product Entitlements Compliance |
