...
WEB_READER_CONTRACT_PATH = ROOT / "shipflow_data/technical/operational-record-web-reader-contract.md"
WEB_READER_FIXTURE_PATH = ROOT / "test/data/shipflow_sources/fixtures/operational_records_web_reader.md"
REQUIRED_TEST_COMMAND = [
    ["flutter", "test", "test/data/shipflow_sources/parsers/operational_record_parser_test.dart"],
]
TUI_WORKDIR = Path("/home/claude/shipflow/tui")
TUI_CHECK_COMMANDS = [
    ["bun", "test"],
    ["bun", "run", "typecheck"],
]
...
def infer_project_from_path(path: Path) -> Optional[str]:
    ...
    if "shipflow_app" in rel:
        return "shipflow_app"
    if "shipflow_data" in rel:
        return "shipflow_data"
    return None
