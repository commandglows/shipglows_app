...
linked_systems:
  ...
  - "ShipFlow App Flutter readers"
  - "ShipFlow terminal TUI"
  - "ShipFlow skills writer protocols"
evidence:
  - "CLAUDE.md"
  - "AGENT.md"
  ...
  - "/home/claude/shipflow/shipflow_data/workflow/specs/shipflow-terminal-tui-v1.md"
...
depends_on: []
supersedes: []
next_step: "/sf-spec Traffic-first Markdown operational record format for ShipFlow tasks specs and audits"
---
...
ShipFlow already has a dashboard app and a terminal TUI that read Markdown as the source of truth. The question is whether tasks, audits, and spec indexes should keep their current table-oriented formats and let readers normalize them, or whether the canonical Markdown itself should become easier to read directly by humans, TUI views, and a future web app.
...
- `CLAUDE.md` - confirms `shipflow_app` is the active ShipFlow dashboard and Markdown/repo files remain canonical.
- `AGENT.md` - identifies active source-reader surfaces, including `lib/data/shipflow_sources/**` and `tui/**`.
- `shipflow_data/technical/markdown-source-of-truth.md` - describes Markdown as the canonical operational source.
- `shipflow_data/technical/code-docs-map.md` - maps code readers to documentation responsibilities.
- `/home/claude/shipflow_data/TASKS.md` - shows global task tracking with dashboard and per-project table sections.
- `/home/claude/shipflow_data/AUDIT_LOG.md` - shows the current global audit log as a wide Markdown table.
- `shipflow_data/workflow/TASKS.md` - shows local task tracking with legends, top priority, and tables.
- `shipflow_data/workflow/AUDIT_LOG.md` - shows local audit tracking as a table with local project inference.
- `/home/claude/shipflow/shipflow_data/workflow/specs/shipflow-terminal-tui-v1.md` - shows current spec frontmatter plus structured sections and history tables.
- `lib/data/shipflow_sources/parsers/tasks_parser.dart` - parses project sections, task status markers, and top priority text.
- `lib/data/shipflow_sources/parsers/audit_log_parser.dart` - parses Markdown audit tables with expected columns.
- `lib/data/shipflow_sources/parsers/specs_parser.dart` - parses frontmatter and flow sections from specs.
- `tui/src/sources/readers.ts` - already normalizes task, audit, and spec records into TUI-friendly display lines.
- ShipFlow skill references for audit/task workflows - show that several skills write directly to `TASKS.md` and `AUDIT_LOG.md`.
...
The key design question is not just visual formatting. It is whether ShipFlow should define a canonical operational record grammar that every writer and reader can share.
...
```text
🔴 [shipflow_app] 📋 todo - Run /sf-verify for shipflow-github-managed-clone-indexer.md before closing or shipping the implementation
...
```text
🔴 [shipflow_app] 📋 todo - Run /sf-verify for shipflow-github-managed-clone-indexer.md
🟠 [ShipFlow] 🔄 in progress - Harden install.sh supply-chain and failure handling
```
...
```text
🟢 [ShipFlow] spec: ShipFlow Terminal TUI V1 | status: ready | next: /sf-verify
🟡 [shipflow_app] spec: Markdown artifact governance | status: draft | next: /sf-ready
```
...
- Cons:
  ...
  - Requires writer-skill updates across several ShipFlow workflows.
...
- User story seed: As Diane, I want ShipFlow tasks, audits, and specs to use a readable traffic-first Markdown convention so raw files, the terminal TUI, and the future web app all show the same operational signals clearly.
...
- Recommended next command: `/sf-spec Traffic-first Markdown operational record format for ShipFlow tasks specs and audits`
...
|----------|--------------|--------|--------|-----------|
| 2026-05-22 09:55:56 UTC | Explore readable Markdown conventions for tasks, audits, and specs | Read ShipFlow App context, current parsers, global/local trackers, technical docs, and related skill workflow references | Recommended traffic-first operational records with structured attributes and compatibility migration | Draft `/sf-spec` |
