...
''';
    ...
    test('parses web-reader fixture records with source metadata', () {
      ...
      expect(escaped.dedupeKey, contains('task|shipflow_app|title|escaped |title|area|infra'));

      final projected = task.firstWhere(
        (record) => record.title == 'Web reader task with filter and priority',
      );
      expect(projected.project, 'shipflow_app');
      ...
      expect(projected.dedupeKey, contains('task|shipflow_app|title|web reader task with filter and priority|area|reader'));
