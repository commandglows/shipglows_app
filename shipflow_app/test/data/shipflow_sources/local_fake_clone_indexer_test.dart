...
import 'package:shipflow_app/data/shipflow_sources/local_fake_clone_indexer.dart';
...
void main() {
  ...
  group('LocalFakeCloneIndexer', () {
    test('indexes allowed Markdown into Firestore-shaped records', () async {
      ...
      File(
        '${repo.path}/shipflow_data/technical/firestore-data-model.md',
      ).writeAsStringSync('''
---
...
''');
    ...
    test(
      ...
      () async {
        ...
        File('${repo.path}/shipflow_data/business/business.md')
          ..createSync(recursive: true)
          ..writeAsStringSync('# ok');
        File('${repo.path}/shipflow_data/editorial/content-map.md')
          ..createSync(recursive: true)
          ..writeAsStringSync('---\nartifact: [bad\n---\n# bad');
        File('${repo.path}/shipflow_data/technical/large.md')
          ..createSync(recursive: true)
          ..writeAsStringSync('# ${List.filled(80, 'x').join()}');
        File('${repo.path}/shipflow_data/workflow/TASKS.md')
          ..createSync(recursive: true)
          ..writeAsStringSync('# ${List.filled(40, 'y').join()}');

        final previous = IndexedFileRecord(
          fileId: 'old_file',
          path: 'shipflow_data/workflow/specs/deleted.md',
          artifactType: 'spec',
...
Future<Directory> _repoFixture() async {
  final repo = await Directory.systemTemp.createTemp('shipflow_fake_repo_');
  Directory('${repo.path}/shipflow_data/business').createSync(recursive: true);
  Directory('${repo.path}/shipflow_data/editorial').createSync(recursive: true);
  Directory('${repo.path}/shipflow_data/technical').createSync(recursive: true);
  Directory(
    '${repo.path}/shipflow_data/workflow/specs',
  ).createSync(recursive: true);
