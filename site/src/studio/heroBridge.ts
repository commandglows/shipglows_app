...
export interface HeroStudioBridgeRuntime {
  ...
  reset(): void;
  post(message: StudioReadyMessage | StudioSelectedMessage, targetOrigin: string): void;
}
...
export class HeroStudioBridgeController {
  readonly #runtime: HeroStudioBridgeRuntime;
  readonly #parentOrigin: string;
  readonly #journals = new Map<string, { readonly revision: number; readonly signature: string }>();
  #channelId: string | null = null;

  constructor(runtime: HeroStudioBridgeRuntime, parentOrigin: string = STUDIO_PARENT_ORIGIN) {
  ...
  receive(event: StudioHostEvent): boolean {
    if (!event.sourceIsParent || !isTrustedStudioOrigin(event.origin, this.#parentOrigin)) return false;
    const message = parseStudioHostMessage(event.data);
    if (!message) return false;

    if (message.type === "studio.attach") {
      if (this.#channelId !== null && this.#channelId !== message.channelId) return false;
      const anchors = this.#runtime.snapshots();
      const expectedIds = Object.keys(HERO_STUDIO_ANCHORS);
      if (anchors.length !== expectedIds.length || anchors.some((anchor) => !expectedIds.includes(anchor.id))) return false;
      this.#channelId = message.channelId;
      this.#runtime.post(createStudioReadyMessage(message.channelId, anchors), this.#parentOrigin);
      return true;
    }

    if (message.channelId !== this.#channelId) return false;
    if (message.type === "studio.select") return this.#selectAndSend(message.anchorId);

    const signature = JSON.stringify(message.commands);
    const previous = this.#journals.get(message.channelId);
    if (previous !== undefined) {
      if (message.revision < previous.revision) return false;
      if (message.revision === previous.revision) return signature === previous.signature;
    }
    this.#runtime.reset();
    for (const command of message.commands) {
      if (!this.#runtime.apply(command)) {
        this.#runtime.reset();
        return false;
      }
    }
    this.#journals.set(message.channelId, { revision: message.revision, signature });
    return true;
  }

  activate(anchorId: HeroStudioAnchorId): boolean {
    return this.attached && this.#selectAndSend(anchorId);
  }

  #selectAndSend(anchorId: HeroStudioAnchorId): boolean {
    const anchor = this.#runtime.select(anchorId);
    return anchor ? this.#sendSelected(anchor) : false;
  }

  #sendSelected(anchor: StudioAnchorSnapshot): boolean {
    if (!this.#channelId) return false;
    this.#runtime.post(createStudioSelectedMessage(this.#channelId, anchor), this.#parentOrigin);
    return true;
  }
}

const fixedAnchorSelector = "[data-sg-studio-anchor]";
...
const bridgeStyles = `
[${selectedAttribute}="true"] { outline: 3px solid #0f766e !important; outline-offset: 4px !important; }
...
[${overlayAttribute}] { position: fixed; z-index: 2147483647; pointer-events: none; padding: 4px 8px; border-radius: 999px; color: #fff; background: #0f766e; font: 600 12px/1.4 system-ui, sans-serif; box-shadow: 0 4px 18px rgba(0,0,0,.22); }
...
export class BrowserHeroStudioRuntime implements HeroStudioBridgeRuntime {
  ...
  snapshots(): readonly StudioAnchorSnapshot[] {
    ...
    if (elements.size !== ids.length || ids.some((id) => !elements.has(id))) return [];
  ...
  apply(command: StudioPreviewCommand): StudioAnchorSnapshot | null {
    ...
    if (!element) return null;
    const plan = previewOverridePlan(command);
    for (const mutation of plan.mutations) this.#setStyle(element, mutation.property, mutation.value);
  ...
  post(message: StudioReadyMessage | StudioSelectedMessage, targetOrigin: string): void {
    if (this.#window.parent !== this.#window) this.#window.parent.postMessage(message, targetOrigin);
  }
  ...
  dispose(): void {
    ...
    for (const element of this.#document.querySelectorAll<HTMLElement>(fixedAnchorSelector)) {
      const id = element.dataset.sgStudioAnchor;
      if (!id || !(id in HERO_STUDIO_ANCHORS) || result.has(id as HeroStudioAnchorId)) return new Map();
      result.set(id as HeroStudioAnchorId, element);
    }
    return result;
  }

  #snapshot(id: HeroStudioAnchorId, element: HTMLElement): StudioAnchorSnapshot {
  ...
  #setStyle(element: HTMLElement, property: string, value: string): void {
    ...
    if (!properties.has(property)) properties.set(property, { value: element.style.getPropertyValue(property), priority: element.style.getPropertyPriority(property) });
...
export function installHeroStudioBridge(options: {
  readonly parentOrigin?: string;
  ...
  if (targetWindow.parent === targetWindow || parentOrigin !== STUDIO_PARENT_ORIGIN) return () => undefined;

  const runtime = new BrowserHeroStudioRuntime(targetWindow, targetDocument);
  const controller = new HeroStudioBridgeController(runtime, parentOrigin);
  const onMessage = (event: MessageEvent) => controller.receive({ origin: event.origin, sourceIsParent: event.source === targetWindow.parent, data: event.data });
  const onClick = (event: MouseEvent) => {
    if (!(event.target instanceof Element)) return;
    const anchored = event.target.closest<HTMLElement>(fixedAnchorSelector);
    const anchorId = anchored?.dataset.sgStudioAnchor as HeroStudioAnchorId | undefined;
    if (anchorId && anchorId in HERO_STUDIO_ANCHORS && controller.activate(anchorId)) {
      event.preventDefault();
      event.stopPropagation();
      return;
    }
    if (controller.attached && event.target.closest("a[href], [formaction]")) {
      event.preventDefault();
      event.stopPropagation();
    }
  };
  const onSubmit = (event: SubmitEvent) => {
    if (controller.attached) event.preventDefault();
  };
