...
depends_on:
  - "docs/explorations/2026-05-08-legacy-contentflow-shipflow-inventory.md@1.0.0"
...
Operational move/archive/delete status is tracked in `docs/technical/legacy-file-migration-tracker.md`.
...
- `web_auth/**`
- `specs/*.md`
- legacy sections in `README.md`, `CLAUDE.md`, `AGENT.md`, and `TASKS.md`
- `docs/technical/legacy-file-migration-tracker.md`
...
| `specs/*contentflow*`, FastAPI specs | Legacy specs | classify | medium | Mark active, reference, archive, or superseded one by one |
| `README.md`, `CLAUDE.md`, `AGENT.md`, `TASKS.md` | Mixed product guidance | rewrite | high | Make ShipFlow active and preserve legacy decisions as archive |

## Spec Classification

| Spec | Current origin | ShipFlow decision | Risk | Next action |
| --- | --- | --- | --- | --- |
| `specs/shipflow-legacy-contentflow-fusion.md` | ShipFlow | active | high | Use as current migration/fusion chantier |
| `specs/SPEC-offline-sync-v2.md` | ContentFlow | reference-only | medium | Preserve concepts for future sync/projection spec |
| `specs/architecture-cible-fastapi-clerk-flutter.md` | ContentFlow | reference-only | high | Do not treat FastAPI or Clerk as active ShipFlow decisions |
| `specs/feedback-admin-v1-contentflow.md` | ContentFlow | keep-concept | medium | Mine later for text-only feedback spec |
| `specs/feedback-backend-contract-fastapi.md` | ContentFlow | reference-only | high | Do not adopt FastAPI feedback backend without new spec |
| `specs/SPEC-content-pipeline-unification.md` | ContentFlow | park | medium | User asked not to touch pipeline for now |
| `specs/SPEC-project-flows-selection-onboarding-archive.md` | ContentFlow | adapt-candidate | medium | Review later for ShipFlow project/source onboarding |
| `specs/spec-no-ui-jump-on-resume.md` | ContentFlow | adapt-candidate | medium | Keep as UX reliability reference |
| `specs/foundation-scrollable-nav-affiliations.md` | ContentFlow | park | low | ContentFlow UI/domain reference only |
| `specs/late-integration-finalization.md` | ContentFlow | reference-only | medium | Review only if integrations become active ShipFlow scope |
| `specs/migrate-flutter-core-majors-baseline.md` | Flutter maintenance | keep-concept | medium | Useful for dependency/runtime maintenance |
| `specs/SPEC-migrate-flutter-core-majors.md` | Flutter maintenance | keep-concept | medium | Useful for dependency/runtime maintenance |
| `specs/PRD-lifetime-deal-early-bird-payg.md` | ContentFlow business | archive-later | low | Not active ShipFlow product scope |
...
rg -n "Clerk|OpenRouter|FastAPI|Supabase|Firebase|Firestore|feedback|pipeline" lib docs specs README.md
```
...
Every legacy cleanup PR or chantier must update this inventory and `docs/technical/legacy-file-migration-tracker.md` before moving or removing files.
