...
linked_systems:
  ...
  - "specs/shipflow-legacy-file-migration-tracker.md"
  - "lib/"
  - "web_auth/"
  - "specs/"
depends_on:
  - "specs/shipflow-legacy-file-migration-tracker.md@0.1.0"
  - "docs/technical/legacy-contentflow-inventory.md@0.1.0"
  - "specs/shipflow-legacy-contentflow-fusion.md@0.4.0"
supersedes: []
evidence:
...
- `future specs/legacy/contentflow/...` is the default future target for legacy specs if a cleanup spec approves movement.
- `future docs/legacy/contentflow/...` is the default future target for legacy docs or root-guide excerpts if a cleanup spec approves movement.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `lib/shipflow/` | Active ShipFlow dashboard runtime | keep | `active-keep` | `stay-current` | `CONTENT_MAP.md`; `runtime-boundary.md` | low | `rg -n "ShipFlowApp|shipflow" lib/shipflow lib/main.dart` | Protect from legacy cleanup |
...
| `lib/domain/project_health/` | Active project health logic | keep | `active-keep` | `stay-current` | `markdown-source-of-truth.md`; `CONTENT_MAP.md` | medium | `flutter test test/domain/project_health` | Protect active domain layer |
...
| `lib/core/openrouter_guard.dart` | BYOK/OpenRouter guard | keep concept | `blocked-needs-spec` | `stay-current` until BYOK security spec | `legacy-contentflow-inventory.md`; `shipflow-legacy-reuse-roadmap.md` | high | `rg -n "OpenRouter|openrouter|BYOK" lib test docs specs` | No activation or deletion before BYOK spec |
...
| `lib/data/models/project.dart` | Legacy project model | adapt candidate | `adapt-candidate` | future ShipFlow project model by owner spec | `legacy-contentflow-inventory.md`; foundational project specs | medium | `rg -n "class Project|data/models/project" lib test` | Compare against GitHub repo project model |
...
| `lib/data/services/api_service.dart` | Legacy FastAPI facade | park | `blocked-needs-spec` | `stay-current`; future `legacy/contentflow/data/services/api_service.dart` only after cleanup spec | `legacy-contentflow-inventory.md` | high | `rg -n "ApiService|FastAPI|api_service" lib test docs specs` | Do not adopt as backend default |
| `lib/data/services/clerk_auth_service*` | Legacy Clerk auth | reference only | `blocked-needs-spec` | `stay-current`; future `legacy/contentflow/data/services/` only after auth decision | `legacy-contentflow-inventory.md` | high | `rg -n "Clerk|clerk_auth_service" lib web_auth test docs specs` | Archive only after auth provider decision |
...
| `web_auth/` | Legacy Clerk web auth pages | reference only | `blocked-needs-spec` | `stay-current`; future `legacy/contentflow/web_auth/` after auth provider decision | `legacy-contentflow-inventory.md` | high | `rg -n "CLERK|sign-in|sso-callback|web_auth" web_auth scripts README.md docs specs` | Archive only after auth provider is chosen |
| `README.md`, `CLAUDE.md`, `AGENT.md`, `TASKS.md` legacy sections | Mixed product guidance | rewrite/preserve archive decisions | `needs-decision` | `stay-current`; future excerpts under `docs/legacy/contentflow/` only after docs cleanup spec | `legacy-contentflow-inventory.md` | high | `rg -n "ContentFlow|contentflow|contentflow_app" README.md CLAUDE.md AGENT.md TASKS.md` | Keep ShipFlow active, preserve useful legacy decisions |

## Legacy Spec Rows

| Current path | Current role | Decision | Migration status | Target path | Decision source | Risk | Validation | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `specs/shipflow-legacy-contentflow-fusion.md` | Active ShipFlow migration/fusion chantier | active | `active-keep` | `stay-current` | self; `legacy-contentflow-inventory.md` | high | `rg -n "ShipFlow Legacy ContentFlow Fusion|Current Chantier Flow" specs/shipflow-legacy-contentflow-fusion.md` | Keep as active parent chantier |
| `specs/SPEC-offline-sync-v2.md` | ContentFlow offline/sync reference | reference only | `reference-only` | `stay-current`; future `specs/legacy/contentflow/SPEC-offline-sync-v2.md` after cleanup spec | `legacy-contentflow-inventory.md` | medium | `rg -n "offline|sync" specs/SPEC-offline-sync-v2.md` | Preserve concepts for future sync/projection spec |
| `specs/architecture-cible-fastapi-clerk-flutter.md` | ContentFlow FastAPI/Clerk architecture | reference only | `blocked-needs-spec` | `stay-current`; future `specs/legacy/contentflow/architecture-cible-fastapi-clerk-flutter.md` after auth/backend decisions | `legacy-contentflow-inventory.md` | high | `rg -n "FastAPI|Clerk|Flutter" specs/architecture-cible-fastapi-clerk-flutter.md` | Do not treat as ShipFlow architecture |
| `specs/feedback-admin-v1-contentflow.md` | ContentFlow feedback admin idea | keep concept | `blocked-needs-spec` | `stay-current`; future owner spec decides reuse/archive | `legacy-contentflow-inventory.md` | medium | `rg -n "feedback|admin" specs/feedback-admin-v1-contentflow.md` | Mine later for text feedback spec |
| `specs/feedback-backend-contract-fastapi.md` | FastAPI feedback backend contract | reference only | `blocked-needs-spec` | `stay-current`; future `specs/legacy/contentflow/feedback-backend-contract-fastapi.md` after feedback/backend decisions | `legacy-contentflow-inventory.md` | high | `rg -n "FastAPI|feedback|backend" specs/feedback-backend-contract-fastapi.md` | Do not adopt FastAPI backend without new spec |
| `specs/SPEC-content-pipeline-unification.md` | ContentFlow pipeline spec | park | `park` | `stay-current`; future `specs/legacy/contentflow/SPEC-content-pipeline-unification.md` if cleanup spec approves | `legacy-contentflow-inventory.md` | medium | `rg -n "pipeline|content" specs/SPEC-content-pipeline-unification.md` | Keep parked; do not touch pipeline now |
| `specs/SPEC-project-flows-selection-onboarding-archive.md` | ContentFlow project flow reference | adapt candidate | `adapt-candidate` | `stay-current` until project/onboarding owner spec supersedes or archives | `legacy-contentflow-inventory.md` | medium | `rg -n "project|onboarding|archive" specs/SPEC-project-flows-selection-onboarding-archive.md` | Review against ShipFlow onboarding specs |
| `specs/spec-no-ui-jump-on-resume.md` | UX reliability reference | adapt candidate | `adapt-candidate` | `stay-current` until UX reliability owner spec supersedes or archives | `legacy-contentflow-inventory.md` | medium | `rg -n "resume|jump|route" specs/spec-no-ui-jump-on-resume.md` | Keep as UX reliability reference |
| `specs/foundation-scrollable-nav-affiliations.md` | ContentFlow UI/domain reference | park | `park` | `stay-current`; future `specs/legacy/contentflow/foundation-scrollable-nav-affiliations.md` if cleanup spec approves | `legacy-contentflow-inventory.md` | low | `rg -n "Affiliations|Navigation|ContentFlow" specs/foundation-scrollable-nav-affiliations.md` | Keep parked |
| `specs/late-integration-finalization.md` | Legacy integration spec | reference only | `reference-only` | `stay-current`; future `specs/legacy/contentflow/late-integration-finalization.md` after integrations decision | `legacy-contentflow-inventory.md` | medium | `rg -n "integration|LATE|Zernio" specs/late-integration-finalization.md` | Review only if integrations become active |
| `specs/migrate-flutter-core-majors-baseline.md` | Flutter maintenance baseline | keep concept | `keep-concept` | `stay-current` | `legacy-contentflow-inventory.md` | medium | `rg -n "Flutter|dependencies|baseline" specs/migrate-flutter-core-majors-baseline.md` | Keep for maintenance history |
| `specs/SPEC-migrate-flutter-core-majors.md` | Flutter maintenance chantier | keep concept | `keep-concept` | `stay-current` | `legacy-contentflow-inventory.md` | medium | `rg -n "Flutter|Riverpod|migration" specs/SPEC-migrate-flutter-core-majors.md` | Keep for dependency/runtime maintenance |
| `specs/PRD-lifetime-deal-early-bird-payg.md` | ContentFlow business PRD | archive later | `archive-later` | `stay-current`; future `specs/legacy/contentflow/PRD-lifetime-deal-early-bird-payg.md` after business archive decision | `legacy-contentflow-inventory.md` | low | `rg -n "ContentFlow|Lifetime|BYOK" specs/PRD-lifetime-deal-early-bird-payg.md` | Not active ShipFlow product scope |
...
```bash
rg -n "Current path|Target path|Migration status|Decision source|Validation|blocked-needs-spec" docs/technical/legacy-file-migration-tracker.md
rg -n "lib/router.dart|lib/providers/providers.dart|web_auth|feedback|OpenRouter|pipeline|FastAPI|SPEC-offline-sync-v2" docs/technical/legacy-file-migration-tracker.md
rg -n "legacy-file-migration-tracker" CONTENT_MAP.md docs/technical/code-docs-map.md docs/technical/legacy-contentflow-inventory.md specs/shipflow-legacy-contentflow-fusion.md
rg --files lib web_auth specs docs | rg "router.dart|providers.dart|web_auth|feedback|openrouter|clerk|drip|SPEC-offline-sync-v2|PRD-lifetime"
git diff --name-status -- docs/technical/legacy-file-migration-tracker.md docs/technical/legacy-contentflow-inventory.md docs/technical/code-docs-map.md CONTENT_MAP.md specs/shipflow-legacy-contentflow-fusion.md specs/shipflow-legacy-file-migration-tracker.md
```
...
Every future legacy cleanup PR or chantier must update this tracker before moving, archiving, or deleting files. If a cleanup changes runtime ownership, update `docs/technical/runtime-boundary.md`, `docs/technical/code-docs-map.md`, and `CONTENT_MAP.md` in the same chantier.
