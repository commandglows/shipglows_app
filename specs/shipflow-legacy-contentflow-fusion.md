...
depends_on:
  ...
  - artifact: "docs/explorations/2026-05-08-legacy-contentflow-shipflow-inventory.md"
  ...
  - artifact: "TASKS.md"
    artifact_version: "unknown"
    required_status: "unknown"
  - artifact: "docs/technical/legacy-file-migration-tracker.md"
    artifact_version: "0.1.0"
...
- Etant donne les decisions produit, les docs indiquent que les fichiers Markdown du repo utilisateur sont source de verite et que toute base de donnees future est une projection/index/sync.
...
- Si une documentation externe recente devient necessaire pour Firebase, Firestore, FastAPI, OpenRouter, Clerk ou un SDK auth, la spec de cette integration doit consulter les docs officielles avant implementation.

# Problem

Le repo contient un melange entre ShipFlow et un heritage ContentFlow. La branche distante semble etre la version saine, tandis que le local contient un WIP Supabase archive et des idees non fiables comme base de merge. Les docs et une partie du code parlent encore de ContentFlow, Clerk, FastAPI, OpenRouter, feedback, pipeline et autres briques qui ne sont pas toutes actives pour ShipFlow. Supprimer trop vite ferait perdre des idees utiles; garder tout sans frontiere rend le projet illisible.

# Solution

Creer une frontiere documentaire et technique entre ShipFlow actif et ContentFlow legacy, puis faire une migration/fusion par classification. Le chantier commence par les docs de gouvernance et l'inventaire, preserve le runtime ShipFlow par defaut, garde le legacy temporairement, et reporte les choix de stack lourds dans des specs separees.
...
- Classer les zones legacy: auth, feedback, BYOK/OpenRouter, pipeline, API service, providers, models, web auth, specs, tasks, docs.
...
- Produire une liste de decisions restantes utilisable pour des specs futures.
...
- Documentation projet actuelle: `README.md`, `CLAUDE.md`, `AGENT.md`, `TASKS.md`, `docs/`, `specs/`.
- Exploration source: `docs/explorations/2026-05-08-legacy-contentflow-shipflow-inventory.md`.
- Fresh external docs: not needed for this chantier because it does not implement Firebase, Firestore, FastAPI, OpenRouter, Clerk, or Supabase behavior. Future stack specs must check official docs at that time.
...
- Les docs ne doivent pas presenter ContentFlow comme produit actif.
...
- `TASKS.md` doit distinguer backlog ShipFlow, archives ContentFlow, et decisions en attente.
- `docs/technical/` doit exister ou etre cree pour les decisions d'architecture.
- `docs/technical/code-docs-map.md` et `CONTENT_MAP.md` doivent etre crees ou audites avant une execution de code large.
- Les anciennes specs ContentFlow doivent etre classees, pas melangees aux specs actives ShipFlow.
- `docs/technical/legacy-file-migration-tracker.md` doit suivre tout futur mouvement, archivage ou suppression de fichier legacy avant mutation.
...
1. Creer `docs/technical/legacy-contentflow-inventory.md` depuis `docs/explorations/2026-05-08-legacy-contentflow-shipflow-inventory.md`, avec une table `path`, `current role`, `ShipFlow decision`, `risk`, `next action`.
2. Creer `docs/technical/markdown-source-of-truth.md` pour formaliser: Markdown/repo utilisateur canonique, base de donnees projection, ecriture applicative par modification Markdown, gestion des conflits par Git/review.
3. Creer `docs/technical/runtime-boundary.md` pour documenter `APP_TARGET=shipflow`, `APP_TARGET=legacy/contentflow`, et les conditions de retrait futur du legacy.
4. Creer ou mettre a jour `docs/technical/code-docs-map.md` pour lier les modules actifs ShipFlow, les modules legacy, les specs actives, et les docs techniques.
5. Creer ou mettre a jour `CONTENT_MAP.md` pour distinguer contenu produit actif, docs techniques, specs, archives, et exploration.
...
8. Mettre a jour `TASKS.md` en conservant les informations legacy utiles sous une section archive ou decision, puis en mettant le backlog ShipFlow migration/fusion en tete.
...
10. Classer les anciennes specs `specs/*.md` dans `docs/technical/legacy-contentflow-inventory.md` sans les deplacer tant qu'une decision d'archive n'est pas explicite.
10b. Creer et maintenir `docs/technical/legacy-file-migration-tracker.md` comme gate operationnelle avant tout deplacement, archivage ou suppression legacy.
...
- Une personne ou un agent frais comprend depuis les docs racine que ShipFlow est le produit actif.
...
- `rg -n "ContentFlow|contentflow|contentflow_app" README.md CLAUDE.md AGENT.md TASKS.md docs specs lib test` pour distinguer mentions actives, legacy et archives.
...
- Verification manuelle des docs modifiees: elles doivent etre coherentes sans lire l'historique de conversation.

# Risks

- Risque haut de suppression prematuree d'une idee reutilisable.
- Risque moyen de docs qui presentent une decision stack comme acquise alors qu'elle est ouverte.
...
- No safe `Execution Batches` are declared because several files racine and docs overlap.
- This chantier is not a stack migration. It prepares later specs for Firebase/Firestore/Auth/FastAPI/BYOK/feedback.
...
- Any legacy move/archive/delete must cite `docs/technical/legacy-file-migration-tracker.md` and preserve the non-destructive gate unless a later cleanup spec explicitly authorizes mutation.
...
| 2026-05-08 20:16:43 UTC | sf-docs | GPT-5 Codex | created | Bootstrapped `docs/technical/`, refreshed `CONTENT_MAP.md`, and updated root guidance. |
...
| 2026-05-09 11:39:42 UTC | sf-docs | GPT-5 Codex | partial | Added `docs/technical/shipflow-legacy-reuse-roadmap.md` with recommended reuse order and decision questions. |
...
| 2026-05-10 10:36:30 UTC | sf-start | GPT-5 Codex | partial | Created `docs/technical/legacy-file-migration-tracker.md` and linked it as the non-destructive gate before future legacy file movement. |

# Current Chantier Flow

| Step | Status | Notes |
| --- | --- | --- |
| sf-spec | done | Spec created in `specs/shipflow-legacy-contentflow-fusion.md`. |
| sf-ready | ready | Ready for first safe migration/fusion slice; future stack choices remain scoped out. |
| governance corpus gate | created | `docs/technical/`, `docs/technical/code-docs-map.md`, and `CONTENT_MAP.md` now exist for ShipFlow. |
