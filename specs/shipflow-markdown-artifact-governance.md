...
depends_on:
  ...
  - artifact: "/home/claude/shipflow/specs/shipflow-governance-corpus-bootstrap-and-sf-build-integration.md"
    artifact_version: "1.0.0"
    required_status: "ready"
  - artifact: "/home/claude/shipflow/docs/technical/artifact-metadata-and-linter.md"
...
evidence:
  ...
  - "Repo evidence 2026-05-10: sf-docs context prefers `shipflow_data/business/*`, `shipflow_data/editorial/*`, and `shipflow_data/technical/*` before root legacy docs."
...
Draft foundational spec. This spec defines what Markdown artifacts ShipFlow indexes as governance data and how `shipflow_data/` becomes the canonical per-project corpus. It must be reviewed with the onboarding, Firestore, auth/access, and runner/indexer specs before implementation.
...
- Given both `shipflow_data/` and root `BUSINESS.md`, `CONTENT_MAP.md`, `docs/technical/*`, or similar files exist, `shipflow_data/` is canonical and root files are not allowed to override it.
...
- Given tracker files like `TASKS.md`, `AUDIT_LOG.md`, `PROJECTS.md`, `TEST_LOG.md`, or `BUGS.md`, the indexer reads them as trackers without requiring ShipFlow frontmatter.
...
- Fresh external docs: not needed. This is local ShipFlow corpus doctrine and Markdown parsing policy.
...
- Legacy migration: existing root docs remain useful evidence but should gradually move into `shipflow_data/`.
...
- Update `docs/technical/markdown-source-of-truth.md` to name `shipflow_data/` as the preferred governance corpus path.
...
1. Create `docs/technical/markdown-artifact-governance.md` with corpus root, path allowlist, ignore rules, artifact families, and examples.
2. Update `docs/technical/markdown-source-of-truth.md` to prefer `shipflow_data/`.
...
6. Define tracker parsing contract for `TASKS.md`, `AUDIT_LOG.md`, `PROJECTS.md`, `TEST_LOG.md`, and `BUGS.md`.
...
10. Update docs maps and foundational references.
...
- Root-level governance docs are fallback/legacy when `shipflow_data/` exists.
...
- Medium migration risk because existing projects may use root docs or different casing.
...
- Technical decision: root files like `BUSINESS.md`, `CONTENT_MAP.md`, `docs/technical/*`, and `specs/*.md` remain migration fallback for older projects.
...
- Fresh-docs verdict: not needed; this spec depends on local ShipFlow doctrine and repository evidence.
