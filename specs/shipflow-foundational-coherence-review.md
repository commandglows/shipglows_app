...
depends_on:
  ...
  - artifact: "docs/technical/foundational-specs-handoff.md"
    artifact_version: "0.1.0"
    required_status: "draft"
  ...
  - artifact: "specs/shipflow-markdown-artifact-governance.md"
    artifact_version: "0.1.0"
    required_status: "draft"
  - artifact: "specs/shipflow-dashboard-readonly-projection.md"
    artifact_version: "0.1.0"
    required_status: "draft"
  ...
  - artifact: "https://firebase.google.com/docs/firestore/security/rules-conditions"
    artifact_version: "checked-2026-05-10"
    required_status: "active"
  - artifact: "https://firebase.google.com/docs/functions"
    artifact_version: "checked-2026-05-10"
    required_status: "active"
...
- Proof of success: `sf-ready` checklist can be answered without ambiguity for all required gates (behavior, ordering, security, docs coherence, language doctrine, freshness gate) and outputs a single next command.
...
The repository has a complete foundational corpus but the contracts are spread across multiple draft specs and a handoff doc. A fresh implementation context can still choose inconsistent status names, permission checks, query boundaries, and error semantics. Without a single coherence spec, `/sf-ready` can pass individual docs while cross-spec contradictions remain.
...
- Define required state vocabulary and mapping rules between specs.
...
- Must remain compatible with `docs/technical/shipflow-foundational-architecture.md` decisions.
...
- Fresh external docs verdict: `fresh-docs checked` via official Firebase and GitHub docs already referenced by foundational specs; coherence spec must fail readiness if those dependencies become stale or contradictory.
...
- Upstream dependencies:
  - Auth/session semantics from Firebase Auth and GitHub App docs.
...
| --- | --- | --- | --- | --- |
| Project identity | One ShipFlow project equals exactly one GitHub repository, keyed by opaque `projectId`; GitHub owner/name/repository ID are metadata for deduplication and access checks. | `specs/shipflow-firestore-data-model.md` | Auth, onboarding, indexer, dashboard | Any spec uses `owner/repo` as durable Firestore document ID or creates duplicate projects for the same repo. |
| Identity versus repository access | Firebase Auth identifies the ShipFlow user; GitHub App installation/access proves repository readability. These are separate and both are required for sensitive project actions. | `specs/shipflow-auth-github-access.md` | Onboarding, Firestore rules, dashboard | Any spec treats Firebase login alone as proof of repo access or makes GitHub login mandatory for Firebase identity. |
| Server-side authorization | Repo listing, create-or-join, refresh, clone, indexing, projection writes, and access resync are initiated through trusted backend checks. Client UI can request, not authorize. | `specs/shipflow-auth-github-access.md` | Indexer, onboarding, Firestore model | Any sensitive operation can be completed with UI-only checks or client-supplied repo metadata. |
| Read-only V1 boundary | V1 may read repository content and write app/projection state; it must not write Markdown, commit, push, open PRs, run terminal sessions, or execute agents. | `docs/technical/shipflow-foundational-architecture.md` | Indexer, dashboard, onboarding | Any foundational spec adds repo write-back, terminal, or agent execution behavior. |
| Projection authority | Firestore stores app state and projection, never canonical repository content. GitHub/clone content wins conflicts. | `specs/shipflow-firestore-data-model.md` | Indexer, dashboard, Markdown governance | Any spec lets Firestore override repo Markdown without a future write-back spec. |
| Managed clone | Clone materialization is mandatory for reliable indexing, infrastructure-owned, and hidden from user configuration. | `specs/shipflow-github-managed-clone-indexer.md` | Onboarding, dashboard diagnostics | Any spec asks the user to choose clone paths or exposes clone paths as user-managed settings. |
| Governance corpus | `shipflow_data/` is the preferred project-local governance corpus; root-level docs are fallback or legacy evidence during migration and cannot override `shipflow_data/`. | `specs/shipflow-markdown-artifact-governance.md` | Indexer, dashboard projection | Any spec indexes arbitrary repository Markdown or lets stale root docs override `shipflow_data/`. |
| Dashboard read model | Dashboard reads user-scoped Firestore projection, project refs, diagnostics, and stale/access warnings without triggering repo writes. | `specs/shipflow-dashboard-readonly-projection.md` | Firestore model, auth/access, indexer | Any dashboard query can scan globally, cross tenants, mutate canonical content, or hide access-loss warnings. |
| Access loss | Existing projection can remain visible/searchable with clear warning; refresh, clone, indexing, and access-sensitive writes are blocked until GitHub access is restored. | `specs/shipflow-auth-github-access.md` | Dashboard, indexer, Firestore model | Any spec deletes shared project data immediately or keeps refreshing after access is lost. |
| Idempotent create-or-join | Concurrent or repeated onboarding for the same GitHub repo resolves to one shared project with safe membership updates. | `specs/shipflow-project-onboarding-flow.md` | Firestore model, auth/access | Duplicate requests can create duplicate projects, widen permissions, or overwrite membership incorrectly. |
...
| `archived` | Shared project is no longer active for normal indexing/visibility, usually after orphaning or explicit later cleanup policy. | Trusted backend/admin policy | Dashboard filters, cleanup docs | Immediate destructive repository or projection deletion. |
...
| --- | --- | --- | --- |
| Authentication | Every client request that touches user/project state must bind to Firebase Auth identity. | `specs/shipflow-auth-github-access.md` | Rules/functions examples reject unauthenticated requests. |
| Authorization | Repo-sensitive actions revalidate GitHub App installation access server-side at action time. | `specs/shipflow-auth-github-access.md` | No create, refresh, clone, or index action relies only on cached client claims. |
| Tenant boundary | Client reads are scoped through user membership/project refs; queries must be compatible with Firestore rules because rules are not filters. | `specs/shipflow-firestore-data-model.md` | No dashboard/global collection query can depend on rules to trim inaccessible rows. |
| Backend trust | Cloud Functions or equivalent trusted runner owns token exchange, repo listing, clone/index, and projection writes. | `specs/shipflow-github-managed-clone-indexer.md` | Flutter/client never receives GitHub App installation tokens or clone credentials. |
| Token handling | GitHub installation tokens are short-lived, generated server-side, never stored in client-readable Firestore, and never logged. | `specs/shipflow-auth-github-access.md` | Token storage/logging is explicitly prohibited. |
...
- [x] Task 1: Create coherence contract matrix section in this spec.
  - Fichier : `specs/shipflow-foundational-coherence-review.md`
  ...
  - Depends on : None.
  - Validate with : `rg -n "contract matrix|state mapping|authority" specs/shipflow-foundational-coherence-review.md`.
...
- [x] Task 2: Normalize cross-spec state vocabulary.
  ...
  - Depends on : Task 1.
  - Validate with : `rg -n "access_lost|stale|indexing|archived|state vocabulary" specs/shipflow-foundational-coherence-review.md`.
...
- [x] Task 3: Define mandatory security control checklist.
  ...
  - Depends on : Task 1.
  - Validate with : `rg -n "Authentication|Authorization|Multi-tenant|rate|replay|idempot" specs/shipflow-foundational-coherence-review.md`.
...
- [x] Task 4: Define coherence acceptance criteria and failure gates.
  ...
  - Validate with : `rg -n "CA [0-9]+" specs/shipflow-foundational-coherence-review.md`.
  - Notes : Include at least one misuse/bypass criterion.

- [x] Task 5: Update foundational reference docs to point to this canonical gate.
  - Fichier : `docs/technical/foundational-specs-handoff.md`, `docs/technical/shipflow-foundational-architecture.md`, `docs/technical/code-docs-map.md`
  ...
  - Depends on : Task 4.
  - Validate with : `rg -n "shipflow-foundational-coherence-review.md|Foundational Coherence" docs/technical/foundational-specs-handoff.md docs/technical/shipflow-foundational-architecture.md docs/technical/code-docs-map.md`.
...
- [ ] CA 7 : Given documentation pointers are reviewed, when starting from architecture or handoff docs, then this canonical coherence spec is discoverable as the mandatory readiness gate.

# Test Strategy

- Spec consistency checks:
  - `rg -n "# (Title|Status|User Story|Minimal Behavior Contract|Success Behavior|Error Behavior|Problem|Solution|Scope In|Scope Out|Constraints|Dependencies|Invariants|Links & Consequences|Documentation Coherence|Edge Cases|Implementation Tasks|Acceptance Criteria|Test Strategy|Risks|Execution Notes|Open Questions|Skill Run History|Current Chantier Flow)" specs/shipflow-foundational-coherence-review.md`
- Cross-reference checks:
  - `rg -n "ShipFlow Foundational Coherence Review|shipflow-foundational-coherence-review.md" docs/technical specs`
- Invariant drift checks across foundational specs:
  - `rg -n "projectId|GitHub wins|read-only|access-lost|stale|shipflow_data" specs/shipflow-*.md docs/technical/shipflow-foundational-architecture.md`
...
- High: tenant boundary leak if project membership and query filters diverge across specs.
- Medium: operational confusion if state vocabulary stays inconsistent.
- Medium: docs drift if architecture/handoff/map references are not updated with this canonical gate.
...
- Explicit constraints:
  ...
  - Do not mark foundational implementation specs `ready` by convenience; enforce evidence.
  - Keep machine-stable headings/anchors in English.
- Validation commands:
  - `rg -n "coherence|state|invariant|access_lost|stale" specs/shipflow-foundational-coherence-review.md specs/shipflow-*.md docs/technical/*.md`
...
| Step | Status | Notes |
|------|--------|-------|
| sf-spec | done | Canonical coherence spec created in `specs/shipflow-foundational-coherence-review.md`. |
