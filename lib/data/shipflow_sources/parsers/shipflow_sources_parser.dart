...
class ParsedShipFlowData {
  const ParsedShipFlowData({
...
class ShipFlowSourcesParser {
  ShipFlowSourcesParser({
  ...
  ParsedShipFlowData parse(SourceSnapshot snapshot) {
    ...
    final auditDoc = documents.firstWhere(
      (doc) =>
          ...
          doc.path.contains('/shipflow_data/'),
      orElse: () =>
          const SourceDocument(path: '', redactedPath: '', content: ''),
    );
