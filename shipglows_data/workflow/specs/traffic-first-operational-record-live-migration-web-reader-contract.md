...
owner: "Diane"
confidence: medium
user_story: "En tant que fondatrice de ShipGlows, je veux que la migration live des tâches, audits et specs vers le format traffic-first soit validée par le contrat de lecture de la future app web, afin que les fichiers Markdown restent la source lisible et exploitable sans normalisation cachée."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
  ...
  - "test/data/shipglows_sources/parsers/"
  - "/home/claude/shipglowz/tui/src/sources/"
  - "future ShipGlows web reader"
...
next_step: "/sf-prod shipglows_app"
---

# Spec: Traffic-First Operational Record Live Migration And Web Reader Contract


🟢 [shipglows_app] spec: Traffic-First Operational Record Live Migration And Web Reader Contract | status: ready | path: shipglows_data/workflow/specs/traffic-first-operational-record-live-migration-web-reader-contract.md | next: /sf-prod shipglows_app
## Title
...
- `shipglows-dashboard-readonly-projection.md` defines the dashboard as read-only projection consumer.
- `shipglows-firestore-data-model.md` defines projection vocabulary for source commit, indexed files, diagnostics, and user-scoped dashboard feeds.
...
- Create `shipglows_data/technical/operational-record-web-reader-contract.md`.
- Update `shipglows_data/technical/markdown-source-of-truth.md` to point to the live migration and web-reader contract once ready.
- Update `shipglows_data/technical/code-docs-map.md` with the web-reader contract and write-mode validation surface.
...
- [x] Task 1: Create the web-reader contract doc.
  - File: `shipglows_data/technical/operational-record-web-reader-contract.md`
  ...
  - Validate with: `rg -n "sourcePath|dedupeKey|traffic|project|diagnostics|projection|web reader" shipglows_data/technical/operational-record-web-reader-contract.md`
...
- [x] Task 2: Add shared fixture examples for web-reader behavior.
  - File: `test/data/shipglows_sources/fixtures/operational_records_web_reader.md`
  ...
  - Depends on: Task 1.
  - Validate with: `flutter test test/data/shipglows_sources/parsers`
...
- [x] Task 3: Extend parser tests for web-facing read-model fields.
  ...
  - Validate with: `flutter test test/data/shipglows_sources/parsers/operational_record_parser_test.dart`
...
- [x] Task 6: Migrate project-local active trackers.
  - File: `shipglows_data/workflow/TASKS.md` and `shipglows_data/workflow/AUDIT_LOG.md`
  ...
  - Validate with: `python3 scripts/migrate_operational_records.py --dry-run` and `flutter test test/data/shipglows_sources`
  - Notes: Preserve non-operational prose only when it remains useful context.

- [x] Task 7: Migrate master trackers.
  - File: `/home/claude/shipglows_data/TASKS.md` and `/home/claude/shipglows_data/AUDIT_LOG.md`
...
- [x] Task 9: Update docs and ownership maps after migration.
  - File: `shipglows_data/technical/markdown-source-of-truth.md`, `shipglows_data/technical/code-docs-map.md`, and `/home/claude/shipglowz/skills/references/operational-record-format.md`
  ...
  - Validate with: `rg -n "operational-record-web-reader-contract|live migration|legacy fallback|traffic-first" shipglows_data/technical /home/claude/shipglowz/skills/references/operational-record-format.md`
...
- [x] Task 10: Run final migration validation and record counts.
  - File: `shipglows_data/workflow/specs/traffic-first-operational-record-live-migration-web-reader-contract.md` and `shipglows_data/workflow/verification/traffic-first-operational-record-live-migration-web-reader-contract.md`
  ...
  - Validate with: `flutter test test/data/shipglows_sources`, `cd /home/claude/shipglowz/tui && bun test && bun run typecheck`, metadata lint, and `git diff --check`
...
- `surface`: local Markdown operational sources, migration tooling, Flutter parsers, TUI readers, web-reader contract docs, and rebuildable projection fields for `task`, `audit`, and `spec` records. Covered source files include `shipglows_data/workflow/TASKS.md`, `shipglows_data/workflow/AUDIT_LOG.md`, `shipglows_data/workflow/specs/*.md`, `/home/claude/shipglows_data/TASKS.md`, `/home/claude/shipglows_data/AUDIT_LOG.md`, `scripts/migrate_operational_records.py`, `lib/data/shipglows_sources/parsers/`, `test/data/shipglows_sources/`, and `/home/claude/shipglowz/tui/src/sources/`.
...
- `checklist_path`: `shipglows_data/workflow/verification/traffic-first-operational-record-live-migration-web-reader-contract.md`.
- `required_scenario_ids`:
  ...
  - `OR-DOC-001`: technical docs and shared ShipGlows grammar reference mention the web-reader contract, live migration state, and zero-legacy post-migration policy.
...
- Run parser tests with `flutter test test/data/shipglows_sources`.
...
- Run ShipGlows metadata lint on touched specs and docs.
- Run `git diff --check` in the project repo and targeted diff checks for `/home/claude/shipglows_data/TASKS.md`.
...
- Do not stage, commit, or ship automatically unless the invoked ShipGlows lifecycle step explicitly requires it.
...
- Treat `/home/claude/shipglows_data/` as a separate high-impact target; re-read files immediately before writing.
...
| 2026-05-30 16:47:00 UTC | sf-ship | GPT-5 Codex | Shipped the verified traffic-first operational record migration and web-reader contract changes. | shipped | /sf-prod shipglows_app |
...
Next step: `/sf-prod shipglows_app`
