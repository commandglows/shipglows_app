...
depends_on:
  ...
  - artifact: "https://firebase.google.com/docs/functions/callable"
    ...
    required_status: "active"
supersedes:
  - "shipglows_data/workflow/specs/shipglows-project-source-onboarding.md as cloud/Firebase onboarding flow target"
evidence:
  - "User decision 2026-05-09: a ShipGlows project is exactly one GitHub repository."
...
ShipGlows Project Onboarding Flow
...
The onboarding flow accepts a signed-in ShipGlows user, guides them to connect or verify GitHub App access, lists only repositories GitHub confirms they can read, lets them explicitly choose one repository, creates or joins the shared ShipGlows project for that repo, starts a server-side read-only clone/index request, and shows indexing status until the project is ready or blocked. If GitHub App access, repo permission, clone, or indexing fails, the UI shows a clear recoverable state without exposing tokens, clone paths, or backend implementation details. The easy edge case to miss is auto-creating projects just because an organization installed the GitHub App; ShipGlows still waits for explicit user repo selection.

# Success Behavior

- Given the user is not signed in, onboarding starts with ShipGlows sign-in through Firebase Auth.
- Given the user is signed in but GitHub is not connected, onboarding shows a GitHub connection step.
- Given GitHub App is not installed, onboarding sends the user to the GitHub App installation flow and returns to ShipGlows with a pending verification state.
- Given GitHub App is installed on all repos or selected repos, ShipGlows lists only repos the user can read.
- Given the user selects a repo, ShipGlows creates or joins the shared `projects/{projectId}` for that GitHub repo.
- Given project creation succeeds, ShipGlows starts index/clone through the trusted backend and shows `queued`, `indexing`, then `ready`.
- Given indexing finds ShipGlows Markdown artifacts, the dashboard opens the project from Firestore projection.
...
The foundational backend and data decisions are mostly clear, but the first-run user path is not. Without a flow spec, implementation could expose technical infrastructure, create projects too early, confuse GitHub installation with ShipGlows project selection, or fail to resume long-running indexing work.
...
- Define links to Firestore, auth/access, and runner/indexer shipglows_data/workflow/specs.
...
- `shipglows_data/workflow/specs/shipglows-auth-github-access.md` defines Firebase Auth/GitHub App separation and access-loss behavior.
- `shipglows_data/workflow/specs/shipglows-firestore-data-model.md` defines project, membership, projectRefs, feedItems, indexRuns, diagnostics, and projection records.
- `shipglows_data/workflow/specs/shipglows-github-managed-clone-indexer.md` defines backend clone/index behavior.
- `shipglows_data/technical/shipglows-foundational-architecture.md` defines source-of-truth and V1 read-only invariants.
- Official GitHub App install shipglows_data checked 2026-05-10 for third-party app installation flow.
- Official Firebase Auth and callable functions shipglows_data checked 2026-05-10 for signed-in user and backend request assumptions.
...
- User preferences after onboarding live in user-scoped shipglows_data.
...
- Docs: old local-clone onboarding assumptions in `shipglows-project-source-onboarding.md` are superseded for cloud/Firebase target flow.
...
- GitHub App installed on an org before the user connects ShipGlows.
...
- Indexing succeeds but no ShipGlows Markdown artifacts are found.
...
- [ ] Task 3: Define fake backend/repository contracts consumed by onboarding.
  - File: `test/shipglows/` or nearest existing ShipGlows test directory.
...
- A repo maps to one shared ShipGlows project.
...
- Empty repo/no known ShipGlows artifacts is a valid state.
- Access-loss behavior matches `shipglows-auth-github-access.md`.
...
- `checklist_path`: `shipglows_data/workflow/verification/shipglows-project-onboarding-flow.md`.
...
- Product decision: onboarding should feel like "Connect GitHub -> Choose repository -> ShipGlows prepares it", not "configure backend/clone/database".
