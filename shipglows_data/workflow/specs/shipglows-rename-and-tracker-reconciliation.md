...
owner: "Diane"
confidence: high
user_story: "En tant qu'operatrice ShipGlows, je veux que l'application, le site et leur gouvernance utilisent le nom ShipGlows de facon coherente afin que l'ancien nom ShipGlows ne cree plus de confusion dans le produit ou les chantiers actifs."
risk_level: high
...
linked_systems:
  ...
  - "/home/claude/shipglows compatibility symlink"
depends_on:
...
supersedes: []
evidence:
  - "The repository contains 172 tracked or untracked files with ShipGlows/shipglows references."
  - "The active application directory is shipglows_app and its canonical governance corpus is the monorepo-root shipglows_data/."
  - "The global /home/claude/shipglows path is already a compatibility symlink to /home/claude/shipglows."
  - "TASKS.md reports several completed implementations while their specs still expose stale ready states and unchecked tasks."
next_step: "/104-sg-end ShipGlows rename and tracker reconciliation"
---

# Spec: ShipGlows rename and tracker reconciliation

🟢 [shipglows_app] spec: ShipGlows rename and tracker reconciliation | status: ready | path: shipglows_data/workflow/specs/shipglows-rename-and-tracker-reconciliation.md | next: /104-sg-end ShipGlows rename and tracker reconciliation

## Title

ShipGlows rename and tracker reconciliation
...
En tant qu'operatrice ShipGlows, je veux que l'application, le site et leur gouvernance utilisent le nom ShipGlows de facon coherente afin que l'ancien nom ShipGlows ne cree plus de confusion dans le produit ou les chantiers actifs.

## Minimal Behavior Contract

La migration remplace le nom produit ShipGlows par ShipGlows dans toutes les surfaces actives du depot, renomme les chemins et identifiants internes qui portent encore l'ancien nom, preserve les compatibilites externes explicitement necessaires, et rend les trackers coherents avec l'implementation constatee. Si un renommage rend un import, un chemin de donnees ou une commande invalide, les validations bloquent la fin du chantier; les archives historiques peuvent conserver l'ancien terme uniquement lorsqu'il designe explicitement l'etat passe.

## Success Behavior

- L'application et le site affichent ShipGlows sur les surfaces actives.
- Le monorepo utilise `app/`, `shipglows_data/`, `lib/shipglows/` et `lib/data/shipglows_sources/` comme chemins canoniques.
- Le package Dart et les symboles actifs utilisent `shipglows` sans imports `package:shipglows_app`.
...
- Le lien global `/home/claude/shipglows -> /home/claude/shipglows` reste une compatibilite externe documentee.

## Error Behavior

- Une reference active restante a ShipGlows bloque la cloture, sauf allowlist historique ou compatibilite documentee.
...
- `shipglows_app/` renomme en `app/`.
- Le corpus imbrique `app/shipglows_data/` est deplace vers le corpus canonique `shipglows_data/` a la racine du monorepo.
- Modules, tests, scripts, package Dart, textes et fichiers contenant `shipglows`.
...
- Suppression du lien de compatibilite global `/home/claude/shipglows`.
...
- Reecriture des archives ContentFlow sans rapport avec le nom ShipGlows.
...
- Les chemins absolus actifs doivent utiliser `/home/claude/shipglows` et `/home/claude/shipglows_app`.
...
- Les slugs publics existants peuvent conserver une redirection si leur renommage casserait des liens; sinon ils migrent vers ShipGlows.

## Documentation Coherence

- Mettre a jour README, CLAUDE, AGENT, CHANGELOG, docs techniques, contenus du site et cartes de documentation.
- Renommer les specs et preuves ShipGlows actives; conserver les mentions historiques dans les paragraphes qui decrivent explicitement la migration.
- Reparer les commandes obsoletes `/home/claude/shipglows_app` et les anciens chemins du corpus imbrique.

## Edge Cases

- Les termes `shipglows` peuvent designer une compatibilite globale encore volontaire.
...
- [x] Task 1: Establish rename inventory and historical compatibility allowlist
  - File: `shipglows_data/workflow/specs/shipglows-rename-and-tracker-reconciliation.md`
  ...
  - Depends on: none
  - Validate with: scoped `rg -i shipglows` inventory.
- [x] Task 2: Rename repository paths and Dart package/module identifiers
  - File: `shipglows_app/`, `shipglows_data/`, `lib/shipglows/`, `lib/data/shipglows_sources/`, `test/shipglows/`
  - Action: Move to `app/`, root `shipglows_data/`, `lib/shipglows/`, `lib/data/shipglows_sources/`, and `test/shipglows/`; update imports and scripts.
  - User story link: Make ShipGlows canonical in the active implementation.
  - Depends on: Task 1
...
- [x] Task 4: Reconcile trackers and completed specs from durable evidence
  - File: `shipglows_data/workflow/TASKS.md`, `shipglows_data/workflow/specs/*.md`, `shipglows_data/workflow/verification/*.md`
...
- [x] AC 1: No active UI, package, module, test, script, README or tracker identifies the product as ShipGlows.
- [x] AC 2: Canonical repository paths are `app/` and `shipglows_data/`.
...
- [x] AC 5: Remaining case-insensitive `shipglows` references are limited to documented history or compatibility.
...
```bash
rg -n -i "shipglows" app site PITCH.md
cd app && flutter test
...
None. The user explicitly selected ShipGlows as the new name; repository-local canonical paths follow that decision while the existing global symlink preserves compatibility.

## Skill Run History

| Date UTC | Skill | Model | Action | Result | Next step |
|----------|-------|-------|--------|--------|-----------|
| 2026-07-11 | 100-sg-spec | GPT-5 Codex | Created repository rename and tracker reconciliation contract | draft | /101-sg-ready ShipGlows rename and tracker reconciliation |
| 2026-07-11 | 101-sg-ready | GPT-5 Codex | Reviewed scope, migration safety, proof contract, and WIP preservation | ready | /102-sg-start ShipGlows rename and tracker reconciliation |
| 2026-07-11 | 102-sg-start | GPT-5 Codex | Renamed repository paths, Dart modules/package, governance corpus, scripts, tests, documentation, and public copy while preserving existing site WIP | implemented | /103-sg-verify ShipGlows rename and tracker reconciliation |
| 2026-07-11 | 103-sg-verify | GPT-5 Codex | Verified zero residual ShipGlows references, 137 Flutter tests, Flutter analysis, boundary suite, Astro build, metadata lint, diff integrity, and compatibility symlink | verified | /104-sg-end ShipGlows rename and tracker reconciliation |
