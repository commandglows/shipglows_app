...
owner: "Diane"
confidence: high
user_story: "En tant qu'operatrice de ShipGlows App, je veux que l'acces produit soit verifie par un entitlement serveur pour product_id=shipglows_app, afin qu'une session authentifiee, un acces GitHub ou un cache local ne puisse jamais accorder a lui seul l'acces aux donnees protegees."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
  - "ShipGlows App Flutter"
  - "ShipGlows app access state"
  - "AppBootstrap"
  - "OPEN_ACCESS"
...
evidence:
  ...
  - "Local preflight: ShipGlows App has no product entitlement ledger."
  - "Suite preflight: WinFlowz owns productEntitlements, productAccessEvents, and suiteAccess-style mirror patterns."
next_step: "/sf-ship ShipGlows Product Entitlements Compliance verification trace"
---
# Spec: ShipGlows Product Entitlements Compliance
🔴 [shipglows_app] spec: ShipGlows Product Entitlements Compliance | status: active | path: shipglows_data/workflow/specs/shipglows-product-entitlements-compliance.md | next: /sf-ship ShipGlows Product Entitlements Compliance verification trace

# Title

ShipGlows Product Entitlements Compliance
...
En tant qu'operatrice de ShipGlows App, je veux que l'acces produit soit verifie par un entitlement serveur pour `product_id=shipglows_app`, afin qu'une session authentifiee, un acces GitHub ou un cache local ne puisse jamais accorder a lui seul l'acces aux donnees protegees.

# Minimal Behavior Contract

ShipGlows App accepte une session d'identite valide, lit un snapshot d'entitlement serveur pour `product_id=shipglows_app` dans le bootstrap, et n'autorise les donnees protegees que si ce snapshot est actif ou trialing non expire, `grants_access=true`, et dans le bon environnement. Sans snapshot, avec snapshot malformed, inactive, expired, revoked, refunded, pending review, unavailable, ou avec cache seulement, l'app fail closed et affiche un etat recuperable.

# Success Behavior

- Given un bootstrap contient un snapshot `active` ou `trialing` non expire pour `shipglows_app`, with `grants_access=true` et environnement courant, when access is resolved, then `AppAccessState` devient `ready` ou `needsOnboarding`.
...
- canonical product id `shipglows_app`;
...
- Any second durable ShipGlows-local entitlement ledger.
...
- `checklist_path`: `shipglows_data/workflow/verification/shipglows-product-entitlements-compliance.md`.
...
- Suite architecture: `/home/claude/winflowz/shipglows_data/workflow/specs/unified-suite-authentication.md`.
...
- `shipglows_app` is the product namespace.
...
- `shipglows_data/technical/product-entitlement-bridge-contract.md`
- `shipglows_data/technical/product-entitlements-support-runbook.md`
- `shipglows_data/technical/firestore-data-model.md`
- `shipglows_data/technical/code-docs-map.md`
- `shipglows_data/editorial/content-map.md`
- `shipglows_data/workflow/verification/shipglows-product-entitlements-compliance.md`
...
- [x] Task 1: Create ShipGlows product entitlement contract doc.
...
- [x] AC 1: Signed-in user without `shipglows_app` entitlement is denied protected data.
...
- High product risk if ShipGlows creates a second durable ledger.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-05-30 18:53:35 UTC | sf-spec | GPT-5 Codex | Created product entitlements compliance spec from audit findings, product-entitlements playbook, local ShipGlows App scan, and suite ledger preflight. | draft saved | /sf-ready ShipGlows Product Entitlements Compliance |
| 2026-05-30 18:58:26 UTC | sf-ready | GPT-5 Codex | Reviewed structure, metadata, user-story alignment, product-entitlements doctrine, suite ledger preflight, adversarial/security readiness, language doctrine, fresh-doc gate, and proof contract. | ready | /sf-start ShipGlows Product Entitlements Compliance |
| 2026-05-30 20:44:06 UTC | sf-start | GPT-5.3 Codex Spark subagent plus GPT-5 Codex integration | Implemented local-contract-first entitlement model, access gates, route/UI states, docs, support runbook, verification checklist, and focused tests. | implemented | /sf-verify ShipGlows Product Entitlements Compliance |
| 2026-05-30 20:46:31 UTC | sf-verify | GPT-5 Codex | Verified user story, success/error behavior, fail-closed security gates, local Flutter tests, analyzer, metadata lint, docs coherence, project development mode, and checklist scenarios. | verified | /sf-end ShipGlows Product Entitlements Compliance |
| 2026-05-30 20:47:08 UTC | sf-end | GPT-5 Codex | Closed local-contract-first implementation with TASKS and CHANGELOG bookkeeping; no commit or push performed. | closed | /sf-ship ShipGlows Product Entitlements Compliance |
| 2026-05-30 20:47:50 UTC | sf-ship | GPT-5 Codex | Prepared commit and push for local-contract-first entitlement gate after focused tests, full analyzer, metadata lint, and diff hygiene passed. | shipped | /sf-prod shipglows_app |
| 2026-05-30 20:59:30 UTC | sf-verify | GPT-5 Codex | Re-verified post-ship local contract, focused tests, full analyzer, metadata lint, GitHub commit status, Vercel production deployment readiness, prod alias HTTP 200, and proof gaps. | partial | /sf-browser https://shipglowsapp.vercel.app verify Flutter web render and absence of gray page |
| 2026-05-30 21:01:09 UTC | sf-browser | GPT-5 Codex | Captured production Chromium headless screenshot and DOM proof for `https://shipglowsapp.vercel.app/`; confirmed Flutter UI renders and is not a gray page. | pass | /sf-verify ShipGlows Product Entitlements Compliance close browser proof gap |
| 2026-05-30 21:04:26 UTC | sf-verify | GPT-5 Codex | Reconciled browser proof gap with sf-browser evidence, metadata lint, production deploy proof, local test/analyzer evidence, and contract gates. | verified | /sf-ship ShipGlows Product Entitlements Compliance verification trace |
