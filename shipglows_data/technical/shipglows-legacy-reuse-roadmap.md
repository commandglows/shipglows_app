...
depends_on:
  ...
  - "shipglows_data/workflow/specs/shipglows-legacy-contentflow-fusion.md@0.3.0"
supersedes: []
evidence:
...
# ShipGlows Legacy Reuse Roadmap

## Purpose

This roadmap turns the embedded ContentFlow code into ShipGlows decisions. It is not a deletion plan. It is a decision aid for choosing what to reuse, rewrite, park, or archive.

## Owned Files

- `shipglows_data/technical/shipglows-legacy-reuse-roadmap.md`
...
| 2 | Settings/preferences | Adapt | ShipGlows will need source paths, theme, language, BYOK and project settings | Split generic settings from ContentFlow content settings |
...
- Split settings into `ShipGlowsAppSettings` later.
- Remove content-specific assumptions only when the replacement model exists.

Decision question:

- Should ShipGlows settings be per device first, per workspace first, or both with local defaults and cloud projection later?
...
- Adapt the project concept into a ShipGlows GitHub repository model.
...
- Answered 2026-05-09: a ShipGlows project is necessarily a GitHub repository. It has a local clone and a remote database projection. Future workspaces may group repositories, but project and GitHub repo are not separate concepts.
...
- Is the first hosted ShipGlows account model "one user owns private repos" or "workspace/team with shared repos"?
...
- Delay write queue design until ShipGlows write-back rules are explicit.

Decision question:

- Do we need ShipGlows to edit Markdown from the UI soon, or is V1.5 still read/index/analyze only?
