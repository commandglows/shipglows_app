...
ShipGlows App separates identity, GitHub authorization, cache, and product access. The canonical product id is `shipglows_app`.

WinFlowz suite ledger is the durable source of truth. ShipGlows App may keep a bounded snapshot or server-owned mirror, but it must not create a second durable entitlement ledger.
...
```bash
rg -n "shipglows_app|grantsAccess|suite ledger|fail closed|OPEN_ACCESS|cache" shipglows_data/technical/product-entitlements.md
