...
- `WS /v1/operator-sessions/:sessionId/stream` carries only bounded PTY input, output, resize and status frames. The capability travels as a WebSocket subprotocol rather than in the URL. A single PTY listener retains at most 256 KiB of pre-attachment output in chunks capped at 64 KiB, flushes it in order before the connected status, and retires the unattached PTY on overflow. That post-flush marker lets Flutter batch the initial bytes offscreen and publish one complete frame before live output continues; Flutter also sends its current dimensions immediately to force the correct tmux/Neovim redraw. Invalid, expired and concurrent attachments close with a bounded denial; disconnect permits bounded reconnect to the same server PTY, and the authenticated close route releases only an actor-owned session.
...
- Flutter now exposes a dedicated operator Workspace route from a server-backed project detail. It creates the short-lived protocol-v2 session in one authenticated HTTP round trip, connects through `web_socket_channel`, renders output and captures keyboard input with `xterm`, forwards bounded resize frames, and closes the socket and owned server capability concurrently on screen disposal or surface change. Editor and Terminal retain separate volatile `xterm` buffers for the current project, so revisiting a surface paints its last frame immediately while the single live capability reconnects. Incoming initial chunks are staged in a separate terminal and coalesced on a Flutter frame boundary; only after the runner's post-flush connected marker does a canonical 120 ms cross-fade publish the complete live frame, with zero duration under reduced motion. The switcher is project-keyed, input remains disabled until connected, buffers never persist, and a project change drops both without animating the outgoing project's content. Unavailable and interrupted states remain explicit; no SSH credential, server path, PTY handle or tmux identifier is presented. Cockpit and semantic Codex conversations remain the normal user surface.
