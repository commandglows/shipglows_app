...
- `BoundedProjectAiReadinessEvaluator` adds a separate advisory projection for catalog-backed projects. Version 1 scores structure, schemas/contracts, agent guidance, `llms.txt`, web sitemap/indexability when applicable, and fast feedback from lockfiles plus standard validation entry points. It reads metadata only, follows no symlink, stops at fixed depth/entry/file-size budgets, returns partial/unavailable truth without a score, and exposes only closed summaries and recommendations—never repository paths or file content. The Flutter Cockpit renders the score and its evidence separately from project health; it grants no execution or trust authority.
