...
| `app/lib/shipglows/data/managed_runner_api.dart` + project/Cockpit/conversation/Workspace providers, screens, widgets, and matching tests | Typed server-first Cockpit, one unified Projects surface with persistent active/default selection and local registry actions, normalized semantic conversations, and a separate authorized operator Workspace; explicit unavailable capability and recovery states | `shipglows_data/technical/managed-runner-foundation.md` and `shipglows_data/workflow/specs/shipglows-managed-codex-cockpit-mvp.md` | `cd app && flutter analyze lib/shipglows test/shipglows && flutter test test/shipglows` | Any runner API path, project registry/selection/default/archive/disconnect behavior, Cockpit projection, conversation event/action, approval UX, auth-token attachment, SSE/WebSocket behavior, project identity mapping, or interactive terminal rendering change |
