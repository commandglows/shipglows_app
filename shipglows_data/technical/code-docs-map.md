...
evidence:
  ...
  - "Final Studio evidence on 2026-08-16: site 13/13 with check/build/exclusion, runner 35/35 with typecheck/lint, and Flutter 24 Studio plus five theme tests (29/29 combined) with clean analysis/format."
  - "Five focused defect closures cover exact handshake validation, loop/revision ordering, atomic idempotency, distinct 256 KiB bridge-message and 16 KiB command limits, and late provider cleanup; compile still fails closed without a proved OCI worker."
next_review: "2026-09-11"
next_step: "Refresh Studio routing after dedicated OCI worker and generated compile/reload proof."
---
...
| `app/lib/domain/studio/` + `app/lib/shipglows/**/studio*` + `app/test/domain/studio/` + `app/test/shipglows/studio/` | Target-neutral Studio state and canonical-token Flutter Web shell for the real Astro iframe, exact bridge selection, semantic commands, synchronized ephemeral sessions, undo/redo, Laboratory reasons, variants, and closed runner-aligned compile intents | `shipglows_data/workflow/specs/shipglows-visual-studio-and-laboratory-mvp.md` and `shipglows_data/technical/design-system-authority.md` | `cd app && flutter analyze && flutter test test/domain/studio test/shipglows/studio test/shipglows/theme` (24 Studio plus five theme tests, 29/29 combined, with clean analysis/format on 2026-08-16) | Any Studio state, route, capability, iframe/message boundary, selection, command, journal, Laboratory, variant, compile DTO, limit, profile, or client-domain contract change |
| `runner/src/studio/` + `runner/test/studio/` + Studio route registration in `runner/src/app.ts` | Exact clean-repository/runtime attestation, authenticated capability and synchronized session routes, closed semantic commands, journal compaction, undo/redo, expiry, variants, Laboratory policy, immutable compile intent, and dedicated OCI admission contract; main injects no worker and compile fails closed | `shipglows_data/technical/managed-runner-foundation.md`, `shipglows_data/technical/operator-guides/studio-oci-worker.md`, and `shipglows_data/workflow/specs/shipglows-visual-studio-and-laboratory-mvp.md` | `cd runner && npx tsx --test test/studio/*.test.ts test/contracts/config.test.ts && npm run typecheck && npm run lint` (35/35 focused tests with clean typecheck/lint on 2026-08-16) | Any Studio schema, route, state, target profile, repository/runtime identity, command, session, idempotency, provider admission, worker capability, lifecycle, origin, or isolation rule change |
...
| Studio/OCI operations | Enablement, repository/runtime attestation, session limits, compile fail-closed behavior, worker proof checklist, recovery, and incident boundaries | `shipglows_data/technical/operator-guides/studio-oci-worker.md` | Focused Astro/Flutter/runner gates plus independent Linux worker and hosted browser proof before enablement | Any Studio configuration, repository digest, runtime origin, worker image/runtime/policy, capacity, lease, cleanup, credential, network, or public routing change |
