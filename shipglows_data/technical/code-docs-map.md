...
evidence:
  ...
  - "Managed-sandbox evidence and account-free Vercel adapter conformance passed independent verification on 2026-08-16: 48/48 focused tests, 73/73 full Studio tests, typecheck, lint, diff check, and zero high-severity offline audit findings."
...
next_step: "Refresh Studio routing after separately approved real-provider admission/probe/release evidence; compile execution/export remains a later approval."
---
...
| `runner/src/studio/` + `runner/test/studio/` + Studio route registration in `runner/src/app.ts` | Exact clean-repository/runtime attestation, authenticated capability and synchronized session routes, closed semantic commands, journal compaction, undo/redo, expiry, variants, Laboratory policy, immutable compile intent, provider-neutral managed-sandbox admission/evidence contracts, and an account-free injected Vercel facade; main injects no provider and compile fails closed | `shipglows_data/technical/managed-runner-foundation.md`, `shipglows_data/technical/operator-guides/studio-oci-worker.md`, `shipglows_data/technical/platforms/vercel.md`, and `shipglows_data/workflow/specs/shipglows-visual-studio-and-laboratory-mvp.md` | `cd runner && npx tsx --test test/studio/workerProvider.test.ts test/studio/managedSandboxAttestation.test.ts test/studio/vercelSandboxProvider.test.ts && npx tsx --test test/studio/*.test.ts && npm run typecheck && npm run lint && npm audit --audit-level=high --offline` (48/48 focused managed-sandbox tests and 73/73 full Studio tests with clean static/audit gates on 2026-08-16) | Any Studio schema, route, state, target profile, repository/runtime identity, command, session, idempotency, provider admission, evidence, resource/cost budget, lifecycle reservation, origin, isolation, network, credential, persistence, ingress, or cleanup rule change |
